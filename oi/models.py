...
if typing.TYPE_CHECKING:
    ...
    from oi.api import Self, ValuesType, IdType
    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            if '_module' not in attrs:
                ...
                assert module.startswith('oi.addons.'), \
                    f"Invalid import of {module}.{name}, it should start with 'oi.addons'."
...
class BaseModel(metaclass=MetaModel):
    """Base class for Oi models.

    Oi models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~oi.models.BaseModel._register` attribute may be set to False.
    """
    ...
    If set to ``False``, override :meth:`~oi.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~oi.models.AbstractModel`.
    """
    ...
    _module = None                      #: the model's module (in the Oi sense)
    ...
    .. warning::
      ...
      :attr:`~oi.models.Model._inherits`-ed models, the inherited field will
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields.get(field_name), oi.fields.One2many):
    ...
    def _compute_display_name(self):
        ...
        it can be made field-dependent using :attr:`~oi.api.depends` and
        context-dependent using :attr:`~oi.api.depends_context`.
        """
        if self._rec_name:
        ...
        if field.type in ('datetime', 'date') or (field.type == 'properties' and granularity):
            ...
            if granularity == 'week':
                ...
                if granularity == 'day_of_week':
                    """
                    formula: ((7 - first_day_of_week_in_oi) + result_from_SQL) %  --> 0 based first day of week
                                    week start on
                                monday   sunday    sat
                                  1     |  7    |  6   <-- first day of week in oi
                           SQL | -----------------------
                    Monday  1  |  0     |  1    |  2
                    tuesday 2  |  1     |  2    |  3
                    wed     3  |  2     |  3    |  4
    ...
    def _read_group_fill_temporal(self, data, groupby, annoted_aggregates,
        ...
        if fill_from:
            fill_from = oi.fields.Datetime.to_datetime(fill_from) if isinstance(fill_from, datetime.datetime) else oi.fields.Date.to_date(fill_from)
        ...
        if fill_to:
            fill_to = oi.fields.Datetime.to_datetime(fill_to) if isinstance(fill_to, datetime.datetime) else oi.fields.Date.to_date(fill_to)
    ...
    def _field_to_sql(self, alias: str, fname: str, query: (Query | None) = None, flush: bool = True) -> SQL:
        ...
        result to make method :meth:`~oi.api.Environment.execute_query` flush
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~oi.addons.base.models.res_company`).

        For :class:`~oi.addons.base.models.res_users` relational fields,
    ...
    def unlink(self):
        ...
        from oi.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
    ...
    def write(self, vals: ValuesType) -> typing.Literal[True]:
        ...
        * For numeric fields (:class:`~oi.fields.Integer`,
          :class:`~oi.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~oi.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~oi.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~oi.fields.Many2one`, the value should be the
          database identifier of the record to set
        * The expected value of a :class:`~oi.fields.One2many` or
          :class:`~oi.fields.Many2many` relational field is a list of
          :class:`~oi.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~oi.fields.Command.create`,
          :meth:`~oi.fields.Command.update`,
          :meth:`~oi.fields.Command.delete`,
          :meth:`~oi.fields.Command.unlink`,
          :meth:`~oi.fields.Command.link`,
          :meth:`~oi.fields.Command.clear`, and
          :meth:`~oi.fields.Command.set`.
        * For :class:`~oi.fields.Date` and `~oi.fields.Datetime`,
          ...
          .. warning::
            ...
            :const:`oi.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
            :const:`oi.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an Oi domain.
    ...
    def ensure_one(self) -> Self:
        ...
        :raise oi.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env: api.Environment) -> Self:
        ...
        :param env:
        :type env: :class:`~oi.api.Environment`
    ...
    def with_company(self, company) -> Self:
        ...
        :type company: :class:`~oi.addons.base.models.res_company` or int
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted Oi models.

    Oi models are created by inheriting from this class::
