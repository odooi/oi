# -*- coding: utf-8 -*-
"""
The module :mod:`oi.tests.common` provides unittest test cases and a few
...
import oi
from oi import api
from oi.exceptions import AccessError
from oi.fields import Command
from oi.http import BadRequest
from oi.modules import module
from oi.modules.registry import Registry
from oi.service import security
from oi.sql_db import BaseCursor, Cursor
from oi.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from oi.tools.mail import single_email_re
from oi.tools.misc import find_in_path, lower_logging
from oi.tools.xml_utils import _validate_xml
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: oi.tests.common.Form is deprecated, use oi.tests.Form",
...
# The oi library is supposed already configured.
ADDONS_PATH = oi.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = oi.SUPERUSER_ID
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding Oi module name.
    """
    def register(func):
        # register func by oi module name
        if func.__module__.startswith('oi.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most oi operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('oi.addons.'):
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for Oi-specific code. This class is abstract and
    expects self.registry, self.cr and self.uid to be initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.odoodooi.com/r/Vmh
    warm = True             # False during warm-up phase (see :func:`warmup`)
    _python_version = sys.version_info

    _tests_run_count = int(os.environ.get('OI_TEST_FAILURE_RETRIES', 0)) + 1
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~oi.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(oi.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            oi.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = oi.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('oi.sql_db.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/oi/addons/" in filename:
                        filename = filename.rsplit("/oi/addons/", 1)[1]
                    if count > expected:
    ...
    def check_test_cursor(self, operation):
        if oi.modules.module.current_test != self:
            message = f"Trying to open a test cursor for {self.canonical_tag} while already in a test {oi.modules.module.current_test.canonical_tag}"
            _logger.runbot(message)
            raise BadRequest(message)
        request = oi.http.request
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | oi.addons.base.models.res_currency.Currency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(oi.modules.registry._logger.name)
    freeze_time = None
    ...
    def _gc_filestore(cls):
        ...
        with Registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, oi.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls.env = api.Environment(cls.cr, oi.SUPERUSER_ID, {})
        ...
        cls._crypt_context_patcher = patch('oi.addons.base.models.res_users.Users._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(oi.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_oi')
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way Oi widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    @classmethod
    def http_port(cls):
        if oi.service.server.server is None:
            return None
        return oi.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('oi.service.http.request.')]
        ...
        if request_threads:
            ...
            oi.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        oi.http.root.session_store.save(self.session)

    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            oi.http.root.session_store.delete(self.session)

        self.session = session = oi.http.root.session_store.new()
        session.update(oi.http.get_default_session(), db=get_db_name())
        session.context['lang'] = oi.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('oi.addons.base.models.res_users.Users._check_credentials', new=patched_check_credentials):
        ...
        oi.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, cookies=None, error_checker=None, watch=False, success_signal=DEFAULT_SUCCESS_SIGNAL, debug=False, cpu_throttling=None, **kw):
        ...
        try:
            ...
            cpu_throttling_os = os.environ.get('OI_BROWSER_CPU_THROTTLING') # used by dedicated runbot builds
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        options = {
            ...
            'startUrl': url_path,
            'delayToCheckUndeterminisms': kwargs.pop('delay_to_check_undeterminisms', int(os.getenv("OI_TOUR_DELAY_TO_CHECK_UNDETERMINISMS", "0")) or 0),
        }
        code = kwargs.pop('code', f"oi.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"oi.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('oi.http.Request._get_profiler_context_manager', route_profiler))
...
def tagged(*tags):
    ...
    By default, all Test classes from oi.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in Oi test suites
