...
import oi
from oi.tools import config, topological_sort, unique
from oi.modules.registry import Registry
from oi.netsvc import init_logger
from oi.tests import standalone_tests
import oi.tests.loader

_logger = logging.getLogger('oi.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Oi modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra Oi modules")
...
def test_standalone(args):
    ...
    for module_name in registry._init_modules:
        ...
        oi.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        oi.tools.config['addons_path'] = ','.join([args.addons_path, oi.tools.config['addons_path']])
        if args.data_dir:
            oi.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'oi.modules.loading': {'level': 'CRITICAL'},
            'oi.sql_db': {'level': 'CRITICAL'},
            'oi.models.unlink': {'level': 'WARNING'},
            'oi.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
