...
from .suite import OiSuite
from .result import OiTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'oi.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"oi.upgrade.{module}",
        f"oi.addons.{module}.migrations",
        f"oi.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return OiSuite(sorted(tests, key=lambda t: t.test_sequence))
...
def run_suite(suite, global_report=None):
    ...
    results = OiTestResult(global_report=global_report)
    suite(results)
