...
from oi import modules
from oi.exceptions import UserError
from oi.tools import config, file_path, mute_logger
...
@patch('oi.addons.base.models.ir_mail_server.SMTP_TIMEOUT', .1)
...
class TestIrMailServerSMTPD(TransactionCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        # decrease aiosmtpd verbosity, oi INFO = aiosmtpd WARNING
        ...
        # force aiosmtpd/oi to bind/connect to a fixed ipv4 OR ipv6
        # address.
    ...
    def test_man_in_the_middle_matrix(self):
        ...
        traffic in between the Oi server and the legitimate SMTP
        server.
        """
        mail_server = self.env['ir.mail_server'].create({
            'name': 'test smtpd',
