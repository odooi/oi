...
from oi.tools import pdf
from oi.tools.misc import file_open
from oi.tools.pdf import reshape_text
import io
...
class TestPdf(TransactionCase):
    ...
    def setUp(self):
        ...
        self.minimal_pdf_reader = pdf.OiPdfFileReader(self.minimal_reader_buffer)

    def test_oi_pdf_file_reader(self):
        ...
        r = pdf.OiPdfFileReader(io.BytesIO(out.getvalue()))
        self.assertEqual(len(list(r.getAttachments())), 1)

    def test_oi_pdf_file_writer(self):
        attachments = list(self.minimal_pdf_reader.getAttachments())
        self.assertEqual(len(attachments), 0)
        ...
        for count, (name, data) in enumerate([
            ...
            pdf_writer = pdf.OiPdfFileWriter()
    ...
    def test_oi_pdf_file_reader_with_owner_encryption(self):
        pdf_writer = pdf.OiPdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
        ...
        with io.BytesIO(encrypted_content) as reader_buffer:
            pdf_reader = pdf.OiPdfFileReader(reader_buffer)
    ...
    def test_merge_pdf(self):
        ...
        merged_pdf_reader = pdf.OiPdfFileReader(merged_reader_buffer)
    ...
    def test_branded_file_writer(self):
        ...
        self.assertEqual(pdf_info['/Producer'], 'Oi')
        self.assertEqual(pdf_info['/Creator'], 'Oi')
        reader_buffer.close()
