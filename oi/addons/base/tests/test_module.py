...
import oi.addons
from oi.modules.module import load_manifest
from oi.modules.module import get_manifest
from oi.release import major_version
from oi.tests.common import BaseCase


class TestModuleManifest(BaseCase):
    @classmethod
    def setUpClass(cls):
        cls._tmp_dir = tempfile.TemporaryDirectory(prefix='oi-test-addons-')
        cls.addClassCleanup(cls._tmp_dir.cleanup)
        cls.addons_path = cls._tmp_dir.name

        patcher = patch.object(oi.addons, '__path__', [cls.addons_path])
        cls.startClassPatcher(patcher)

    def setUp(self):
        self.module_root = tempfile.mkdtemp(prefix='oi-test-module-', dir=self.addons_path)
    ...
    def test_default_manifest(self):
        ...
        with self.assertNoLogs('oi.modules.module', 'WARNING'):
    ...
    def test_missing_manifest(self):
        with self.assertLogs('oi.modules.module', 'DEBUG') as capture:
            manifest = load_manifest(self.module_name)
    ...
    def test_missing_license(self):
        ...
        with self.assertLogs('oi.modules.module', 'WARNING') as capture:
            manifest = load_manifest(self.module_name)
