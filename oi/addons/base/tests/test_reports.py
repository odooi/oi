...
@oi.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(oi.tests.TransactionCase):
...
class TestReportsRenderingCommon(oi.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        oi.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@oi.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@oi.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
...
class TestAggregatePdfReports(oi.tests.HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
