...
import oi
from oi.http import Controller, route, dispatch_rpc, request, Response
from oi.fields import Date, Datetime, Command
from oi.tools import lazy
from oi.tools.misc import frozendict
...
# User code must use the exceptions defined in ``oi.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, oi.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, oi.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, oi.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, oi.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, oi.exceptions.MissingError):
    ...
    #InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(oi.tools.exception_to_unicode(e), formatted_info)

    return dumps(fault)


class OiMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, oi is sending binary as base64 string.
...
def dumps(params: list | tuple | xmlrpc.client.Fault) -> str:
    response = OiMarshaller(allow_none=False).dumps(params)
    return f"""\
<?xml version="1.0"?>
<methodResponse>
{response}
...
class RPC(Controller):
    ...
    def jsonrpc(self, service, method, args):
        """ Method used by client APIs to contact Oi. """
        _check_request()
