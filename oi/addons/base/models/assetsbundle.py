...
from oi import release, SUPERUSER_ID, _
from oi.http import request
from oi.tools import (func, misc, transpile_javascript,
    is_oi_module, SourceMapGenerator, profiler, OrderedSet)
from oi.tools.json import scriptsafe as json
from oi.tools.constants import SCRIPT_EXTENSIONS, STYLE_EXTENSIONS
from oi.tools.misc import file_open, file_path
            ...
            if self.templates:
                ...
                template_bundle = textwrap.dedent(f"""
                    ...
                    *******************************************/
                    ...
                    oi.define("{self.name}.bundle.xml", ["@web/core/templates"], function(require) {{
                        "use strict";
        ...
        * 't-inherit-mode':  'primary' or 'extension'.
        ...
        for asset in self.templates:
            ...
            try:
                ...
                template = content if content.startswith('<oi>') else f'<templates>{asset.content}</templates>'
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            if '.' not in ref and line not in imports and not ref.startswith(('.', '/', '~')):
                imports.append(line)
                return line
            msg = "Local import '%s' is forbidden for security reasons. Please remove all @import {your_file} imports in your custom files. In Oi you have to import all files in the assets, and not through the @import statement." % ref
            _logger.warning(msg)
...
class JavascriptAsset(WebAsset):
    ...
    def is_transpiled(self):
        if self._is_transpiled is None:
            self._is_transpiled = bool(is_oi_module(self.url, super().content))
