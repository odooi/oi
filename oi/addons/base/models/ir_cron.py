...
BASE_VERSION = oi.modules.get_manifest('base')['version']
...
OI_NOTIFY_FUNCTION = os.getenv('OI_NOTIFY_FUNCTION', 'pg_notify')
...
class ir_cron(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['usage'] = 'ir_cron'
        if os.getenv('OI_NOTIFY_CRON_CHANGES'):
    ...
    def _process_jobs(cls, db_name):
        ...
        try:
            ...
            # The table ir_cron does not exist; this is probably not an Oi database.
    ...
    def _check_modules_state(cls, cr, jobs):
        ...
        # reset_module_states.
        oi.modules.reset_modules_state(cr.dbname)
    ...
    def write(self, vals):
        self._try_lock()
        if ('nextcall' in vals or vals.get('active')) and os.getenv('OI_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list):
        ...
        if min(at_list) <= now or os.getenv('OI_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
        return triggers

    def _notifydb(self):
        """ Wake up the cron workers
        The OI_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        ir_cron modification and on trigger creation (regardless of call_at)
        """
        with oi.sql_db.db_connect('postgres').cursor() as cr:
            cr.execute(SQL("SELECT %s('cron_trigger', %s)", SQL.identifier(OI_NOTIFY_FUNCTION), self.env.cr.dbname))
