...
from oi.sql_db import close_db, db_connect
from oi.tests import HOST, BaseCase, Like, get_db_name, tagged
from oi.tools import lazy_property, mute_logger, SQL
...
class TestHttpRegistry(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.addClassCleanup(lazy_property.reset_all, oi.http.root)
        cls.classPatch(oi.conf, 'server_wide_modules', ['base', 'web', 'test_http'])

        # make sure there are always many databases, to break monodb
        cls._db_list = cls.startClassPatcher(patch('oi.http.db_list'))
        cls._db_list.return_value = ['postgres', get_db_name()]
        cls.startClassPatcher(patch('oi.http.db_filter',
    ...
    def authenticate(self, *, db=None):
        session = oi.http.root.session_store.new()
        session.update(oi.http.get_default_session(), db=db or get_db_name())
    ...
    def url_open(self, path, *, allow_redirects=False):
        ...
        url = urljoin(f"http://{HOST}:{oi.tools.config['http_port']}", path)
    ...
    def test_signaling(self):
        ...
        with self.assertLogs('oi.modules.registry', logging.INFO) as capture:
            res = self.url_open('/test_http/ensure_db')
            self.assertEqual(res.status_code, 200)
        self.assertEqual(capture.output, [
            "INFO:oi.modules.registry:Reloading the model registry after database signaling.",
            Like("INFO:oi.modules.registry:Registry loaded in ...s"),
        ])
    ...
    def test_missing_db(self):
        ...
        with self.assertLogs('oi.http', logging.WARNING) as capture:
            res = self.url_open('/test_http/ensure_db')
        ...
        self.assertEqual(capture.output, [
            Like("WARNING:oi.http:Database or registry unusable, trying without\n"
                 f'Traceback...database "{db_duplicate}" does not exist...')
        ] * 2)

    @mute_logger('oi.sql_db')
    def test_corrupt_ir_module_module_table(self):
        ...
        with self.assertLogs('oi.modules.registry', logging.ERROR) as capture1, \
             self.assertLogs('oi.http', logging.WARNING) as capture2:
            res = self.url_open('/test_http/greeting-public')
            self.assertEqual(res.status_code, 404)
        self.assertEqual(capture1.output, [
            "ERROR:oi.modules.registry:Failed to load registry",
        ])
