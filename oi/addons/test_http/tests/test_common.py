...
import oi
from oi.http import Session
from oi.addons.base.tests.common import HttpCaseWithUserDemo
from oi.tools.func import lazy_property
from oi.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(oi.http.Application, 'session_store', session_store)
        cls.classPatch(oi.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(oi.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        oi.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('oi.http.db_list') as db_list, \
             patch('oi.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('oi.http.db_list') as db_list, \
             patch('oi.http.db_filter') as db_filter, \
             patch('oi.http.Registry') as Registry:
