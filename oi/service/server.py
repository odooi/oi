...
import oi
from oi.modules import get_modules
from oi.modules.registry import Registry
from oi.release import nt_service_name
from oi.tools import config
from oi.tools.cache import log_ormcache_stats
from oi.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    if oi.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'oi.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The OI_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("OI_MAX_HTTP_THREADS")
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in oi.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="oi.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from oi.addons.base.models.ir_cron import ir_cron
        def _run_cron(cr):
            pg_conn = cr._cnx
            ...
            while config['limit_time_worker_cron'] <= 0 or (time.monotonic() - alive_time) <= config['limit_time_worker_cron']:
                ...
                registries = oi.modules.registry.Registry.registries
        ...
        while True:
            conn = oi.sql_db.db_connect('postgres')
    ...
    def cron_spawn(self):
        ...
        for i in range(oi.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="oi.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from oi.tests.result import _logger as logger  # noqa: PLC0415
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('oi: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(oi.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The OI_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("OI_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = oi.service.db.list_dbs(True)
        return db_names
    ...
    def start(self):
        ...
        dbconn = oi.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = list(oi.conf.server_wide_modules)
    server_wide_modules.extend(m for m in ('base', 'web') if m not in server_wide_modules)
    for m in server_wide_modules:
        try:
            oi.modules.module.load_oi_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the addons found in the `oi-web` project.
Maybe you forgot to add those addons in your addons_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute oi-server process with (nearly) the same arguments"""
    if oi.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    ...
    from oi.tests import loader  # noqa: PLC0415
    from oi.tests.suite import OiSuite  # noqa: PLC0415
            ...
            if config['test_enable']:
                ...
                if post_install_suite.has_http_case():
                    with registry.cursor() as cr:
                        env = oi.api.Environment(cr, oi.SUPERUSER_ID, {})
...
def start(preload=None, stop=False):
    """ Start the oi http server and cron processor.
    """
    global server
    ...
    if oi.evented:
        server = GeventServer(oi.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(oi.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Oi is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(oi.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not oi.evented:
        if inotify:
