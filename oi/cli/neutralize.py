...
class Neutralize(Command):
    ...
    def run(self, args):
        ...
        opt = oi.tools.config.parse_config(args, setup_logging=True)

        dbname = oi.tools.config['db_name']
        if not dbname:
        ...
        try:
            with oi.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = oi.modules.neutralize.get_installed_modules(cursor)
                    queries = oi.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    oi.modules.neutralize.neutralize_database(cursor)
