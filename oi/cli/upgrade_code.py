...
* addon: the oi addon in which the file is;
...
try:
    ...
    from oi.modules import initialize_sys_path
    from oi.tools import config, parse_version
except ImportError:
    # Assume the script is directy executed (by opposition to be
    # executed via oi-bin), happily release/parse_version are
    # standalone so we can hack our way there without importing oi
...
class UpgradeCode(Command):
    """ Rewrite the entire source code using the scripts found at /oi/upgrade_code """
    name = 'upgrade_code'
    ...
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            ...
            description=__doc__.replace('/oi/upgrade_code', str(UPGRADE)),
    ...
    def run(self, cmdargs):
        ...
        if initialize_sys_path:
            ...
            initialize_sys_path()
            options.addons_path = oi.addons.__path__
        else:
