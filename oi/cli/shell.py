...
class Shell(Command):
    """Start oi in an interactive shell"""
    ...
    def init(self, args):
        ...
        oi.cli.server.report_configuration()
        oi.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                uid = oi.SUPERUSER_ID
                ctx = oi.api.Environment(cr, uid, {})['res.users'].context_get()
                env = oi.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
