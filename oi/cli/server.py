...
"""
Oi - Server
Oi is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - Oi SA
"""
...
__author__ = oi.release.author
__version__ = oi.release.version

# Also use the `oi` logger for the main script.
_logger = logging.getLogger('oi')

re._MAXCACHE = 4096  # default is 512, a little too small for oi
...
def setup_pid_file():
    ...
    """
    config = oi.tools.config
    if not oi.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = oi.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            oi.tools.translate.trans_export(config["language"],
...
def import_translation():
    ...
    registry = oi.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = oi.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    oi.tools.config.parse_config(args, setup_logging=True)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                oi.service.db._create_empty_database(db_name)
            ...
            except oi.service.db.DatabaseExists:
                pass
    ...
    setup_pid_file()
    rc = oi.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the oi server (default command)"""
    def run(self, args):
        oi.tools.config.parser.prog = f'{Path(sys.argv[0]).name} {self.name}'
        main(args)
