...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "oi.netsvc.log is deprecated starting Oi 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if tools.config['log_db'] != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('oi.sql_db'), sql_db.db_connect(tools.config['log_db'], allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('oi.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='oi.tools.image')
    ...
    # Check that handler.stream has a fileno() method: when running Oi
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("OI_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'oi.http.rpc.request:INFO',
    'oi.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['oi:DEBUG', 'oi.sql_db:INFO', 'oi.http.rpc:DEBUG'],
    'debug_rpc': ['oi:DEBUG', 'oi.sql_db:INFO', 'oi.http.rpc.request:DEBUG'],
    'debug': ['oi:DEBUG', 'oi.sql_db:INFO'],
    'debug_sql': ['oi.sql_db:DEBUG'],
    'info': [],
    'runbot': ['oi:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['oi:WARNING', 'werkzeug:WARNING'],
    'error': ['oi:ERROR', 'werkzeug:ERROR'],
    'critical': ['oi:CRITICAL', 'werkzeug:CRITICAL'],
}
