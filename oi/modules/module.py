...
import oi
import oi.tools as tools
import oi.release as release
...
MANIFEST_NAMES = ('__manifest__.py', '__oi__.py')
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `oi.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^oi\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("oi.addons.base.maintenance.migrations", "oi.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``oi.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook oi.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in oi.addons.__path__:
        oi.addons.__path__.append(dd)

    # hook oi.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(ad.strip()))
        if ad not in oi.addons.__path__:
            oi.addons.__path__.append(ad)

    # hook oi.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in oi.addons.__path__ and os.path.isdir(base_path):
        oi.addons.__path__.append(base_path)

    # hook oi.upgrade on upgrade-path
    from oi import upgrade
    ...
    # create decrecated module alias from oi.addons.base.maintenance.migrations to oi.upgrade
    spec = importlib.machinery.ModuleSpec("oi.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["oi.addons.base.maintenance"] = maintenance_pkg
    sys.modules["oi.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from oi to oi and "crm"-like to oi.addons
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(oi.addons.__path__, key=len, reverse=True)
...
def module_manifest(path):
    ...
    for manifest_name in MANIFEST_NAMES:
        ...
        if os.path.isfile(candidate):
            if manifest_name == '__oi__.py':
                warnings.warn(
                    "__oi__.py manifests are deprecated since 17.0, "
...
def load_oi_module(module_name):
    """ Load an Oi module, if not already loaded.
    ...
    qualname = f'oi.addons.{module_name}'
