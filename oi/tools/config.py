...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://services.odoodooi.com/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.oirc (or to ~/.oi_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store Oi data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'oi_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "oi.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="oi.http:DEBUG", help='shortcut for --log-handler=oi.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="oi.sql_db:DEBUG", help='shortcut for --log-handler=oi.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate Oi to another language. "
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        This method initializes oi.tools.config and oi.conf (the
        ...
        Typical usage of this method:
            ...
            oi.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        if setup_logging is not False:
            oi.netsvc.init_logger()
            ...
            if setup_logging is None:
                warnings.warn(
                    "As of Oi 18, it's recommended to specify whether"
                    " you want Oi to setup its own logging (or want to"
                    " handle it yourself)",
                    category=PendingDeprecationWarning,
                    stacklevel=2,
                )
        self._warn_deprecated_options()
        oi.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'oi.conf')
        else:
            rcfilepath = os.path.expanduser('~/.oirc')
            old_rcfilepath = os.path.expanduser('~/.oi_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.oirc' and '.oi_serverrc' in your path. Please keep only one of "\
                "them, preferably '.oirc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('OI_RC') or os.environ.get('OI_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from oi.modules.module import MANIFEST_NAMES
