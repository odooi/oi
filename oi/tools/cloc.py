...
import oi
from oi.tools.config import config
...
STANDARD_MODULES = ['web', 'web_plus', 'theme_common', 'base']
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in oi.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [oi.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(oi.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be oi/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        registry = oi.modules.registry.Registry(config['db_name'])
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="Oi cloc", lines="Line", other="Other", code="Code")
        s += hr
