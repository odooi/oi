...
PYTHON_TRANSLATION_COMMENT = 'oi-python'

# translation used for javascript code in web client
JAVASCRIPT_TRANSLATION_COMMENT = 'oi-javascript'
...
def get_translated_module(arg: str | int | typing.Any) -> str:  # frame not represented as hint
    ...
    if isinstance(arg, str):
        if arg.startswith('oi.addons.'):
            # get the name of the module
            return arg.split('.')[2]
        if '.' in arg or not arg:
            # module name is not in oi.addons.
            return 'base'
        else:
            return arg
    else:
        ...
        if not frame:
            return 'base'
        if (module_name := frame.f_globals.get("__name__")) and module_name.startswith('oi.addons.'):
            # just a quick lookup because `get_resource_from_path is slow compared to this`
            return module_name.split('.')[2]
        path = inspect.getfile(frame)
        path_info = oi.modules.get_resource_from_path(path)
...
def _get_cr(frame):
    ...
    if (local_self := frame.f_locals.get('self')) is not None:
        ...
        if (cr := getattr(local_self, 'cr', None)) is not None:
            return cr
    try:
        from oi.http import request  # noqa: PLC0415
...
def TranslationFileReader(source, fileformat='po'):
    """ Iterate over translation file to return Oi translation entries """
    if fileformat == 'csv':
...
class PoFileReader:
    """ Iterate over po file to return Oi translation entries """
    def __init__(self, source):
...
class TranslationReader:
    def __init__(self, cr, lang=None):
        ...
        self.env = oi.api.Environment(cr, oi.SUPERUSER_ID, {})
        self._to_translate = []
...
class TranslationModuleReader(TranslationReader):
    ...
    def __init__(self, cr, modules=None, lang=None):
        ...
        self._path_list = [(path, True) for path in oi.addons.__path__]
            ...
            for root, dummy, files in os.walk(path, followlinks=True):
                ...
                if fnmatch.fnmatch(root, '*/static/src*'):
                    ...
                    for fname in fnmatch.filter(files, '*.xml'):
                        self._babel_extract_terms(fname, path, root, 'oi.tools.translate:babel_extract_qweb',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if fnmatch.fnmatch(root, '*/data/*'):
                    for fname in filter(spreadsheet_files_regex.match, files):
                        self._babel_extract_terms(fname, path, root, 'oi.tools.translate:extract_spreadsheet_terms',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if not recursive:
...
def get_po_paths(module_name: str, lang: str, env: oi.api.Environment | None = None):
...
def _get_translation_upgrade_queries(cr, field):
    ...
    """
    from oi.modules.registry import Registry  # noqa: PLC0415
