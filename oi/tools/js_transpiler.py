"""
This code is what let us use ES6-style modules in oi.
Classic Oi modules are composed of a top-level :samp:`oi.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@oi-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** oi-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual oi one with minimal changes.
...
from oi.tools.misc import OrderedSet

_logger = logging.getLogger(__name__)

def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom oi modules.
    ...
    legacy_oi_define = get_aliased_oi_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_oi_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_oi_define:
        content += legacy_oi_define
    return content
...
def url_to_module_path(url):
    """
    Oi modules each have a name. (oi.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_oi_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the oi.define call.
    ...
    """
    return f"""oi.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
OI_MODULE_RE = re.compile(r"""
    \s*                             # starting white space
    \/(\*|\/)                       # /* or //
    .*                              # any comment in between (optional)
    @oi-module                    # '@oi-module' statement
...
def is_oi_module(url, content):
    """
    Detect if the file is a native oi module.
    We look for a comment containing @oi-module.

    :param url:
    :param content: source code
    :return: is this a oi module that need transpilation ?
    """
    result = OI_MODULE_RE.match(content)
...
def get_aliased_oi_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the oi source base being:
    ...
    > oi.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @oi-module */
        // after
        /** @oi-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @oi-module */
        // after
        /** @oi-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = OI_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\noi.define(`%s`, ['%s'], function (require) {
                        return require('%s');
