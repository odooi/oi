...
Oi domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False, filename=None):
    ...
    an Oi domain expression from an untrusted source.
    ...
    except oi.exceptions.UserError:
        raise
    except oi.exceptions.RedirectWarning:
        raise
...
`oi.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `oi.tools.safe_eval`.
""")
    return d
...
from oi._monkeypatches.pytz import patch_pytz  # noqa: E402, F401
patch_pytz()
