...
version_info = ...
nt_service_name = ...
exec(open(os.path.join(ROOTDIR, 'oi', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/oi && \
    groupadd -g %(group_id)s oi && \
    useradd -u %(user_id)s -g oi oi -d /var/lib/oi && \
    mkdir /data && \
    chown oi:oi /var/lib/oi /data
USER oi
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class OiTestTimeoutError(Exception):
    pass


class OiTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise OiTestError("Installation of package failed")
        else:
        ...
        raise OiTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/oi_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.oi_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'oi/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'oi-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
        ...
        self.docker_template = Path(docker_templates[self.arch]).read_text(encoding='utf-8').replace('USER oi', DOCKERUSER)
    ...
    def run(self, cmd, build_dir, container_name, user='oi', exposed_port=None, detach=False, timeout=None):
    ...
    def test_oi(self):
        logging.info('Starting to test Oi install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'oi.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise OiTestTimeoutError('Oi pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise OiTestError('Error while installing/starting Oi after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'oi-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/oi-*.tar.gz' % self.args.build_dir)[0], '%s/oi_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/oi-*.zip' % self.args.build_dir)[0], '%s/oi_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su postgres -s /bin/bash -c "createuser -s oi"',
            'su oi -s /bin/bash -c "python3 -m venv /var/lib/oi/oivenv"',
            'su oi -s /bin/bash -c "/var/lib/oi/oivenv/bin/python3 -m pip install --upgrade pip"',
            'su oi -s /bin/bash -c "/var/lib/oi/oivenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su oi -s /bin/bash -c "/var/lib/oi/oivenv/bin/python3 -m pip install /data/src/oi_{VERSION}.{TSTAMP}.tar.gz"',
            'su oi -s /bin/bash -c "createdb mycompany"',
            'su oi -s /bin/bash -c "/var/lib/oi/oivenv/bin/oi -d mycompany -i base --stop-after-init"',
            'su oi -s /bin/bash -c "/var/lib/oi/oivenv/bin/oi -d mycompany --pidfile=/data/src/oi.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'oi-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_oi()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/oi (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../oi_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'oi-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f'/usr/bin/apt-get install -y /data/src/oi_{VERSION}.{TSTAMP}_all.deb',
            'su oi -s /bin/bash -c "oi -d mycompany -i base --pidfile=/data/src/oi.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'oi-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_oi()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/oi/rpmbuild'
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/oi.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./oi-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/oi-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" /data/src/setup/rpm/oi.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/oi*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'oi-rpm-build-{TSTAMP}')
        os.rename(glob('%s/dist/oi-*.noarch.rpm' % self.args.build_dir)[0], '%s/oi_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'sleep 5',
            'su postgres -c "createuser -s oi"',
            'su oi -c "createdb mycompany"',
            'dnf install -d 0 -e 0 /data/src/oi_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su oi -s /bin/bash -c "oi -c /etc/oi/oi.conf -d mycompany -i base --stop-after-init"',
            'su oi -s /bin/bash -c "oi -c /etc/oi/oi.conf -d mycompany --pidfile=/data/src/oi.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'oi-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_oi()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'oi-rpm-createrepo-%s' % TSTAMP)
...
class DockerWine(Docker):
    ...
    def build(self):
        ...
        container_python = '/var/lib/oi/.wine/drive_c/oibuild/WinPy64/python-3.12.3.amd64/python.exe'
        ...
        cmds = [
            ...
            rf'wine "c:\nsis-3.11\makensis.exe" {nsis_args} "c:\oibuild\server\setup\win32\setup.nsi"',
            rf'wine {container_python} -m pip list'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'oi-win-build-%s' % TSTAMP)
