...
!ifndef SERVICENAME
	!define SERVICENAME 'oi-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\oibuild'
!endif

!define PRODUCT_NAME "Oi"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Oi Server ${VERSION}"
...
OutFile "${TOOLSDIR}\server\oi_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\oi-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\oi-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\oi-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\oi-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.odoodooi.com/page/contactus"
...
; English
LangString DESC_Oi_Server ${LANG_ENGLISH} "Install the Oi Server with all the Oi standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Oi."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Oi for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the Oi Server"
...
LangString Profile_AllInOne ${LANG_ENGLISH} "Oi Server And PostgreSQL Server"
LangString Profile_Server ${LANG_ENGLISH} "Oi Server Only"
LangString Profile_IOT ${LANG_ENGLISH} "Oi IoT"
LangString TITLE_Oi_Server ${LANG_ENGLISH} "Oi Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString TITLE_IOT ${LANG_ENGLISH} "Oi IoT"
LangString TITLE_Nginx ${LANG_ENGLISH} "Nginx WebServer"
LangString TITLE_Ghostscript ${LANG_ENGLISH} "Ghostscript interpreter"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start Oi"
LangString UnsafeDirText ${LANG_ENGLISH} "Installing outside of $PROGRAMFILES64 is not recommended.$\nDo you want to continue ?"

; French
LangString DESC_Oi_Server ${LANG_FRENCH} "Installation du Serveur Oi avec tous les modules Oi standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par Oi."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Oi pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur Oi"
...
LangString Profile_AllInOne ${LANG_FRENCH} "Serveur Oi Et Serveur PostgreSQL"
LangString Profile_Server ${LANG_FRENCH} "Seulement Le Serveur Oi"
LangString Profile_IOT ${LANG_FRENCH} "Oi IoT"
LangString TITLE_Oi_Server ${LANG_FRENCH} "Serveur Oi"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString TITLE_IOT ${LANG_FRENCH} "Oi IoT"
...
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer Oi"
...
Section $(TITLE_Oi_Server) SectionOi_Server
    SectionIn 1 2 3
    ...
    File /r /x "wkhtmltopdf" /x "plus" "${TOOLSDIR}\server\*"
    ...
    # If there is a previous install of the Oi Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\oi.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\oi.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\oi.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\oi.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\oi.conf" "options" "addons_path" "$INSTDIR\server\oi\addons"
    WriteIniStr "$INSTDIR\server\oi.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    # Set data_dir
    WriteIniStr "$INSTDIR\server\oi.conf" "options" "data_dir" "$INSTDIR\sessions"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\oi.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    # Productivity Apps
    WriteIniStr "$INSTDIR\server\oi.conf" "options" "default_productivity_apps" "True"
    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\oi-bin" --stop-after-init -c "$INSTDIR\server\oi.conf" --logfile "$INSTDIR\server\oi.log" -s'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\oi-bin\" -c "\"$INSTDIR\server\oi.conf\"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} ObjectName "LOCALSERVICE"'
    AccessControl::GrantOnFile  "$INSTDIR" "LOCALSERVICE" "FullAccess"

    Call RestartOiService
SectionEnd
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    StrCpy $postgresql_url "https://get.plusdb.com/postgresql/$postgresql_exe_filename"
...
Section $(TITLE_IOT) IOT
    ...
    WriteIniStr "$INSTDIR\server\oi.conf" "options" "server_wide_modules" "web,hw_posbox_homepage,hw_drivers"
    WriteIniStr "$INSTDIR\server\oi.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\server\oi.conf" "options" "max_cron_threads" "0"
    nsExec::ExecToStack '"$INSTDIR\python\python.exe" "$INSTDIR\server\oi-bin" genproxytoken'
    pop $0
    pop $ProxyTokenPwd
SectionEnd
...
Section $(TITLE_Nginx) Nginx
    ...
    File "..\..\oi\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\certs\nginx-cert.crt"
    File "..\..\oi\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\private\nginx-cert.key"
SectionEnd
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@odoodooi.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.odoodooi.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@odoodooi.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionOi_Server} $(DESC_Oi_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\Oi ${VERSION}"
...
Function ShowProxyTokenDialogPage
    ...
    doProxyToken:
        ...
        ${NSD_CreateLabel} 0 0 100% 25% "Here is your access token for the Oi IOT, please write it down in a safe place, you will need it to configure the IOT"
        Pop $ProxyTokenLabel
...
Function ComponentLeave
    SectionGetFlags ${SectionOi_Server} $0
...
Function RestartOiService
    DetailPrint "Restarting Oi Service"
