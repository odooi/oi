...
import { queryAll, queryFirst, queryOne } from "@oi/hoot-dom";
import { Component, useState, useExternalListener } from "@oi/owl";
...
const getShortestSelector = (paths) => {
    paths.reverse();
    let filteredPath = [];
    let hasOiClass = false;
    for (
        let currentElem = paths.pop();
        (currentElem && queryAll(filteredPath.join(" > ")).length !== 1) || !hasOiClass;
        ...
        const oiClass = [...currentElem.classList].find((c) => c.match(OI_CLASS_REGEX));
        if (oiClass) {
            currentPredicate = `.${oiClass}`;
            hasOiClass = true;
        }
