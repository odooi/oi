...
import { afterEach, beforeEach, describe, expect, test } from "@oi/hoot";
import { advanceTime, animationFrame, queryFirst } from "@oi/hoot-dom";
...
test("a tour with invalid step trigger", async () => {
    ...
    await oi.startTour("tour_invalid_trigger", { mode: "auto" }); // Use oi to run tour from registry because this is a test tour
    await waitForMacro();
    const expectedSteps = [
...
test("a failing tour logs the step that failed in run", async () => {
    ...
    await oi.startTour("tour2", { mode: "auto" }); // Use oi to run tour from registry because this is a test tour
    await waitForMacro();
    const expectedError = [
...
test("a failing tour with disabled element", async () => {
    ...
    await oi.startTour("tour3", { mode: "auto" });
    await waitForMacro();
    const expectedError = [
...
test("a failing tour logs the step that failed", async () => {
    ...
    await oi.startTour("tour1", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("check tour with inactive steps", async () => {
    ...
    await oi.startTour("pipu_tour", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("automatic tour with invisible element", async () => {
    ...
    await oi.startTour("tour_de_wallonie", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("automatic tour with alternative trigger", async () => {
    ...
    await mountWithCleanup(Root);
    await oi.startTour("tour_des_flandres", { mode: "auto" });
    await waitForMacro();
...
test("check not possible to click below modal", async () => {
    ...
    await oi.startTour("tour_check_modal", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("a tour where hoot trigger failed", async () => {
    ...
    await oi.startTour("tour_hoot_failed", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
