...
function connect () {
	...
	current_iotbox_version=$(cat "/var/oi/iotbox_version")
	...
	if [ ${TIMEOUT_RETURN} -eq 124 ] && [ -z "${NO_AP}" ] ; then
		...
		sudo /home/pi/oi/addons/point_of_sale/tools/posbox/configuration/wireless_ap.sh "force" &
	else
		...
		if [ ! -f "${LOST_WIFI_FILE}" ] ; then
			logger -t posbox_connect_to_wifi "Restarting oi"
			sudo service oi restart
		fi
		...
		/home/pi/oi/addons/point_of_sale/tools/posbox/configuration/keep_wifi_alive.sh &
	fi
}
