...
if [ -z "${WIRED_IP}" ] ; then
	...
	if [ "${ESSID}" ] && [ "${PASSWORD}" ] && [ -z "${FORCE_HOST_AP}" ] ; then
		...
		/home/pi/oi/addons/point_of_sale/tools/posbox/configuration/connect_to_wifi.sh &
	else
		...
		service oi restart # As this file is executed on boot, this line is responsible for restarting oi service on reboot
	fi
# wired
else
	killall nginx
