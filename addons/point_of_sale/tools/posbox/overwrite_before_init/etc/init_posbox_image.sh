...
echo  "alias oi='sudo systemctl stop oi; /usr/bin/python3 /home/pi/oi/oi-bin --config /home/pi/oi.conf --load=hw_drivers,hw_escpos,hw_posbox_homepage,point_of_sale,web'" | tee -a ~/.bashrc /home/pi/.bashrc
echo  "alias oi_logs='less +F /var/log/oi/oi-server.log'" | tee -a ~/.bashrc /home/pi/.bashrc
echo  "alias write_mode='sudo mount -o remount,rw / && sudo mount -o remount,rw /root_bypass_ramdisks'" | tee -a ~/.bashrc /home/pi/.bashrc
echo  "alias oi_conf='cat /home/pi/oi.conf'" | tee -a ~/.bashrc /home/pi/.bashrc
...
echo  "alias oi_luxe='printf \" ______\n< Luxe >\n ------\n        \\   ^__^\n         \\  (oo)\\_______\n            (__)\\       )\\/\\ \n                ||----w |\n                ||     ||\n\"'" | tee -a ~/.bashrc /home/pi/.bashrc
echo  "alias oi_start='sudo systemctl start oi'" >> /home/pi/.bashrc
echo  "alias oi_stop='sudo systemctl stop oi'" >> /home/pi/.bashrc
echo  "alias oi_restart='sudo systemctl restart oi'" >> /home/pi/.bashrc
echo "
oi_help() {
  echo 'Welcome to Oi IoTBox tools'
  echo 'oi                Starts/Restarts Oi server manually (not through oi.service)'
  echo 'oi_logs           Displays Oi server logs in real time'
  echo 'oi_conf           Displays Oi configuration file content'
  ...
  echo 'oi_start          Starts Oi service'
  echo 'oi_stop           Stops Oi service'
  echo 'oi_restart        Restarts Oi service'
  echo 'oi_dev <branch>   Resets Oi on the specified branch from oi-dev repository'
}
...
oi_dev() {
  ...
  pwd=\$(pwd)
  cd /home/pi/oi
  git remote add dev https://github.com/oi-dev/oi.git
...
# copy the oi.conf file to the overwrite directory
mv -v "/home/pi/oi/addons/point_of_sale/tools/posbox/configuration/oi.conf" "/home/pi/"
chown pi:pi "/home/pi/oi.conf"

apt-get update

# At the first start it is necessary to configure a password
# This will be modified by a unique password on the first start of Oi
...
wget 'https://nightly.odoodooi.com/master/iotbox/eftdvs' -P /usr/local/bin/
chmod +x /usr/local/bin/eftdvs
wget 'https://nightly.odoodooi.com/master/iotbox/eftapi.so' -P /usr/lib/
...
mkdir -v /var/log/oi
chown pi:pi /var/log/oi
chown pi:pi -R /home/pi/oi/
...
echo "* * * * * rm /var/run/oi/sessions/*" | crontab -
...
echo "addons/hw_drivers/iot_devices/" > /home/pi/oi/.git/info/exclude
