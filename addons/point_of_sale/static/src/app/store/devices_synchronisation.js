...
export default class DevicesSynchronisation {
    ...
    async collect(data) {
        ...
        if (oi.debug === "assets") {
            ...
            console.info("Login number", oi.login_number, login_number);
            console.info("Session Ids", oi.pos_session_id, session_id);
        }

        if (login_number == oi.login_number || session_id != oi.pos_session_id) {
            return;
        }
    ...
    constructOrdersDomain(serverOpenOrders) {
        ...
        domain = Domain.and([
            ...
            new Domain([
                "|",
                ["config_id", "=", oi.pos_config_id],
                [
                    "config_id",
                    "in",
                    this.models["pos.config"].get(oi.pos_config_id).raw.trusted_config_ids,
                ],
            ]),
        ]);
