...
import { oiExceptionTitleMap, ErrorDialog } from "@web/core/errors/error_dialogs";
...
export function handleRPCError(error, dialog) {
    const { data } = error;
    if (oiExceptionTitleMap.has(error.exceptionName)) {
        const title = oiExceptionTitleMap.get(error.exceptionName).toString();
    ...
    if (originalError instanceof ConnectionLostError) {
        if (!env.services.pos.data.network.warningTriggered) {
            env.services.dialog.add(AlertDialog, {
                ...
                body: _t(
                    "Until the connection is reestablished, Oi Point of Sale will operate with limited functionality."
                ),
