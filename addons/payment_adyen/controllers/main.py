...
from oi.tools import py_to_js_locale
...
class AdyenController(http.Controller):
    ...
    def adyen_payment_methods(self, provider_id, formatted_amount=None, partner_id=None):
        ...
        shopper_reference = partner_sudo and f'OI_PARTNER_{partner_sudo.id}'
    ...
    def adyen_return_from_3ds_auth(self, **data):
        ...
        The route is flagged with `save_session=False` to prevent Oi from assigning a new session
        to the user if they are redirected to this route with a POST request. Indeed, as the session
        cookie is created without a `SameSite` attribute, some browsers that don't implement the
        recommended default `SameSite=Lax` behavior will not include the cookie in the redirection
        request from the payment provider to Oi. As the redirection to the '/payment/status' page
        ...
        # of Oi in some cases. For instance, when a 3DS1 authentication is required, or for
