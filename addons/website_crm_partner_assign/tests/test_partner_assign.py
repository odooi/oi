...
from oi.tests.common import tagged, new_test_user, TransactionCase
from oi.tools import mute_logger

from oi.addons.base.tests.common import HttpCase
...
from oi.addons.website_crm_partner_assign.controllers.main import (
    WebsiteAccount,
...
class TestPartnerAssign(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('oi.addons.base_geolocalize.models.base_geocoder.GeoCoder.geo_find', wraps=geo_find)
...
class TestPartnerLeadPortal(TestCrmCommon):
    ...
    def test_03_crm_partner_assign_geolocalization(self, GeoIpMock):
        """
            This test checks situation when "{OiURL}/partners" is visited from foreign country without resellers.
        ...
        """
        # Patch GeoIp so it acts, as if Oi client is located in Mexico
