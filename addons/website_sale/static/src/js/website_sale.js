        ...
        if (salePage.dataset.ecomZoomAuto && !uiUtils.isSmall()) {
            ...
            for (const image of images) {
                ...
                this.zoomCleanup.push(() => {
                    ...
                    const zoomOi = $image.data("zoomOi");
                    if (zoomOi) {
                        zoomOi.hide();
                        $image.unbind();
                    }
            ...
            for (const image of images ) {
                const handler = () => {
                    if (salePage.dataset.ecomZoomAuto) {
                        // Remove any flyout
                        const flyouts = document.querySelectorAll(".zoomoi-flyout");
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (wysiwyg) {
            wysiwyg.oiEditor.observerUnactive('_onApplyShopLayoutChange');
        }
        ...
        if (wysiwyg) {
            wysiwyg.oiEditor.observerActive('_onApplyShopLayoutChange');
        }
    },
});
