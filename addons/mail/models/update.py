...
from oi import api, fields, release, SUPERUSER_ID
from oi.exceptions import UserError
from oi.models import AbstractModel
...
class PublisherWarrantyContract(AbstractModel):
    ...
    def _get_message(self):
        ...
        plus_code = IrParamSudo.get_param('database.plus_code')
    ...
    def update_notification(self, cron_mode=True):
        """
        Send a message to Oi's publisher warranty server to check the
        ...
        try:
            ...
            for message in result["messages"]:
                try:
                    ...
                    pass
            if result.get('plus_info'):
                # Update expiration date
                set_param = self.env['ir.config_parameter'].sudo().set_param
                set_param('database.expiration_date', result['plus_info'].get('expiration_date'))
                set_param('database.expiration_reason', result['plus_info'].get('expiration_reason', 'trial'))
                set_param('database.plus_code', result['plus_info'].get('plus_code'))
                set_param('database.already_linked_subscription_url', result['plus_info'].get('database_already_linked_subscription_url'))
                set_param('database.already_linked_email', result['plus_info'].get('database_already_linked_email'))
                set_param('database.already_linked_send_mail_url', result['plus_info'].get('database_already_linked_send_mail_url'))
