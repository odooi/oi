...
import { Deferred } from "@oi/hoot-mock";
...
test("reply: discard on pressing escape", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({
        email: "testpartnert@odoodooi.com",
        name: "TestPartner",
    });
    const messageId = pyEnv["mail.message"].create({
        body: "not empty",
...
test("error notifications should not be shown in Inbox", async () => {
    ...
    await contains(`.o-mail-Message-header a[href*='/oi/res.partner/${partnerId}']`, {
        text: "Demo User",
    });
...
test("Counter should be incremented by 1 when receiving a message with a mention in a channel", async () => {
    ...
    withUser(userId, () =>
        rpc("/mail/message/post", {
            post_data: {
                body: `<a href="https://www.hoot.test/oi/res.partner/17" class="o_mail_redirect" data-oe-id="${mention[0]}" data-oe-model="res.partner" target="_blank" contenteditable="false">@${mentionName}</a> mention`,
                message_type: "comment",
                partner_ids: mention,
...
test("can reply to email message", async () => {
    ...
    const messageId = pyEnv["mail.message"].create({
        author_id: null,
        email_from: "md@odoodooi.company.fr",
    ...
    await contains(".o-mail-Message");
    await click("[title='Reply']");
    await contains(".o-mail-Composer", { text: "Replying to md@odoodooi.company.fr" });
});
