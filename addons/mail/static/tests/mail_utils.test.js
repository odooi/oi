...
test("add_link utility function", () => {
    const testInputs = {
        ...
        "https://www.transifex.com/oi/oi-11/translate/#fr/lunch?q=text%3A'La+Tartiflette'": true,
        "https://www.transifex.com/oi/oi-11/translate/#fr/$/119303430?q=text%3ATartiflette": true,
        "https://tenor.com/view/chỗgiặt-dog-smile-gif-13860250": true,
        "http://www.boîtenoire.be": true,
        "https://github.com/oi/plus/compare/16.0...oi-dev:plus:16.0-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/oi/plus/compare/16.0...16.0-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/oi/plus/compare/16.0...chỗgiặt-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/oi/plus/compare/chỗgiặt...chỗgiặt-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/oi/plus/compare/@...}-voip-fix_demo_data-tsm?expand=1": true,
        "https://x.com": true,
    };
...
test("addLink: utility function and special entities", () => {
    const testInputs = [
        ...
        [
            markup("https://odoodooi.com/%5B%5D"),
            `<a target="_blank" rel="noreferrer noopener" href="https://odoodooi.com/%5B%5D">https://odoodooi.com/[]</a>`,
        ],
    ];
...
test("url", async () => {
    ...
    const messageBody = "https://odoodooi.com?test=~^|`{}[]#";
...
test("url with comma at the end", async () => {
    ...
    await start();
    await openDiscuss(channelId);
    const messageBody = "Go to https://odoodooi.com, it's great!";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button[aria-label='Send']:enabled");
    await contains(".o-mail-Message a:contains(https://odoodooi.com)");
...
test("url with number in subdomain", async () => {
    ...
    await start();
    await openDiscuss(channelId);
    const messageBody = "https://www.45017478-master-all.runbot134.odoodooi.com/oi";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button[aria-label='Send']:enabled");
    await contains(
        ".o-mail-Message a:contains(https://www.45017478-master-all.runbot134.odoodooi.com/oi)"
    );
});
