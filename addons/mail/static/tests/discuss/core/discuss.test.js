...
import { describe, test } from "@oi/hoot";
import { mockDate } from "@oi/hoot-mock";
...
test("subscribe to known partner presences", async () => {
    ...
    const expectedPresences = [
        `oi-presence-res.partner_${serverState.partnerId}`,
        `oi-presence-res.partner_${serverState.oibotId}`,
    ];
    ...
    expectedPresences.push(`oi-presence-res.partner_${bobPartnerId}`);
    await assertSteps([`subscribe - [${expectedPresences.join(",")}]`]);
});
