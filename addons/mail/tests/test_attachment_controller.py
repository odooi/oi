...
import oi
from oi.tools import file_open, mute_logger
from oi.addons.mail.tests.test_controller_common import TestControllerCommon
...
class TestAttachmentControllerCommon(TestControllerCommon):
    ...
    def _upload_attachment(self, thread_id, thread_model, route_kw):
        with mute_logger("oi.http"), file_open("addons/web/__init__.py") as file:
            res = self.url_open(
...
class TestAttachmentController(TestAttachmentControllerCommon):
    ...
    def test_send_attachment_without_body(self):
        self.start_tour("/oi/discuss", "create_thread_for_attachment_without_body",login="admin")
