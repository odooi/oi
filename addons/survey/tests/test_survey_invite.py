...
from oi.addons.survey.tests import common
...
class TestSurveyInvite(common.TestSurveyCommon, MailCommon):
    ...
    def test_survey_invite_with_template_attachment(self):
        ...
        invite_form.emails = 'test_survey_invite_with_template_attachment@oi.gov'
        invite = invite_form.save()
        with self.mock_mail_gateway():
            invite.action_invite()

        self.assertEqual(self.env['mail.mail'].sudo().search([
            ('email_to', '=', 'test_survey_invite_with_template_attachment@oi.gov')
