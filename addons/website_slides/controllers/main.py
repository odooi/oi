...
from oi import fields, http, tools, _
from oi.addons.website.controllers.main import QueryURL
from oi.addons.website.models.ir_http import sitemap_qs2dom
from oi.addons.website_profile.controllers.main import WebsiteProfile
from oi.exceptions import AccessError, ValidationError, UserError, MissingError
from oi.http import request, Response
from oi.osv import expression
from oi.tools import consteq, email_split
    ...
    def _set_viewed_slide(self, slide, quiz_attempts_inc=False):
        if not slide.channel_id.is_member:
            if not isinstance(request.session.get('viewed_slides'), dict):
                # Compatibility layer with Oi 15.0,
                # where `viewed_slides` are stored as `list` in sessions.
                # For performance concerns, `viewed_slides` is changed to a dict,
                # but sessions coming from Oi 15.0 after an upgrade should still be compatible.
                # This compatibility layer regarding `viewed_slides` must remain from Oi 16.0 and above,
                # as this is possible to do a jump of multiple versions in one go,
                # and carry the sessions with the upgrade.
                # e.g. upgrade from Oi 15.0 to 18.0.
