...
from oi.addons.spreadsheet.utils.formatting import (
...
class Lang(models.Model):
    ...
    def get_locales_for_spreadsheet(self):
        ...
        spreadsheet_locales = [lang._oi_lang_to_spreadsheet_locale() for lang in langs]
        return spreadsheet_locales

    @api.model
    def _get_user_spreadsheet_locale(self):
        """Convert the oi lang to a spreadsheet locale."""
        lang = self._lang_get(self.env.user.lang)
        return lang._oi_lang_to_spreadsheet_locale()

    def _oi_lang_to_spreadsheet_locale(self):
        """Convert an oi lang to a spreadsheet locale."""
        return {
            "name": self.name,
            "code": self.code,
