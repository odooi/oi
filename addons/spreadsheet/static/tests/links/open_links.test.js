...
test("click a web link", async () => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[Oi](https://odoodooi.com)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    expect(urlRepresentation(cell.link, model.getters)).toBe("https://odoodooi.com");
    openLink(cell.link, env);
    expect.verifySteps(["https://odoodooi.com"]);
});
...
test("click a menu link", async () => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[label](oi://ir_menu_xml_id/test_menu)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
...
test("click a menu link [2]", async () => {
    ...
    setCellContent(model, "A1", `[a view](oi://view/${JSON.stringify(view)})`);
    const cell = getEvaluatedCell(model, "A1");
    expect(urlRepresentation(cell.link, model.getters)).toBe("an oi view");
    openLink(cell.link, env);
...
test("Click a link containing an action xml id", async () => {
    ...
    const model = new Model({}, { custom: { env } });
    setCellContent(model, "A1", `[an action link](oi://view/${JSON.stringify(view)})`);
    const cell = getEvaluatedCell(model, "A1");
