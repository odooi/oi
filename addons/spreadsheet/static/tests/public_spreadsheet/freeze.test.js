...
import { registries } from "@oi/o-spreadsheet";
...
import { freezeOiData } from "@spreadsheet/helpers/model";
import { OiPivot, OiPivotRuntimeDefinition } from "@spreadsheet/pivot/oi_pivot";
...
test("oi pivot functions are replaced with their value", async function () {
    ...
    const data = await freezeOiData(model);
...
test("Pivot with a type different of OI is not converted", async function () {
    // Add a pivot with a type different of OI
...
test("oi pivot functions detection is not case sensitive", async function () {
    const { model } = await createSpreadsheetWithPivot();
    setCellContent(model, "A1", '=pivot.value(1,"probability:avg")');
    const data = await freezeOiData(model);
...
test("oi charts are replaced with an image", async function () {
...
test("oi links are replaced with their label", async function () {
    const view = {
        name: "an oi view",
        viewType: "list",
        action: {
    ...
    const data = {
        sheets: [
            {
                cells: {
                    A1: { content: "[menu_xml](oi://ir_menu_xml_id/test_menu)" },
                    A2: { content: "[menu_id](oi://ir_menu_id/12)" },
                    A3: { content: `[oi_view](oi://view/${JSON.stringify(view)})` },
                    A4: { content: "[external_link](https://odoodooi.com)" },
    ...
    const frozenData = await freezeOiData(model);
    expect(frozenData.sheets[0].cells.A1.content).toBe("menu_xml");
    expect(frozenData.sheets[0].cells.A2.content).toBe("menu_id");
    expect(frozenData.sheets[0].cells.A3.content).toBe("oi_view");
    expect(frozenData.sheets[0].cells.A4.content).toBe("[external_link](https://odoodooi.com)");
