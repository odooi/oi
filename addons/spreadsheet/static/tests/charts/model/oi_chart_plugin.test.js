...
import { OiBarChart } from "@spreadsheet/chart/oi_chart/oi_bar_chart";
import { OiChart } from "@spreadsheet/chart/oi_chart/oi_chart";
import { OiLineChart } from "@spreadsheet/chart/oi_chart/oi_line_chart";
...
test("Can add an Oi Bar chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_bar" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof OiBarChart).toBe(true);
...
test("Can add an Oi Line chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_line" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof OiLineChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
    expect(model.getters.getChartRuntime(chartId).chartJsConfig.type).toBe("line");
});

test("Can add an Oi Pie chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_pie" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof OiChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
...
test("Oi bar chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "oi_bar",
...
test("Oi pie chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "oi_pie",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Oi line chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "oi_line",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Can import/export an Oi chart", async () => {
    const model = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "oi_line");
    const data = model.exportData();
    const figures = data.sheets[0].figures;
    expect(figures.length).toBe(1);
    const figure = figures[0];
    expect(figure.tag).toBe("chart");
    expect(figure.data.type).toBe("oi_line");
...
test("Can undo/redo an Oi chart creation", async () => {
    const model = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "oi_line");
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("charts with no legend", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_pie" });
    insertChartInSpreadsheet(model, "oi_bar");
    insertChartInSpreadsheet(model, "oi_line");
    const sheetId = model.getters.getActiveSheetId();
...
test("Can copy/paste Oi chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[1]) instanceof OiChart).toBe(true);
...
test("Can cut/paste Oi chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[0]) instanceof OiChart).toBe(true);
    expect(JSON.stringify(model.getters.getChartRuntime(chartIds[0]))).toBe(
        JSON.stringify(chartRuntime)
    );
});

test("Duplicating a sheet correctly duplicates Oi chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Load oi chart spreadsheet with models that cannot be accessed", async function () {
...
test("Can insert oi chart from a different model", async () => {
    const model = await createModelWithDataSource();
...
test("Oi chart legend color changes with background color update", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
...
test("Remove oi chart when sheet is deleted", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_line" });
    const sheetId = model.getters.getActiveSheetId();
    ...
    expect(model.getters.getOiChartIds().length).toBe(1);
    model.dispatch("DELETE_SHEET", { sheetId });
    expect(model.getters.getOiChartIds().length).toBe(0);
});

test("Oi chart datasource display name has a default when the chart title is empty", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
    expect(model.getters.getOiChartDisplayName(chartId)).toBe("(#1) Partners");
    model.dispatch("UPDATE_CHART", {
        definition: {
            ...definition,
            title: { text: "" },
    ...
    expect(model.getters.getOiChartDisplayName(chartId)).toBe("(#1) Oi Line Chart");
});
...
test("Displays correct thousand separator for positive value in Oi Bar chart Y-axis", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const runtime = model.getters.getChartRuntime(chartId);
...
test("Thousand separator in Oi Bar chart Y-axis is locale-dependent", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "oi_bar" });
