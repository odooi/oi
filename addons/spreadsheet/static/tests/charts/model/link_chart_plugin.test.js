...
test("Links between charts and ir.menus are correctly imported/exported", async function () {
    ...
    expect(exportedData.chartOiMenusReferences[chartId]).toBe(1, {
        message: "Link to oi menu is exported",
    });
    const importedModel = new Model(exportedData, { custom: { env } });
    const chartMenu = importedModel.getters.getChartOiMenu(chartId);
    expect(chartMenu.id).toBe(1, { message: "Link to oi menu is imported" });
});

test("Can undo-redo a LINK_OI_MENU_TO_CHART", async function () {
    const env = await makeMockEnv();
    const model = new Model({}, { custom: { env } });
    createBasicChart(model, chartId);
    model.dispatch("LINK_OI_MENU_TO_CHART", {
    ...
    expect(model.getters.getChartOiMenu(chartId).id).toBe(1);
    model.dispatch("REQUEST_UNDO");
    expect(model.getters.getChartOiMenu(chartId)).toBe(undefined);
...
test("Links of Oi charts are duplicated when duplicating a sheet", async function () {
    ...
    expect(model.getters.getChartOiMenu(newChartId)).toEqual(
        model.getters.getChartOiMenu(chartId)
    );
});
