...
import { OiPivot } from "@spreadsheet/pivot/oi_pivot";
...
export async function insertPivotInSpreadsheet(model, pivotId, params) {
    ...
    if (!(ds instanceof OiPivot)) {
        throw new Error("The pivot data source is not an OiPivot");
    }
    await ds.load();
 ...
 * @returns {Promise<{ model: OiSpreadsheetModel, env: object, pivotId: string}>}
 */
