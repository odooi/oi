 ...
 * @typedef {import("@spreadsheet/helpers/model").OiSpreadsheetModel} OiSpreadsheetModel
...
export function setupDataSourceEvaluation(model) {
    model.config.custom.oiDataProvider.addEventListener("data-source-updated", () => {
 ...
 * @returns {Promise<OiSpreadsheetModel>}
 */
export async function createModelWithDataSource(params = {}) {
    ...
    const model = new Model(params.spreadsheetData, {
        ...
        custom: {
            env,
            oiDataProvider: new OiDataProvider(env),
            ...config?.custom,
        },
    });
