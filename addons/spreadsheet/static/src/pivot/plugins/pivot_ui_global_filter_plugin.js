...
export class PivotUIGlobalFilterPlugin extends OiUIPlugin {
     ...
     */
    _addDomain(pivotId) {
        if (this.getters.getPivotCoreDefinition(pivotId).type !== "OI") {
            return;
        }
        const domainList = [];
    ...
    _addDomains() {
        for (const pivotId of this.getters
            .getPivotIds()
            .filter((pivotId) => this.getters.getPivot(pivotId).type === "OI")) {
