...
import { EvaluationError, helpers } from "@oi/o-spreadsheet";
 ...
 * @typedef {import("@oi/o-spreadsheet").Granularity} Granularity
 * */
...
export const OI_AGGREGATORS = {};

for (const type in AGGREGATORS_BY_FIELD_TYPE) {
    OI_AGGREGATORS[type] = {};
    for (const aggregator of AGGREGATORS_BY_FIELD_TYPE[type]) {
        OI_AGGREGATORS[type][aggregator] = AGGREGATOR_NAMES[aggregator];
    }
}
...
/**
 * @typedef {import("@spreadsheet").OiField} OiField
 */

/**
 * Parses the positional char (#), the field and operator string of pivot group.
 * e.g. "create_date:month"
 * @param {Record<string, OiField | undefined>} allFields
 * @param {string} groupFieldString
 * @returns {{field: OiField, granularity: Granularity, isPositional: boolean, dimensionWithGranularity: string}}
 */
