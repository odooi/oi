...
import { EvaluationError, CellErrorType } from "@oi/o-spreadsheet";
...
export class OiPivotLoader {
    /**
     * @param {OiDataProvider} oiDataProvider
     * @param {Function} load Function to fetch data
     */
    constructor(oiDataProvider, load) {
        /** @private @type {OiDataProvider} */
        this.oiDataProvider = oiDataProvider;
        /** @protected */
        this.loadFn = load;
    ...
    async load(options) {
        if (options && options.reload) {
            this.oiDataProvider.cancelPromise(this.loadPromise);
            this.loadPromise = undefined;
        }
        if (!this.loadPromise) {
            this.isFullyLoaded = false;
            ...
            await this.oiDataProvider.notifyWhenPromiseResolves(this.loadPromise);
        }
     ...
     * @returns {Promise<OiFields>} Fields of the model
     */
    async getFields(model) {
        return getFields(this.oiDataProvider.serverData, model);
    }
    /**
     * @param {string} model Technical name of the model
    ...
    async getModelLabel(model) {
        const result = await this.oiDataProvider.serverData.fetch(
            "ir.model",
            "display_name_for",
            [[model]]
        );
