...
import { NO_RECORD_AT_THIS_POSITION, OiPivotModel } from "./pivot_model";
import { EvaluationError, PivotRuntimeDefinition, registries, helpers } from "@oi/o-spreadsheet";
import { LOADING_ERROR } from "@spreadsheet/data_sources/data_source";
import { omit } from "@web/core/utils/objects";
import { OiPivotLoader } from "./oi_pivot_loader";
...
/**
 * @typedef {import("@oi/o-spreadsheet").FunctionResultObject} FunctionResultObject
 * @typedef {import("@oi/o-spreadsheet").PivotMeasure} PivotMeasure
 * @typedef {import("@oi/o-spreadsheet").PivotDomain} PivotDomain
 * @typedef {import("@oi/o-spreadsheet").PivotDimension} PivotDimension
 * @typedef {import("@oi/o-spreadsheet").PivotCoreMeasure} PivotCoreMeasure
 * @typedef {import("@spreadsheet").WebPivotModelParams} WebPivotModelParams
 * @typedef {import("@spreadsheet").OiPivot<OiPivotRuntimeDefinition>} IPivot
 * @typedef {import("@spreadsheet").OiFields} OiFields
 * @typedef {import("@spreadsheet").OiPivotCoreDefinition} OiPivotCoreDefinition
 * @typedef {import("@spreadsheet").SortedColumn} SortedColumn
 * @typedef {import("@spreadsheet").OiGetters} OiGetters
 * @typedef {import("@spreadsheet/data_sources/oi_data_provider").OiDataProvider} OiDataProvider
 */

/**
 * @implements {IPivot}
 */
export class OiPivot {
    /**
     *
     * @override
     * @param {Object} services Services (see DataSource)
     * @param {Object} params
     * @param {OiPivotCoreDefinition} params.definition
     * @param {OiGetters} params.getters
     */
    constructor(services, { definition, getters }) {
        /** @type {"OI"} */
        this.type = "OI";

        /** @type {OiPivotCoreDefinition} @protected */
        this.coreDefinition = definition;

        this.needsReevaluation = false;

        /** @type {OiPivotRuntimeDefinition | undefined} @protected */
        this.runtimeDefinition = undefined;

        /** @type {OiPivotModel | undefined} @protected */
        this.model = undefined;

        /** @type {OiGetters} @protected */
        this.getters = getters;

        /** @protected */
        this.loader = new OiPivotLoader(services.oiDataProvider, this._load.bind(this));

        /** @type {OiFields | undefined} @protected */
        this._fields = undefined;

        /** @protected @type {OiDataProvider}*/
        this.oiDataProvider = services.oiDataProvider;
    ...
    /**
     * @param {OiPivotCoreDefinition} nextDefinition
     */
    onDefinitionChange(nextDefinition) {
        ...
        if (
            ...
            if (
                ...
                this.coreDefinition = nextDefinition;
                const runtimeDefinition = new OiPivotRuntimeDefinition(
                    this.coreDefinition,
                    this.getFields()
                );
    ...
    async createModelAndDefinition() {
        ...
        const definition = new OiPivotRuntimeDefinition(this.coreDefinition, this.getFields());
        const model = new OiPivotModel(
            { _t },
            {
            ...
            {
                orm: this.oiDataProvider.orm,
                serverData: this.oiDataProvider.serverData,
    ...
    /**
     * @param {import("@oi/o-spreadsheet").Maybe<FunctionResultObject>[]} args
     *
     * @returns {PivotDomain}
     */
...
export class OiPivotRuntimeDefinition extends PivotRuntimeDefinition {
    /**
     * @param {OiPivotCoreDefinition} definition
     * @param {OiFields} fields
     */
...
pivotRegistry.add("OI", {
    ui: OiPivot,
    definition: OiPivotRuntimeDefinition,
    externalData: true,
...
supportedPivotPositionalFormulaRegistry.add("OI", true);
