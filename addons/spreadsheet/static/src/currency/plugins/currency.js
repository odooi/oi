...
import { EvaluationError, helpers, registries } from "@oi/o-spreadsheet";
...
export class CurrencyPlugin extends OiUIPlugin {
    ...
    constructor(config) {
        ...
        this._serverData = config.custom.oiDataProvider?.serverData;
    }

    get serverData() {
        if (!this._serverData) {
            throw new Error(
                "'serverData' is not defined, please make sure a 'OiDataProvider' instance is provided to the model."
            );
        }
...
featurePluginRegistry.add("oiCurrency", CurrencyPlugin);
