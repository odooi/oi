...
patch(spreadsheet.components.FigureComponent.prototype, {
    ...
    async navigateToOiMenu() {
        const menu = this.env.model.getters.getChartOiMenu(this.props.figure.id);
        if (!menu) {
    ...
    get hasOiMenu() {
        return this.env.model.getters.getChartOiMenu(this.props.figure.id) !== undefined;
    },
    async onClick() {
        if (this.env.isDashboard() && this.hasOiMenu) {
            this.navigateToOiMenu();
        }
    },
});
