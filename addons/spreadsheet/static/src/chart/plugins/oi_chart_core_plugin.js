...
const CHART_PLACEHOLDER_DISPLAY_NAME = {
    oi_bar: _t("Oi Bar Chart"),
    oi_line: _t("Oi Line Chart"),
    oi_pie: _t("Oi Pie Chart"),
};

export class OiChartCorePlugin extends OiCorePlugin {
    static getters = /** @type {const} */ ([
        "getOiChartIds",
        "getChartFieldMatch",
        "getOiChartDisplayName",
        "getOiChartFieldMatching",
    ]);
    ...
    constructor(config) {
        ...
        globalFiltersFieldMatchers["chart"] = {
            getIds: () => this.getters.getOiChartIds(),
            getDisplayName: (chartId) => this.getters.getOiChartDisplayName(chartId),
            getFieldMatching: (chartId, filterId) =>
                this.getOiChartFieldMatching(chartId, filterId),
            getModel: (chartId) =>
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "EDIT_GLOBAL_FILTER":
                if (cmd.chart) {
                    this._setOiChartFieldMatching(cmd.filter.id, cmd.chart);
                }
                break;
        }
    }
            ...
            if (sheet.figures) {
                for (const figure of sheet.figures) {
                    if (figure.tag === "chart" && figure.data.type.startsWith("oi_")) {
                        this._addOiChart(figure.id, figure.data.fieldMatching);
                    }
                }
            }
        }
    ...
    /**
     * Get the current oiChartFieldMatching of a chart
     *
     * @param {string} chartId
     * @param {string} filterId
     */
    getOiChartFieldMatching(chartId, filterId) {
        return this.charts[chartId].fieldMatching[filterId];
    }

    /**
     * Sets the current oiChartFieldMatching of a chart
     *
     * @param {string} filterId
     * @param {Record<string,FieldMatching>} chartFieldMatches
     */
    _setOiChartFieldMatching(filterId, chartFieldMatches) {
        const charts = { ...this.charts };
     ...
     */
    _addOiChart(chartId, fieldMatching = undefined) {
        const charts = { ...this.charts };
        if (!fieldMatching) {
