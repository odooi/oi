...
export class ListUIPlugin extends OiUIPlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "INSERT_OI_LIST_WITH_TABLE": {
                this.dispatch("INSERT_OI_LIST", cmd);
                this._addTable(cmd);
                break;
            }
            case "RE_INSERT_OI_LIST_WITH_TABLE": {
                this.dispatch("RE_INSERT_OI_LIST", cmd);
                this._addTable(cmd);
                break;
            }
            case "DUPLICATE_OI_LIST": {
                this._setupListDataSource(cmd.newListId, 0);
                break;
            }
            case "REFRESH_ALL_DATA_SOURCES":
                this._refreshOiLists();
                break;
            ...
            case "REDO": {
                ...
                const updateCommands = cmd.commands.filter(
                    (cmd) =>
                        cmd.type === "UPDATE_OI_LIST_DOMAIN" ||
                        cmd.type === "UPDATE_OI_LIST" ||
                        cmd.type === "INSERT_OI_LIST"
                );
