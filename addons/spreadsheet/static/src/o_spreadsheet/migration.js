...
export const OI_VERSION = 12;

const MAP_V1 = {
    PIVOT: "OI.PIVOT",
    "PIVOT.HEADER": "OI.PIVOT.HEADER",
    "PIVOT.POSITION": "OI.PIVOT.POSITION",
    "FILTER.VALUE": "OI.FILTER.VALUE",
    LIST: "OI.LIST",
    "LIST.HEADER": "OI.LIST.HEADER",
};

const MAP_FN_NAMES_V10 = {
    "OI.PIVOT": "PIVOT.VALUE",
    "OI.PIVOT.HEADER": "PIVOT.HEADER",
    "OI.PIVOT.TABLE": "PIVOT",
};

const dmyRegex = /^([0|1|2|3][1-9])\/(0[1-9]|1[0-2])\/(\d{4})$/i;

migrationStepRegistry.add("oi_migration", {
    versionFrom: "16.1",
    migrate(data) {
        return migrateOiData(data);
    },
});

function migrateOiData(data) {
    const version = data.oiVersion || 0;
    if (version < 1) {
...
function migratePivotDaysParameters(formulaString) {
    ...
    const convertedAst = convertAstNodes(ast, "FUNCALL", (ast) => {
        if (["OI.PIVOT", "OI.PIVOT.HEADER"].includes(ast.value.toUpperCase())) {
...
/**
 * Migrate the pivot data to add the type, by default "OI". And replace the
...
function migrate11to12(data) {
    // remove the calls to OI.PIVOT.POSITION and replace
    ...
    for (const sheet of data.sheets || []) {
        for (const xc in sheet.cells || []) {
            ...
            if (
                ...
                cell.content.includes("OI.PIVOT.POSITION")
            ) {
                const tokens = tokenize(cell.content);
                /* given that oi.pivot.position is automatically set, we know that:
                1) it is always on the form of OI.PIVOT.POSITION(1, ...)
                2) it is always preceded by a dimension of a pivot or header, inside another pivot formula
                3) there is only one oi.pivot.position per cell
                4) oi.pivot.position can only exist after the 3rd token and needs at least 7 tokens to be valid*/
                for (let i = 2; i < tokens.length - 7; i++) {
                    const token = tokens[i];
                    if (
                        token.type === "SYMBOL" &&
                        token.value.toUpperCase() === "OI.PIVOT.POSITION"
                    ) {
                        const order = tokens[i + 6];
                        tokens[i - 2].value = '"#' + tokens[i - 2].value.slice(1); // "dimension" becomes "#dimension"
                        tokens.splice(i, 7); // remove "OI.PIVOT.POSITION", "(", "1", ",", "dimension", ", ", order
                        // tokens[i-1] is the comma before oi.pivot.position
                        tokens[i] = order;
                        cell.content = tokensToString(tokens);
                    }
                }
...
export class OiVersion extends OiCorePlugin {
    static getters = /** @type {const} */ ([]);

    export(data) {
        data.oiVersion = OI_VERSION;
    }
}

corePluginRegistry.add("oiMigration", OiVersion);
