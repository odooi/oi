...
import { parse, helpers, iterateAstNodes } from "@oi/o-spreadsheet";
import { isLoadingError } from "@spreadsheet/o_spreadsheet/errors";
import { loadBundle } from "@web/core/assets";
import { OiSpreadsheetModel } from "@spreadsheet/model";
import { OiDataProvider } from "@spreadsheet/data_sources/oi_data_provider";
...
import {
    ...
    isIrMenuXmlUrl,
} from "@spreadsheet/ir_ui_menu/oi_menu_link_cell";
...
export function createSpreadsheetModel({ env, data, revisions }) {
    const oiDataProvider = new OiDataProvider(env);
    const model = new OiSpreadsheetModel(data, { custom: { oiDataProvider } }, revisions);
    return model;
}

/**
 * @param {OiSpreadsheetModel} model
 */
export async function waitForOiSources(model) {
    ...
    promises.push(
        ...model.getters
            .getPivotIds()
            .filter((pivotId) => model.getters.getPivotCoreDefinition(pivotId).type === "OI")
...
export async function waitForDataLoaded(model) {
    await waitForOiSources(model);
    const oiDataProvider = model.config.custom.oiDataProvider;
    if (!oiDataProvider) {
        return;
    }
    await new Promise((resolve, reject) => {
        function check() {
            ...
            if (isLoaded(model)) {
                oiDataProvider.removeEventListener("data-source-updated", check);
                resolve();
            }
        }
        oiDataProvider.addEventListener("data-source-updated", check);
        check();
    });
}

function containsLinkToOi(link) {
    if (link && link.url) {
        return (
 ...
 */
export async function freezeOiData(model) {
    ...
    for (const sheet of Object.values(data.sheets)) {
        ...
        for (const [xc, cell] of Object.entries(sheet.cells)) {
            ...
            if (containsOiFunction(cell.content)) {
                const pivotId = model.getters.getPivotIdFromPosition(position);
                if (pivotId && model.getters.getPivotCoreDefinition(pivotId).type !== "OI") {
                    continue;
                }
            ...
            if (containsLinkToOi(evaluatedCell.link)) {
                cell.content = evaluatedCell.link.label;
            }
        }
        for (const figure of sheet.figures) {
            if (figure.tag === "chart" && figure.data.type.startsWith("oi_")) {
                await loadBundle("web.chartjs_lib");
                const img = oiChartToImage(model, figure);
                figure.tag = "image";
                figure.data = {
                    path: img,
    ...
    if (data.pivots) {
        data.pivots = Object.fromEntries(
            Object.entries(data.pivots).filter(([id, def]) => def.type !== "OI")
        );
    }
    data.lists = {};
 ...
 * @returns {boolean}
 */
function containsOiFunction(content) {
    if (
        !content ||
        !content.startsWith("=") ||
        (!content.toUpperCase().includes("OI.") &&
    ...
    try {
        ...
        return iterateAstNodes(ast).some(
            (ast) =>
                ast.type === "FUNCALL" &&
                (ast.value.toUpperCase().startsWith("OI.") ||
 ...
 * @returns {string}
 */
function oiChartToImage(model, figure) {
