...
xml_id_url_prefix = "oi://ir_menu_xml_id/"

oi_view_link_prefix = "oi://view/"


def oi_charts(data):
    """return all oi chart definition in the spreadsheet"""
    figures = []
    for sheet in data.get("sheets", []):
        figures += [
            dict(figure["data"], id=figure["id"])
            for figure in sheet.get("figures", [])
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("oi_")
        ]
    return figures
...
def oi_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like oi://view/{... view data...}
    """
    return [
        json.loads(url[len(oi_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(oi_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = oi_charts(data)
    oi_version = data.get("oiVersion", 1)
    if oi_version < 5:
...
def fields_in_spreadsheet(data):
    ...
    oi_pivots = (pivot for pivot in data.get("pivots", dict()).values() if pivot.get("type", "OI") == "OI")
    all_fields = chain(
        extract_fields(list_fields, data.get("lists", dict()).values()).items(),
        extract_fields(pivot_fields, oi_pivots).items(),
        extract_fields(chart_fields, oi_charts(data)).items(),
        extract_fields(oi_view_fields, oi_view_links(data)).items(),
...
def menus_xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartOiMenusReferences", {}).values()) | {
