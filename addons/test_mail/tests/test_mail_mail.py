...
from oi import api, Command, fields, SUPERUSER_ID
from oi.addons.base.models.ir_mail_server import MailDeliveryException
from oi.addons.mail.tests.common import MailCommon
from oi.exceptions import AccessError
from oi.tests import common, tagged, users
...
class TestMailMail(MailCommon):
    ...
    def test_mail_mail_send_exceptions_origin(self):
        ...
        for default_from in [False, '']:
            ...
            self._reset_data()
            with self.mock_mail_gateway(), mute_logger('oi.addons.mail.models.mail_mail'):
...
class TestMailMailServer(MailCommon):
    ...
    @mute_logger('oi.addons.mail.models.mail_mail')
    @patch('oi.addons.base.models.ir_attachment.IrAttachment.file_size', new_callable=PropertyMock)
...
class TestMailMailRace(common.TransactionCase):
    ...
    def test_mail_bounce_during_send(self):
        ...
        def send_email(self, message, *args, **kwargs):
            with this.registry.cursor() as cr, mute_logger('oi.sql_db'):
                try:
