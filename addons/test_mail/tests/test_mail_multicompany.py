...
from oi.addons.test_mail.models.test_mail_corner_case_models import MailTestMultiCompanyWithActivity
from oi.addons.test_mail.tests.common import TestRecipients
from oi.exceptions import AccessError
from oi.tests import tagged, users, HttpCase
...
class TestMailMCCommon(MailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.fake_email = cls.env['mail.message'].create({
            ...
            'message_id': '<123456-oi-%s-mail.test.gateway@%s>' % (cls.test_record.id, socket.gethostname()),
        })
...
class TestMultiCompanySetup(TestMailMCCommon, HttpCase):
    ...
    @users('employee_c2')
    @mute_logger('oi.addons.base.models.ir_rule')
            ...
            with self.subTest(login=login, mc_record=mc_record):
                ...
                if not login:
                    ...
                    if mc_error:
                        ...
                        self.assertEqual(parsed_url.path, '/oi/action-mail.action_discuss')
                    else:
