...
from oi import exceptions
from oi.addons.mail.models.mail_message import Message
from oi.addons.mail.models.mail_thread import MailThread
from oi.addons.mail.tests.common import mail_new_test_user, MailCommon
from oi.addons.test_mail.data import test_mail_data
from oi.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE, THAI_EMAIL_WINDOWS_874
from oi.addons.test_mail.models.test_mail_models import MailTestGateway, MailTestGatewayGroups, MailTestTicket
from oi.sql_db import Cursor
from oi.tests import Form, tagged, RecordCapturer
from oi.tools import mute_logger
from oi.tools.mail import email_split_and_format, formataddr
...
class TestEmailParsing(MailCommon):
    ...
    def test_message_parse_and_replace_binary_octetstream(self):
        ...
        with self.assertLogs('oi.addons.mail.models.mail_thread', level="WARNING") as capture:
        ...
        self.assertEqual(capture.output, [
            ("WARNING:oi.addons.mail.models.mail_thread:Message containing an unexpected "
    ...
    def test_message_parse_eml(self):
        ...
        mail = self.format(test_mail_data.MAIL_EML_ATTACHMENT, email_from='"Sylvie Lelitre" <test.sylvie.lelitre@agrolait.com>', to=f'generic@{self.alias_domain}',
                           msg_id='<cb7eaf62-58dc-2017-148c-305d0c78892f@odoodooi.com>',
                           references='<f3b9f8f8-28fa-2543-cab2-7aa68f679ebb@odoodooi.com>',
    ...
    def test_message_parse_eml_bounce_headers(self):
        ...
        msg_id = '<861878175823148.1577183525.736005783081055-oi-19177-account.invoice@mycompany.example.com>'
        mail = self.format(
    ...
    def test_message_parse_extract_bounce_rfc822_headers_qp(self):
        ...
        message = self.env['mail.message'].create({
            'message_id' : '<368396033905967.1673346177.695352554321289-oi-11-sale.order@eupp00>'
        })
        incoming_bounce = self.format(
            test_mail_data.MAIL_BOUNCE_QP_RFC822_HEADERS,
            email_from='MAILER-DAEMON@mailserver.odoodooi.com (Mail Delivery System)',
            email_to='bounce@xxx.odoodooi.com',
            delivered_to='bounce@xxx.odoodooi.com'
        )
...
class MailGatewayCommon(MailCommon):
    ...
    def _create_gateway_message(cls, record, msg_id_prefix, **values):
        msg_values = {
            ...
            'message_id': f'<{msg_id_prefix}-oi-{record.id}-{record._name}@{socket.gethostname()}>',
            'message_type': 'email',
            'model': record._name,
            'res_id': record.id,
...
class TestMailgateway(MailGatewayCommon):
    ...
    @mute_logger('oi.addons.mail.models.mail_thread', 'oi.models')
    ...
    @mute_logger('oi.addons.mail.models.mail_thread')
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not oibot:
        ...
        oibot = self.env.ref('base.partner_root')
        oibot.active = True
        oibot.email = 'oibot@example.com'
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, oibot.email_formatted, f'groups@{self.alias_domain}',
                subject='Oibot Automatic Answer')

        self.assertEqual(record4.message_ids[0].author_id, oibot)
        self.assertEqual(record4.message_ids[0].email_from, oibot.email_formatted)
        self.assertEqual(record4.message_follower_ids.partner_id, self.env['res.partner'],
                         'message_process: oibot -> no follower')
    ...
    @mute_logger('oi.addons.mail.models.mail_mail', 'oi.addons.mail.models.mail_thread', 'oi.models.unlink', 'oi.tests')
    ...
    @mute_logger('oi.addons.mail.models.mail_mail', 'oi.addons.mail.models.mail_thread', 'oi.models')
    def test_message_route_alias_owner_author_notify(self):
        ...
        self.assertFalse(self.user_root.active, 'notification logic relies on oibot being archived')
        ...
        for email_to, test_user in itertools.product(email_tos, test_users):
            with self.subTest(test_user=test_user, email_to=email_to):
                ...
                self.assertEqual(new_messages.create_uid, self.user_root,
                                 'Oibot should be creating the message')
    ...
    @mute_logger('oi.addons.mail.models.mail_thread', 'oi.models.unlink', 'oi.addons.mail.models.mail_mail')
    ...
    @mute_logger('oi.addons.mail.models.mail_thread', 'oi.addons.mail.models.mail_mail', 'oi.models.unlink')
    ...
    @mute_logger('oi.addons.mail.models.mail_thread', 'oi.addons.mail.models.mail_mail', 'oi.models', 'oi.sql_db')
    def test_message_process_alias_config_invalid_defaults(self):
        ...
        with self.mock_mail_gateway(), patch('oi.addons.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
        ...
        with self.assertRaises(Exception), patch('oi.addons.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
                                                 autospec=True) as _alias_bounce_incoming_email_mock:
    ...
    @mute_logger('oi.addons.mail.models.mail_thread', 'oi.models', 'oi.tests')
    ...
    @mute_logger('oi.addons.mail.models.mail_thread', 'oi.models.unlink')
    ...
    @mute_logger('oi.addons.mail.models.mail_thread', 'oi.models.unlink', 'oi.addons.mail.models.mail_mail', 'oi.tests')
    def test_message_process_external_notification_reply(self):
        ...
        self.assertFalse(new_msg.is_internal, "Responses to messages sent by oibot should always be public.")
        ...
        new_msg = self.test_record.message_ids[0]
        self.assertTrue(new_msg.is_internal, "Responses to messages sent by anyone but oibot should keep"
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by oi should update thread accordingly """
    ...
    def test_message_process_references_external_buggy_message_id(self):
        ...
        oi should update thread accordingly. Special case when the
    ...
    def test_message_process_extra_model_res_id(self):
        """ Incoming email with ref holding model / res_id but that does not match any message in the thread: must raise since Oi saas-3 """
        self.assertRaises(ValueError,
                          self.format_and_process, MAIL_TEMPLATE,
                          self.partner_1.email_formatted, f'noone@{self.alias_domain}', subject='spam',
                          extra=f'In-Reply-To: <12321321-oi-{self.test_record.id}-{self.test_record._name}@{socket.gethostname()}>')

        # when 6.1 messages are present, compat mode is available
        # Oi 10 update: compat mode has been removed and should not work anymore
        ...
        self.assertRaises(
            ...
            self.format_and_process, MAIL_TEMPLATE,
            self.partner_1.email_formatted, f'noone@{self.alias_domain}>', subject='spam',
            extra=f'In-Reply-To: <12321321-oi-{self.test_record.id}-mail.test.gateway@{socket.gethostname()}>')
    ...
    def test_message_hebrew_iso8859_8_i(self):
        ...
        # python, check that Oi is still capable of decoding it.
...
class TestMailGatewayLoops(MailGatewayCommon):
    ...
    @mute_logger('oi.addons.mail.models.mail_mail', 'oi.addons.mail.models.mail_thread')
    ...
    def test_routing_loop_auto_notif(self):
        """ Test Oi servers talking to each other """
        with self.mock_mail_gateway():
            record = self.format_and_process(
                MAIL_TEMPLATE,
                self.other_partner.email_formatted,
    ...
    def test_routing_loop_follower_alias(self):
        ...
        # simulate this email coming back to the same Oi server -> msg_id is
...
class TestMailGatewayReplies(MailGatewayCommon):
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{email.message_id} {log.message_id} {reply.message_id}',  # should contain reference to OiExternal message, logs to fill up history
                },
                'mail_mail_values': {
    ...
    def test_routing_reply_internal_messages(self):
        """ Test routing notably between two Ois when internal messages
        ...
        RFQ-like                                        creation log
                                                        initial_msg
        Oi2 replies           creation log
                                reply                   reply
        -some internal work-                            user_notification
        Oi1 replies           reply_2                 reply_2 (incoming email)
        -some internal work-                            log
        Oi2 replies           reply_3                 reply_3 (outgoing email)

        Purpose: have references from Oi2 containing message IDs to try to
        ...
        gateway_record.message_post(
            author_id=self.env.ref('base.partner_root').id,
            body='OiExternal Inquiry',
            email_from=self.partner_1.email_normalized,
            message_type='comment',
            subtype_id=self.env.ref('mail.mt_comment').id,
        )
        self.assertEqual(gateway_record.message_partner_ids, self.partner_admin)
        log, oiext_msg = gateway_record.message_ids[1], gateway_record.message_ids[0]
        self.assertEqual(oiext_msg.parent_id, log, 'Log serves as thread ancestor')
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    'message_id': reply.message_id,
                    'references': f'{log.message_id} {oiext_msg.message_id} {reply.message_id}',  # should contain reference to OiExternal message
                },
                'mail_mail_values': {
        ...
        # coming from Oi1: their reply as an incoming email
        with self.mock_mail_gateway():
            self.format_and_process(
                MAIL_TEMPLATE, self.email_from, reply.reply_to,
                subject='Gateway Creation',
                extra=f'References: {reply.message_id} <msg1@oi1>',
                debug_log=True,
            )
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{log.message_id} {oiext_msg.message_id} {reply.message_id} {reply_2.message_id}',  # should contain reference to OiExternal message
                },
                'mail_mail_values': {
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{oiext_msg.message_id} {reply.message_id} {reply_2.message_id} {reply_3.message_id}',  # should contain reference to OiExternal message
                },
                'mail_mail_values': {
                    'notified_partner_ids': self.partner_1 + self.partner_admin,
                    'parent_id': log,  # log serves as thread ancestor
