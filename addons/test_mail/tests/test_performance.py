...
from oi.tools.misc import limited_field_access_token
from oi.addons.base.tests.common import TransactionCaseWithUserDemo
from oi.addons.mail.tests.common import MailCommon
from oi.addons.mail.tools.discuss import Store
from oi.tests import Form, users, warmup, tagged
...
class TestBaseAPIPerformance(BaseMailPerformance):
    ...
    def test_activity_full(self):
        ...
        with self.assertQueryCount(admin=6, employee=6):
            ...
            # read activity_type to normalize cache between plus and community
            # voip module read activity_type during create leading to one less query in plus on action_feedback
    ...
    def test_activity_mixin(self):
        ...
        with self.assertQueryCount(admin=5, employee=5):
            ...
            # read activity_type to normalize cache between plus and community
            # voip module read activity_type during create leading to one less query in plus on action_close
            _category = activity.activity_type_id.category
        ...
        cls.messages_all = cls.env['mail.message'].sudo().create([
            {
                ...
                'link_preview_ids': [
                    (0, 0, {
                        'source_url': 'https://www.odoodooi.com',
                    }), (0, 0, {
                            ...
                            "mail.message": self._filter_messages_fields(
                                {
                                    ...
                                    "email_from": '"OiBot" <oibot@example.com>',
                                    "id": message.id,
                                    "is_discussion": True,
                                    "is_note": False,
                                    "linkPreviews": [],
