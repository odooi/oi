...
from oi.addons.project.tests.test_project_base import TestProjectCommon
...
class TestPortalProject(TestProjectPortalCommon):
    ...
    @mute_logger('oi.addons.base.ir.ir_model')
...
class TestAccessRightsPrivateTask(TestAccessRights):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.private_task = cls.env['project.task'].create({'name': 'OiBot Private Task'})
