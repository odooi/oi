...
from oi import Command

from oi.addons.microsoft_calendar.models.microsoft_sync import MicrosoftSync
...
from oi.addons.microsoft_calendar.tests.common import TestCommon, mock_get_token, _modified_date_in_the_future, patch_api
from oi.exceptions import UserError, ValidationError
...
class TestUpdateEvents(TestCommon):
    ...
    # -------------------------------------------------------------------------------
    # Update from Oi to Outlook
    # -------------------------------------------------------------------------------

    # ------ Simple event ------

    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_oi_simple_event_without_sync(self, mock_patch):
        """
        Update an Oi event without Outlook sync enabled
        """
    ...
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_simple_event_from_oi(self, mock_patch):
        """
        Update an Oi event with Outlook sync enabled
        """
    ...
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_simple_event_from_oi_attendee_calendar(self, mock_patch):
        """
        Update an Oi event from the attendee calendar.
        """
    ...
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_name_of_one_event_of_recurrence_from_oi(self, mock_patch):
        """
        Update one Oi event name from a recurrence from the organizer calendar.
        """
    ...
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_start_of_one_event_of_recurrence_from_oi(self, mock_patch):
        """
        Update one Oi event start date from a recurrence from the organizer calendar.
        """
        if not self.sync_oi_recurrences_with_outlook_feature():
            return
        # arrange
    ...
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_start_of_one_event_of_recurrence_from_oi_with_overlap(self, mock_patch):
        """
        Update one Oi event start date from a recurrence from the organizer calendar, in order to
    ...
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_name_of_one_event_of_recurrence_from_oi_attendee_calendar(self, mock_patch):
        """
        Update one Oi event name from a recurrence from the atendee calendar.
        """
        if not self.sync_oi_recurrences_with_outlook_feature():
            return
        # arrange
    ...
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_name_of_one_and_future_events_of_recurrence_from_oi(
        self, mock_patch, mock_insert, mock_delete
    ):
        """
        Update a Oi event name and future events from a recurrence from the organizer calendar.
        """
        if not self.sync_oi_recurrences_with_outlook_feature():
            return
        # arrange
    ...
    @patch.object(MicrosoftCalendarService, 'insert')
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_start_of_one_and_future_events_of_recurrence_from_oi(
        self, mock_patch, mock_insert, mock_delete
    ):
        """
        Update a Oi event start date and future events from a recurrence from the organizer calendar.
        """
        if not self.sync_oi_recurrences_with_outlook_feature():
            return
    ...
    @patch.object(MicrosoftCalendarService, 'insert')
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_start_of_one_and_future_events_of_recurrence_from_oi_with_overlap(
        self, mock_patch, mock_insert, mock_delete
    ):
        """
        Update a Oi event start date and future events from a recurrence from the organizer calendar,
    ...
    @patch.object(MicrosoftCalendarService, 'insert')
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_one_and_future_events_of_recurrence_from_oi_attendee_calendar(
        self, mock_patch, mock_insert, mock_delete
    ):
        """
        Update a Oi event name and future events from a recurrence from the attendee calendar.
        """
        if not self.sync_oi_recurrences_with_outlook_feature():
            return
        # arrange
    ...
    @patch.object(MicrosoftCalendarService, 'insert')
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_name_of_all_events_of_recurrence_from_oi(
        self, mock_patch, mock_insert, mock_delete
    ):
        """
    ...
    @patch.object(MicrosoftCalendarService, 'insert')
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_start_of_all_events_of_recurrence_from_oi(
        self, mock_patch, mock_insert, mock_delete
    ):
        """
    ...
    @patch.object(MicrosoftCalendarService, 'insert')
    @patch.object(MicrosoftCalendarService, 'patch')
    def test_update_all_events_of_recurrence_from_oi_attendee_calendar(
        self, mock_patch, mock_insert, mock_delete
    ):
        """
    ...
    def test_forbid_simple_event_become_recurrence_sync_on(self, mock_patch):
        """
        Forbid in Oi simple event becoming a recurrence when Outlook Calendar sync is active.
        """
    ...
    def test_update_synced_event_with_sync_config_paused(self, mock_patch):
        ...
        # Try to update a simple event in Oi Calendar.
    ...
    def test_changing_event_organizer_to_another_user(self, mock_insert, mock_delete, mock_get_events):
        """
        Allow editing the event organizer to another user only if the proposed organizer have its Oi Calendar synced.
    ...
    def test_restart_sync_with_synced_recurrence(self, mock_patch):
        """ Ensure that sync restart is not blocked when there are recurrence outliers in Oi database. """
    ...
    def test_update_old_event_synced_with_outlook(self, mock_get_events, mock_write_from_microsoft):
        """
        There are old events in Oi which share the same state with Microsoft and get updated (without changes) in Oi
        due to a few seconds of update time difference, triggering lots of unwanted spam for attendees on Microsoft side.
        Don't update old events in Oi if update time difference between Microsoft and Oi is not significant.
        """
        ...
        # Mock the modification time in Microsoft with 10 minutes ahead Oi event 'write_date'.
        # Synchronize Microsoft Calendar and ensure that the skipped event was not updated in Oi.
        mock_get_events.return_value = (
            MicrosoftEvent([dict(
                self.simple_event_from_outlook_organizer,
