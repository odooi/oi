...
from oi.tests.common import HttpCase
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a Oi event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Oi events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Oi events
        # -----------------------------------------------------------------------------------------

        self.expected_oi_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_oi_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_oi_recurrency_events_from_outlook = [
            {
    ...
    def sync_oi_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in Oi. A trick for deactivating the checking
    ...
    def assert_oi_event(self, oi_event, expected_values):
        """
        Assert that an Oi event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        oi_event_values = oi_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(oi_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, oi_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_oi_recurrence(self, oi_recurrence, expected_values):
        """
        Assert that an Oi recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        oi_recurrence_values = oi_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, oi_recurrence_values.get(k), msg=f"'{k}' mismatch")
