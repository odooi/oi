...
class Meeting(models.Model):
    ...
    def create(self, vals_list):
        ...
        # Forbid recurrence creation in Oi, suggest its creation in Outlook due to the spam limitation.
            ...
            if not sender_sync_status and current_sync_status:
                raise ValidationError(
                    _("For having a different organizer in your event, it is necessary that "
                      "the organizer have its Oi Calendar synced with Outlook Calendar."))
    ...
    def _forbid_recurrence_update(self):
        ...
        if any(not record.ms_universal_event_id for record in self):
            # If any event is not synced, suggest deleting it in Oi and recreating it in Outlook.
            error_msg = _(
                "Due to an Outlook Calendar limitation, recurrence updates must be done directly in Outlook Calendar.\n"
                "If this recurrence is not shown in Outlook Calendar, you must delete it in Oi Calendar and recreate it in Outlook Calendar.")
    ...
    def write(self, values):
        ...
        # Forbid recurrence updates through Oi and suggest user to update it in Outlook.
    ...
    def _get_microsoft_sync_domain(self):
        ...
        # Define 'custom_lower_bound_range' param for limiting old events updates in Oi and avoid spam on Microsoft.
    ...
    @api.model
    def _microsoft_to_oi_values(self, microsoft_event, default_reminders=(), default_values=None, with_ids=False):
        ...
        commands_attendee, commands_partner = self._oi_attendee_commands_m(microsoft_event)
        ...
        alarm_commands = self._oi_reminders_commands_m(microsoft_event)
        if alarm_commands:
    ...
    @api.model
    def _microsoft_to_oi_recurrence_values(self, microsoft_event, default_values=None):
        timeZone_start = pytz.timezone(microsoft_event.start.get('timeZone'))
        timeZone_stop = pytz.timezone(microsoft_event.end.get('timeZone'))
        start = parse(microsoft_event.start.get('dateTime')).astimezone(timeZone_start).replace(tzinfo=None)
        if microsoft_event.isAllDay:
    ...
    @api.model
    def _oi_attendee_commands_m(self, microsoft_event):
        commands_attendee = []
        commands_partner = []
        ...
        if microsoft_event.match_with_oi_events(self.env):
            existing_attendees = self.env['calendar.attendee'].search([
                ('event_id', '=', microsoft_event.oi_id(self.env)),
                ('email', 'in', emails)])
        ...
        for oi_attendee in attendees_by_emails.values():
            # Remove old attendees
            if oi_attendee.email not in emails:
                commands_attendee += [(2, oi_attendee.id)]
                commands_partner += [(3, oi_attendee.partner_id.id)]
        return commands_attendee, commands_partner

    @api.model
    def _oi_reminders_commands_m(self, microsoft_event):
        reminders_commands = []
        if microsoft_event.isReminderOn:
            event_id = self.browse(microsoft_event.oi_id(self.env))
    ...
    def _ensure_attendees_have_email(self):
        ...
        if invalid_event_ids:
            ...
            raise ValidationError(_("For a correct synchronization between Oi and Outlook Calendar, "
    ...
    def _cancel_microsoft(self):
        ...
        There are 2 cases:
          1) the organizer is an Oi user: he's the only one able to delete the Oi event. Attendees can just decline.
          2) the organizer is NOT an Oi user: any attendee should remove the Oi event.
        """
        user = self.env.user
