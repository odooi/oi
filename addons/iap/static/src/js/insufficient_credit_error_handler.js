...
class InsufficientCreditDialog extends Component {
    ...
    async onWillStart() {
        ...
        const { isPlus } = oi.info;
        if (errorData.trial && isPlus) {
            this.buttonMessage = _t("Start a Trial at Oi");
        } else {
...
function insufficientCreditHandler(env, error, originalError) {
    ...
    if (data && data.name === "oi.addons.iap.tools.iap_tools.InsufficientCreditError") {
