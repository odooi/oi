...
from oi import _, models, fields
from oi.tools.float_utils import float_is_zero, float_round
    ...
    def _get_valuation_price_and_qty(self, related_aml, to_curr):
        ...
        if related_aml.product_id in boms:
            ...
            if float_is_zero(valuation_total_qty, precision_rounding=related_aml.product_uom_id.rounding or related_aml.product_id.uom_id.rounding):
                raise UserError(_('Oi is not able to generate the anglo saxon entries. The total valuation of %s is zero.', related_aml.product_id.display_name))
