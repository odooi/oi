...
import { getRangePosition } from '@web_editor/js/editor/oi-editor/src/utils/utils';
import { utils as uiUtils } from "@web/core/ui/ui_service";
import { closestScrollableY } from "@web/core/utils/scrolling";
import { useThrottleForAnimation } from "@web/core/utils/timing";
import { onWillUnmount, onWillStart, reactive, status, useSubEnv } from "@oi/owl";
...
export class MassMailingHtmlField extends HtmlField {
    ...
    async commitChanges(...args) {
        ...
        this._pendingCommitChanges = (async () => {
            ...
            this.wysiwyg.oiEditor.historyPauseSteps();
            ...
            this.wysiwyg.oiEditor.observerUnactive('toInline');
            ...
            iframe.remove();
            this.wysiwyg.oiEditor.observerActive('toInline');
            const inlineHtml = editableClone.innerHTML;
            $editorEnable.addClass('editor_enable');
            this.wysiwyg.oiEditor.historyUnpauseSteps();
            this.wysiwyg.oiEditor.historyRevertCurrentStep();
    ...
    async _resetIframe() {
        ...
        this.wysiwyg.$iframeBody.find('.oi-editor-editable').removeClass('o_editable');

        initializeDesignTabCss(this.wysiwyg.getEditable());
        this.wysiwyg.getEditable().find('img').attr('loading', '');

        this.wysiwyg.oiEditor.observerFlush();
        this.wysiwyg.oiEditor.historyReset();
    ...
    async _onSnippetsLoaded() {
        ...
        $themeSelectorNew.on('click', '.dropdown-item', async (e) => {
            ...
            // the Oi editor before resetting the history.
            setTimeout(() => {
                this.wysiwyg.historyReset();
                // Update undo/redo buttons
                this.wysiwyg.oiEditor.dispatchEvent(new Event('historyStep'));

                // The selection has been lost when switching theme.
                const document = this.wysiwyg.oiEditor.document;
                const selection = document.getSelection();
                const p = this.wysiwyg.oiEditor.editable.querySelector('p');
                if (p && selection) {
        ...
        if (editableAreaIsEmpty) {
            ...
            // We actually hide the OiEditor toolbar by calling
        ...
        this.wysiwyg.oiEditor.activateContenteditable();
    }
    _getCodeViewEl() {
    ...
    async _switchThemes(themeParams) {
        ...
        this.wysiwyg.oiEditor.resetContent($newLayout[0].outerHTML);
        ...
        this.onIframeUpdated();
        this.wysiwyg.oiEditor.historyStep(true);
    ...
    async _lazyloadWysiwyg() {
        ...
        const wysiwygModule = await oi.loader.modules.get('@mass_mailing/js/mass_mailing_wysiwyg');
