...
from oi import _, http, tools, SUPERUSER_ID
from oi.addons.html_editor.tools import get_video_url_data
from oi.exceptions import UserError, MissingError, AccessError
...
DEFAULT_LIBRARY_ENDPOINT = 'https://media-api.odoodooi.com'
    ...
    def link_preview_metadata_internal(self, preview_url):
        try:
            ...
            if (action_name.startswith('m-') or '.' in action_name) and action_name in request.env and not request.env[action_name]._abstract:
                # if path format is `oi/<model>/<record_id>` so we use `action_name` as model name
