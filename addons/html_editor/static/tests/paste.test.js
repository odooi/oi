...
import { manuallyDispatchProgrammaticEvent as dispatch, press, waitFor } from "@oi/hoot-dom";
import { animationFrame, tick } from "@oi/hoot-mock";
import { onRpc, patchWithCleanup } from "@web/../tests/web_test_helpers";
import { setupEditor, testEditor } from "./_helpers/editor";
import { cleanLinkArtifacts, unformat } from "./_helpers/format";
import { getContent, setSelection } from "./_helpers/selection";
import { pasteHtml, pasteOiEditorHtml, pasteText, undo } from "./_helpers/user_actions";
...
describe("Special cases", () => {
    describe("lists", () => {
        ...
        test("should paste a deeply nested list copied outside from oi", async () => {
            await testEditor({
                contentBefore: "<ul><li>[]<br></li></ul>",
                stepFunction: async (editor) => {
                    pasteHtml(
...
describe("link", () => {
    describe("range collapsed", () => {
        ...
        test("should replace link for new content (url) when pasting in an empty link (collapsed)", async () => {
            ...
            pasteText(editor, "http://odoodooi.com");
            await animationFrame();
            expect(".o-we-powerbox").toHaveCount(0);
            expect(cleanLinkArtifacts(getContent(el))).toBe(
                `<p>xy<a href="http://odoodooi.com">http://odoodooi.com</a>[]z</p>`
            );
        });
        ...
        test("should paste and transform plain text content over an empty link (collapsed)", async () => {
            await testEditor({
                ...
                stepFunction: async (editor) => {
                    pasteText(editor, "abc www.odoodooi.com xyz");
                },
            ...
            await testEditor({
                contentBefore: '<p><a href="#">[]\u200B</a></p>',
                stepFunction: async (editor) => {
                    pasteText(editor, "odoodooi.com\ngoogle.com");
                },
                contentAfter:
                    '<div><a href="http://odoodooi.com">odoodooi.com</a></div>' +
        ...
        test("should paste html content over an empty link (collapsed)", async () => {
            await testEditor({
                ...
                stepFunction: async (editor) => {
                    pasteHtml(
                        editor,
                        '<a href="www.odoodooi.com">odoodooi.com</a><br><a href="google.com">google.com</a>'
                    );
                },
                contentAfter:
                    '<p><a href="www.odoodooi.com">odoodooi.com</a></p><p><a href="https://google.com">google.com[]</a></p>',
            });
        });
        test("should paste html content over an empty link (collapsed) (2)", async () => {
            await testEditor({
                ...
                stepFunction: async (editor) => {
                    pasteHtml(
                        editor,
                        '<a href="www.odoodooi.com">odoodooi.com</a><br><a href="www.google.com">google.com</a>'
                    );
                },
                contentAfter:
                    '<p><a href="www.odoodooi.com">odoodooi.com</a></p><p><a href="www.google.com">google.com[]</a></p>',
            });
        });
    ...
    describe("range not collapsed", () => {
        ...
        test("should restore selection when pasting plain text followed by UNDO (2) (not collapsed)", async () => {
            await testEditor({
                contentBefore: "<p>[abc]</p>",
                stepFunction: async (editor) => {
                    pasteText(editor, "www.odoodooi.com");
                    undo(editor);
                },
                contentAfter: "<p>[abc]</p>",
            });
        ...
        test("should restore selection when pasting plain text followed by UNDO (3) (not collapsed)", async () => {
            await testEditor({
                contentBefore: "<p>[abc]</p>",
                stepFunction: async (editor) => {
                    pasteText(editor, "def www.odoodooi.com xyz");
                    undo(editor);
                },
                contentAfter: "<p>[abc]</p>",
            });
        ...
        test("should paste and transform plain text content over an image link if all of its contents is selected (not collapsed) (1)", async () => {
            ...
            expect(".o-we-powerbox").toHaveCount(1);
            expect(getContent(el)).toBe(
                `<p>abhttps://download.oicdn.com/icons/website/static/description/icon.png[]cd</p>`
            );
            await press("Enter");
...
describe("Oi editor own html", () => {
    test("should paste html as is", async () => {
        await testEditor({
            ...
            stepFunction: async (editor) => {
                pasteOiEditorHtml(editor, '<div class="custom-paste oe_unbreakable">b</div>');
            },
    ...
    test("should not paste unsafe content", async () => {
        await testEditor({
            contentBefore: "<p>a[]b</p>",
            stepFunction: async (editor) => {
                pasteOiEditorHtml(editor, `<script>console.log('xss attack')</script>`);
            },
...
describe("editable in iframe", () => {
    test("should paste oi-editor html", async () => {
        const { el, editor } = await setupEditor("<p>[]</p>", { props: { iframe: true } });
        pasteOiEditorHtml(editor, `<p>text<b>bold text</b>more text</p>`);
    ...
    <head>
        ...
        <style>
            ...
            .font12 {
                ...
                font-style: italic;
                text-decoration: none;
                font-family: "Oi Unicode Support Noto";
    ...
    <table xmlns="http://www.w3.org/1999/xhtml" cellspacing="0" cellpadding="0" dir="ltr" border="1"
        ...
        <tbody>
            <tr style="height:21px;">
                <td style="overflow:hidden;padding:2px 3px 2px 3px;vertical-align:bottom;font-family:Oi Unicode Support Noto;font-weight:normal;font-style:italic;color:#495057;"
            ...
            <tr style="height:21px;">
                <td style="overflow:hidden;padding:2px 3px 2px 3px;vertical-align:bottom;font-family:Oi Unicode Support Noto;font-weight:bold;color:#495057;"
            ...
            <tr style="height:21px;">
                <td style="overflow:hidden;padding:2px 3px 2px 3px;vertical-align:bottom;background-color:#ffff00;font-family:Oi Unicode Support Noto;font-weight:normal;color:#495057;"
...
describe("onDrop", () => {
    ...
    test("should move an image if it originated from the editor", async () => {
        ...
        await animationFrame();
        const imageHTML = dragdata.getData("application/vnd.oi.oi-editor-node");
        ...
        // Simulate the application/vnd.oi.oi-editor-node data that the browser would do.
        dropData.setData("application/vnd.oi.oi-editor-node", imageHTML);
        await dispatch(pElement, "drop", { dataTransfer: dropData });
        await animationFrame();
