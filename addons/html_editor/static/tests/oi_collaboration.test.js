...
import { advanceTime, animationFrame, tick, waitUntil } from "@oi/hoot-dom";
import { Component, xml } from "@oi/owl";
...
class PeerTest {
    ...
    async focus() {
        return this.plugins["collaborationOi"].joinPeerToPeer();
    }
    ...
    async writeToServer() {
        ...
        const lastId = this.plugins.collaborationOi.getLastHistoryStepId(
        ...
        for (const peer of Object.values(this.peers)) {
            ...
            peer.onlineMutex.exec(async () => {
                return peer.plugins.collaborationOi.onServerLastIdUpdate(String(lastId));
            });
        }
    }
    async setOnline() {
...
describe("Stale detection & recovery", () => {
    describe("detect stale while unfocused", () => {
        test("should do nothing until focus", async () => {
            ...
            expect(peers.p1.plugins.collaborationOi.isDocumentStale).toBe(false, {
            ...
            expect(peers.p2.plugins.collaborationOi.isDocumentStale).toBe(false, {
            ...
            expect(peers.p3.plugins.collaborationOi.isDocumentStale).toBe(true, {
            ...
            expect(peers.p3.plugins.collaborationOi.isDocumentStale).toBe(false, {
    ...
    describe("detect stale while focused", () => {
        describe("recover from missing steps", () => {
            test("should recover from missing steps", async () => {
                ...
                const p3Spies = makeSpies(peers.p3.plugins.collaborationOi, [
                ...
                expect(peers.p1.plugins.collaborationOi.historyShareId).toBe(
                    peers.p2.plugins.collaborationOi.historyShareId,
                    {
                        message: "p1 and p2 should have the same historyShareId",
                    }
                );
                expect(peers.p1.plugins.collaborationOi.historyShareId).toBe(
                    peers.p3.plugins.collaborationOi.historyShareId,
                    {
        ...
        describe("recover from snapshot", () => {
            test("should wait for all peer to recover from snapshot", async () => {
                ...
                const p2Spies = makeSpies(peers.p2.plugins.collaborationOi, [
                    "recoverFromStaleDocument",
                    "resetFromServerAndResyncWithPeers",
                    "processMissingSteps",
                    "applySnapshot",
...
describe("Disconnect & reconnect", () => {
    test("should sync history when disconnecting and reconnecting to internet", async () => {
        ...
        const p1PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p2.plugins.collaborationOi, {
        ...
        const p2PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p1.plugins.collaborationOi, {
                async processMissingSteps() {
                    super.processMissingSteps(...arguments);
                    resolve();
                },
...
describe("History steps Ids", () => {
    ...
    test("should clear history step ids when resetting from server", async () => {
        ...
        expect(peers.p2.plugins.collaborationOi.isDocumentStale).toBe(true, {
