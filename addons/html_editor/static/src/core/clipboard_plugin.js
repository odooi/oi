...
export class ClipboardPlugin extends Plugin {
    ...
    onCopy(ev) {
        ...
        const oiHtml = dataHtmlElement.innerHTML;
        const oiText = selection.textContent();
        ev.clipboardData.setData("text/plain", oiText);
        ev.clipboardData.setData("text/html", oiHtml);
        ev.clipboardData.setData("application/vnd.oi.oi-editor", oiHtml);
        if (commonAncestor && commonAncestor.nodeType === Node.ELEMENT_NODE) {
            this.dispatchTo("normalize_handlers", commonAncestor);
        }
    }
    ...
    onPaste(ev) {
        ...
        this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
            this.handlePasteOiEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePasteOiEditorHtml(clipboardData) {
        const oiEditorHtml = clipboardData.getData("application/vnd.oi.oi-editor");
    ...
    onDragStart(ev) {
        if (ev.target.nodeName === "IMG") {
            ...
            ev.dataTransfer.setData(
                "application/vnd.oi.oi-editor-node",
    ...
    async onDrop(ev) {
        ...
        const imageNodeHTML = ev.dataTransfer.getData("application/vnd.oi.oi-editor-node");
        const image =
            imageNodeHTML &&
            this.dragImage &&
...
// @phoenix @todo: move to Oi plugin?
/**
