...
export function prepareUpdate(...args) {
    const closestRoot =
        args.length &&
        ancestors(args[0]).find((ancestor) => ancestor.classList.contains("oi-editor-editable"));
    ...
    while (positions.length) {
        ...
        if (options.debug) {
            const editable = el && closestElement(el, ".oi-editor-editable");
...
export function restoreState(prevStateData, debug = false) {
    ...
    if (debug) {
        const editable = closestElement(node, ".oi-editor-editable");
        console.log(
            "%c" +
