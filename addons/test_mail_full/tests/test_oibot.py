...
from oi.addons.test_mail.tests.common import TestRecipients
from oi.tests import tagged
from oi.tools import mute_logger


@tagged("oibot")
class TestOibot(MailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(TestOibot, cls).setUpClass()
        cls.test_record = cls.env['mail.test.simple'].with_context(cls._test_context).create({'name': 'Test', 'email_from': 'ignasse@example.com'})

        cls.oibot = cls.env.ref("base.partner_root")
        ...
        cls.oibot_ping_body = f'<a href="http://odoodooi.com/oi/res.partner/{cls.oibot.id}" class="o_mail_redirect" data-oe-id="{cls.oibot.id}" data-oe-model="res.partner" target="_blank">@OiBot</a>'
        cls.test_record_employe = cls.test_record.with_user(cls.user_employee)

    @mute_logger('oi.addons.mail.models.mail_mail')
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_oibot()
        oibot = self.env.ref("base.partner_root")
        oibot_in_fetch_listeners = self.env['discuss.channel.member'].search([('channel_id', '=', channel.id), ('partner_id', '=', oibot.id)])
        self.assertEqual(len(oibot_in_fetch_listeners), 1, 'oibot should appear only once in channel_fetch_listeners')

    @mute_logger('oi.addons.mail.models.mail_mail')
    def test_oibot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.oibot_ping_body, 'partner_ids': [self.oibot.id, self.user_admin.partner_id.id]})
        ...
        # Oibot should not be a follower but user_employee and user_admin should
        follower = self.test_record.message_follower_ids.mapped('partner_id')
        self.assertNotIn(self.oibot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last oibot message instead
            sender=self.oibot,
            answer=("@OiBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.oibot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.oibot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('oi.addons.mail.models.mail_mail')
    def test_oibot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @oibot right now", 'partner_ids': []})
        self.assertNextMessage(
