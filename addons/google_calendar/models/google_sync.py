...
from oi import api, fields, models, _
from oi.modules.registry import Registry
from oi.tools import ormcache_context, email_normalize
from oi.osv import expression
from oi.sql_db import BaseCursor

from oi.addons.google_calendar.utils.google_event import GoogleEvent
from oi.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from oi.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the Oi database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Oi.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Oi DB (_sync_google2oi).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_oi2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2oi(self, google_events: GoogleEvent, write_dates=None, default_reminders=()):
        """Synchronize Google recurrences in Oi. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in Oi
        :param write_dates: A dictionary mapping Oi record IDs to their write dates.
        :return: synchronized oi recurrences
        """
        ...
        oi_values = [
            dict(self._oi_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_oi = self.with_context(dont_notify=True)._create_from_google(new, oi_values)
        cancelled = existing.cancelled()
        cancelled_oi = self.browse(cancelled.oi_ids(self.env))

        # Check if it is a recurring event that has been rescheduled.
        # We have to check if an event already exists in Oi.
        # Explanation:
        ...
        if rescheduled_events:
            ...
            cancelled_oi += self.env['calendar.event'].search([('google_id', 'in', google_ids_to_remove)])

        cancelled_oi.exists()._cancel()
        synced_records = new_oi + cancelled_oi
        pending = existing - cancelled
        pending_oi = self.browse(pending.oi_ids(self.env)).exists()
        for gevent in pending:
            oi_record = self.browse(gevent.oi_id(self.env))
            if oi_record not in pending_oi:
                # The record must have been deleted in the mean time; nothing left to sync
                continue
            # Last updated wins.
            # This could be dangerous if google server time and oi server time are different
            updated = parse(gevent.updated)
            # Use the record's write_date to apply Google updates only if they are newer than Oi's write_date.
            oi_record_write_date = write_dates.get(oi_record.id, oi_record.write_date)
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not oi_record_write_date or updated >= pytz.utc.localize(oi_record_write_date):
                vals = dict(self._oi_values(gevent, default_reminders), need_sync=False)
                oi_record.with_context(dont_notify=True)._write_from_google(gevent, vals)
                synced_records |= oi_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Oi to Google
    ...
    def _check_any_records_to_sync(self):
        """ Returns True if there are pending records to be synchronized from Oi to Google, False otherwise. """
        is_active_clause = (self._active_name, '=', True) if self._active_name else expression.TRUE_LEAF
    ...
    @api.model
    def _oi_values(self, google_event: GoogleEvent, default_reminders=()):
