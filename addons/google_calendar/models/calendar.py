...
from oi import api, fields, models, tools, _
...
class Meeting(models.Model):
    ...
    @api.model
    def _oi_values(self, google_event, default_reminders=()):
        ...
        alarm_commands = self._oi_reminders_commands(reminder_command)
        attendee_commands, partner_commands = self._oi_attendee_commands(google_event)
    ...
    @api.model
    def _oi_attendee_commands(self, google_event):
        attendee_commands = []
        partner_commands = []
        ...
        if google_event.exists(self.env):
            event = google_event.get_oi_event(self.env)
        ...
        for oi_attendee in attendees_by_emails.values():
            ...
            email = tools.email_normalize(oi_attendee.email)
            if email not in emails and email != self.env.user.email:
                attendee_commands += [(2, oi_attendee.id)]
                partner_commands += [(3, oi_attendee.partner_id.id)]
        return attendee_commands, partner_commands

    @api.model
    def _oi_reminders_commands(self, reminders=()):
        commands = []
    ...
    def action_mass_archive(self, recurrence_update_setting):
        """ Delete recurrence in Oi if in 'all_events' or in 'future_events' edge case, triggering one mail. """
        self.ensure_one()
    ...
    def _google_values(self):
        ...
        values = {
            ...
            'extendedProperties': {
                'shared': {
                    '%s_oi_id' % self.env.cr.dbname: self.id,
                },
            },
            'reminders': {
                'overrides': reminders,
        ...
        if self.user_id and self.user_id != self.env.user and not bool(self.user_id.sudo().google_calendar_token):
            # The organizer is an Oi user that do not sync his calendar
            ...
            # the owner is not an Oi user: We don't store the real owner identity (mail)
            ...
            # values['extendedProperties']['private] should be used if the owner is not an oi user
