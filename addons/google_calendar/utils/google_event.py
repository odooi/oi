...
class GoogleEvent(abc.Set):
    ...
    Inspired by Oi recordset, one instance can be a single Google event or a
    ...
    def oi_id(self, env):
        self.oi_ids(env)  # load ids
        return self._oi_id

    def _meta_oi_id(self, dbname):
        """Returns the Oi id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_oi_id' % dbname)
        if o_id:
            return int(o_id)

    def oi_ids(self, env):
        ids = tuple(e._oi_id for e in self if e._oi_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_oi_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_oi_ids_from_metadata(env, model)

        return tuple(e._oi_id for e in self)

    def _load_oi_ids_from_metadata(self, env, model):
        unsure_oi_ids = tuple(e._meta_oi_id(env.cr.dbname) for e in self)
        oi_events = model.browse(_id for _id in unsure_oi_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Oi id.
        # Therefore, we only consider Oi records without google id when trying
        # to match events.
        o_ids = oi_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            oi_id = e._meta_oi_id(env.cr.dbname)
            if oi_id in o_ids:
                e._events[e.id]['_oi_id'] = oi_id

    def _load_oi_ids_from_db(self, env, model):
        oi_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in oi_events}  # {google_id: oi_id}
        existing_google_ids = oi_events.mapped('google_id')
        for e in self:
            oi_id = mapping.get(e.id)
            if oi_id:
                e._events[e.id]['_oi_id'] = oi_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Oi.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Oi (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Oi) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Oi several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_oi_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.oi_ids(env)
        events.oi_ids(env)

        return self.filter(lambda e: e._oi_id)
    ...
    def get_oi_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.oi_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.oi_id(self.env)).base_event_id
