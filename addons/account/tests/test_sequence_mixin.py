...
from oi.tests import Form, tagged, TransactionCase
from oi import fields, api, SUPERUSER_ID, Command
...
class TestSequenceMixin(TestSequenceMixinCommon):
    ...
    def test_journal_sequence(self):
        ...
        with Form(copy2) as move_form:  # It is editable in the form
            with self.assertLogs('oi.tests.form') as cm:
                move_form.name = 'MyMISC/2016/0001'
                self.assertTrue(cm.output[0].startswith('WARNING:oi.tests.form.onchange:'))
    ...
    def test_journal_sequence_ordering(self):
        ...
        with self.assertRaises(psycopg2.DatabaseError), mute_logger('oi.sql_db'), self.env.cr.savepoint():
