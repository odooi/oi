...
function initialize_map() {
    ...
    var map = new google.maps.Map(document.getElementById('oi-google-map'), {
        zoom: 1,
    ...
    async function initializeMarkers(oiPartnerData) {
        // Create the markers and cluster them on the map
        if (oiPartnerData) { /* oi_partner_data special variable should have been defined in google_map.xml */
            const markerPromises = [];
            for (var i = 0; i < oi_partner_data.counter; i++) {
                const prom = set_marker(oi_partner_data.partners[i])
    ...
    initializeMarkers(oi_partner_data);
