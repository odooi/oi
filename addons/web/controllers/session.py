...
from oi.exceptions import AccessError, UserError, AccessDenied
...
class Session(http.Controller):
    ...
    def get_session_info(self):
        # Crapy workaround for unupdatable Oi Mobile App iOS (Thanks Apple :@)
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        if auth_info['uid'] != request.session.uid:
            # Crapy workaround for unupdatable Oi Mobile App iOS (Thanks Apple :@) and Android
        ...
        registry = oi.modules.registry.Registry(db)
        with registry.cursor() as cr:
            env = oi.api.Environment(cr, request.session.uid, request.session.context)
            if not request.db:
    ...
    def account(self):
        ...
        return 'https://accounts.odoodooi.com/oauth2/auth?' + url_encode(params)
