...
class Home(http.Controller):
    ...
    def index(self, s_action=None, db=None, **kw):
        ...
        return request.redirect_query('/oi', query=request.params)

    def _web_client_readonly(self):
        return False

    # ideally, this route should be `auth="user"` but that don't work in non-monodb mode.
    @http.route(['/web', '/oi', '/oi/<path:subpath>', '/scoped_app/<path:subpath>'], type='http', auth="none", readonly=_web_client_readonly)
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if request.httprequest.method == 'POST':
            ...
            except oi.exceptions.AccessDenied as e:
                if e.args == oi.exceptions.AccessDenied().args:
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = oi.SUPERUSER_ID
    ...
    def health(self, db_server_status=False):
        ...
        if db_server_status:
            try:
                oi.sql_db.db_connect('postgres').cursor().close()
