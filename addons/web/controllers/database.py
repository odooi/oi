...
from oi import http
from oi.http import content_disposition, dispatch_rpc, request, Response
from oi.service import db
from oi.tools.misc import file_open, str2bool
from oi.tools.translate import _

from oi.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = oi.tools.config.verify_admin_password('admin')
        d['list_db'] = oi.tools.config['list_db']
        d['langs'] = oi.service.db.exp_list_lang()
        d['countries'] = oi.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = oi.service.db.list_db_incompatible(d['databases'])
        except oi.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = oi.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format='zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            oi.service.db.check_super(master_pwd)
            ...
            dump_stream = oi.service.db.dump_db(name, None, backup_format)
