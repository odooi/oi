...
try:
    ...
    from oi.tools._vendor.send_file import send_file

import oi
import oi.modules.registry
from oi import SUPERUSER_ID, _, http, api
from oi.addons.base.models.assetsbundle import ANY_UNIQUE
from oi.exceptions import AccessError, UserError
from oi.http import request, Response
from oi.tools import file_open, file_path, replace_exceptions, str2bool
from oi.tools.image import image_guess_size_from_field_name
from oi.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
Oi is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Oi is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if oi.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=oi.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or oi.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                _logger.warning("While retrieving the company logo, using the Oi logo instead", exc_info=True)
