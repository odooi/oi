...
function OiEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
OiEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
OiEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * Oi where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new OiEvent(this, name, info);
