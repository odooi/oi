...
(function (oi) {
    ...
    class ModuleLoader {
        /** @type {OiModuleLoader["bus"]} */
        bus = new EventTarget();
        /** @type {OiModuleLoader["checkErrorProm"]} */
        checkErrorProm = null;
        /** @type {OiModuleLoader["factories"]} */
        factories = new Map();
        /** @type {OiModuleLoader["failed"]} */
        failed = new Set();
        /** @type {OiModuleLoader["jobs"]} */
        jobs = new Set();
        /** @type {OiModuleLoader["modules"]} */
        modules = new Map();
        ...
        /** @type {OiModuleLoader["addJob"]} */
        addJob(name) {
            this.jobs.add(name);
            this.startModules();
        }

        /** @type {OiModuleLoader["define"]} */
        define(name, deps, factory, lazy = false) {
            ...
            this.factories.set(name, {
                deps,
                fn: factory,
                ignoreMissingDeps: globalThis.__oiIgnoreMissingDependencies,
            });
            if (!lazy) {
                this.addJob(name);
        ...
        /** @type {OiModuleLoader["findErrors"]} */
        ...
        /** @type {OiModuleLoader["reportErrors"]} */
        ...
        /** @type {OiModuleLoader["startModules"]} */
        startModules() {
            let job;
        ...
        /** @type {OiModuleLoader["startModule"]} */
        startModule(name) {
            /** @type {(dependency: string) => OiModule} */
    ...
    if (oi.debug && !new URLSearchParams(location.search).has("debug")) {
        // remove debug mode if not explicitely set in url
        oi.debug = "";
    }

    const loader = new ModuleLoader();
    oi.define = loader.define.bind(loader);
    oi.loader = loader;
})((globalThis.oi ||= {}));
