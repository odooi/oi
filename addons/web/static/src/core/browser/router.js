...
export function stateToUrl(state) {
    ...
    const start_url = isScopedApp() ? "scoped_app" : "oi";
 ...
 * external website). Unfortunately, Oi wasn't designed to be compatible with this cache, which
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/oi") &&
            (["/web", "/oi"].includes(url.pathname) || url.pathname.startsWith("/oi/")) &&
            ev.target.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/oi") && url.hash) {
