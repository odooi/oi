import { expect, getFixture, test } from "@oi/hoot";
import {
    contains,
    defineModels,
    fieldInput,
...
import { queryAllTexts, queryFirst } from "@oi/hoot-dom";
...
test("in form view", async () => {
    Contact._records = [{ id: 1, email: "john.doe@odoodooi.com" }];
    await mountView({
        type: "form",
        resModel: "contact",
        resId: 1,
    ...
    expect(`.o_field_email input[type="email"]`).toHaveValue("john.doe@odoodooi.com");
    expect(`.o_field_email a`).toHaveCount(1);
    expect(`.o_field_email a`).toHaveAttribute("href", "mailto:john.doe@odoodooi.com");
    expect(`.o_field_email a`).toHaveAttribute("target", "_blank");
    await fieldInput("email").edit("new@odoodooi.com");
    expect(`.o_field_email input[type="email"]`).toHaveValue("new@odoodooi.com");
});

test("in editable list view", async () => {
    Contact._records = [
        { id: 1, email: "john.doe@odoodooi.com" },
        { id: 2, email: "jane.doe@odoodooi.com" },
    ];
    await mountView({
        type: "list",
        resModel: "contact",
    ...
    expect(queryAllTexts(`tbody td:not(.o_list_record_selector) a`)).toEqual([
        "john.doe@odoodooi.com",
        "jane.doe@odoodooi.com",
    ]);
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:john.doe@odoodooi.com");
    ...
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:new@odoodooi.com");
});
