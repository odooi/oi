...
import { describe, dryRun, globals, start, stop } from "@oi/hoot";
import { Deferred } from "@oi/hoot-dom";
import { watchKeys, watchListeners } from "@oi/hoot-mock";
import { whenReady } from "@oi/owl";
...
class ModuleSetLoader extends loader.constructor {
    ...
    constructor(moduleSet) {
        ...
        oi.define = this.define.bind(this);
        oi.loader = this;
    }
...
const AUTO_INCLUDED_ADDONS = {
    /**
     * spreadsheet addons defines a module that does not starts with `@spreadsheet` but `@oi` (`@oi/o-spreadsheet)
     * To ensure that this module is loaded, we have to include `oi` in the dependencies
     */
    spreadsheet: ["oi"],
    /**
     * Add all view types by default
     */
    web_plus: ["web_gantt", "web_grid", "web_map"],
};
const CSRF_TOKEN = oi.csrf_token;
...
const R_DEFAULT_MODULE = /^@oi\/(owl|hoot)/;
