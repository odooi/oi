import { beforeEach, destroy, expect, test } from "@oi/hoot";
import { queryAll, queryAllAttributes, queryAllTexts, resize } from "@oi/hoot-dom";
import { advanceTime, animationFrame, runAllTimers } from "@oi/hoot-mock";
import {
    clearRegistry,
    contains,
    defineMenus,
...
import { Component, onRendered, xml } from "@oi/owl";
...
test("href attribute on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/oi/action-339");
});
...
test("href attribute with path on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/oi/my-path");
});
