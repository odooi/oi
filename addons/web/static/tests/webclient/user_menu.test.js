...
import { click, queryAllAttributes, queryAllProperties, queryAllTexts } from "@oi/hoot-dom";
...
import { oiAccountItem, preferencesItem } from "@web/webclient/user_menu/user_menu_items";
...
test("click on oi account item", async () => {
    patchWithCleanup(browser, {
        open: (url) => expect.step(`open ${url}`),
    });
    userMenuRegistry.add("oi_account", oiAccountItem);
    await mountWithCleanup(UserMenu);
    onRpc("/web/session/account", () => "https://account-url.com");
    stepAllNetworkCalls();
    await contains("button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveCount(1);
    expect(".o-dropdown--menu .dropdown-item").toHaveText("My Oi.com account");
