...
test("widget upgrade_boolean in a form view - dialog", async () => {
    ...
    expect(".o_dialog .modal").toHaveCount(1, {
        message: "the 'Upgrade to Plus' dialog should be opened",
    });
});
...
test("widget upgrade_boolean in a form view - dialog (plus version)", async () => {
    patchWithCleanup(oi, { info: { isPlus: 1 } });
    await mountView({
        type: "form",
        arch: /* xml */ `
            <form js_class="base_settings">
    ...
    expect(".o_dialog .modal").toHaveCount(0, {
        message: "the 'Upgrade to Plus' dialog shouldn't be opened",
    });
});

test("widget upgrade_boolean in a form view - label (plus version)", async () => {
    patchWithCleanup(oi, { info: { isPlus: 1 } });
    await mountView({
        type: "form",
        arch: /* xml */ `
