...
import { describe, test, expect } from "@oi/hoot";
...
import { click, freezeTime, queryAllTexts } from "@oi/hoot-dom";
import {
    ClientErrorDialog,
    Error504Dialog,
    ErrorDialog,
...
test("WarningDialog", async () => {
    ...
    await mountWithCleanup(WarningDialog, {
        env,
        props: {
            exceptionName: "oi.exceptions.UserError",
            message: "...",
...
test("RedirectWarningDialog", async () => {
    ...
    expect(".o_dialog").toHaveCount(1);
    expect("header .modal-title").toHaveText("Oi Warning");
    expect("main").toHaveText("Some strange unreadable message");
...
test("SessionExpiredDialog", async () => {
    ...
    expect(".o_dialog").toHaveCount(1);
    expect(".o_dialog").toHaveCount(1);
    expect("header .modal-title").toHaveText("Oi Session Expired");
    expect("main p").toHaveText(
        "Your Oi session expired. The current page is about to be refreshed."
    );
    expect(".o_dialog footer button").toHaveText("Close");
