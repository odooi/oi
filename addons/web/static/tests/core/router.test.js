import { describe, expect, getFixture, test } from "@oi/hoot";
import { click, on } from "@oi/hoot-dom";
import { tick } from "@oi/hoot-mock";
...
describe("stateToUrl", () => {
    test("encodes URI compatible strings", (assert) => {
        expect(stateToUrl({})).toBe("/oi");
        expect(stateToUrl({ a: "11", b: "summer wine" })).toBe("/oi?a=11&b=summer%20wine");
        expect(stateToUrl({ b: "2", c: "", e: "kloug,gloubi" })).toBe(
            "/oi?b=2&c=&e=kloug%2Cgloubi"
        );
    });
    ...
    test("backwards compatibility: no action stack, action encoded in path", (assert) => {
        expect(stateToUrl({})).toBe("/oi");
        // action
        expect(stateToUrl({ action: "some-path" })).toBe("/oi/some-path");
        expect(stateToUrl({ active_id: 5, action: "some-path" })).toBe("/oi/5/some-path");
        expect(stateToUrl({ active_id: "some-active_id", action: "some-path" })).toBe(
            "/oi/some-path?active_id=some-active_id",
            { message: "only numeric active_id are encoded in path" }
        );
        expect(stateToUrl({ action: "some-path", resId: 2 })).toBe("/oi/some-path/2");
        expect(stateToUrl({ action: "some-path", resId: "some-resId" })).toBe(
            "/oi/some-path?resId=some-resId",
        ...
        expect(stateToUrl({ action: 1, resId: 2 })).toBe("/oi/action-1/2", {
            message: "action id instead of path/tag",
        });
        expect(stateToUrl({ action: "module.xml_id", resId: 2 })).toBe(
            "/oi/action-module.xml_id/2",
            { message: "action xml_id instead of path/tag" }
        );
        // model
        expect(stateToUrl({ model: "some.model" })).toBe("/oi/some.model");
        expect(stateToUrl({ model: "some.model", resId: 2 })).toBe("/oi/some.model/2");
        expect(stateToUrl({ active_id: 5, model: "some.model" })).toBe("/oi/5/some.model");
        ...
        expect(stateToUrl({ active_id: 5, model: "some.model", view_type: "some_viewtype" })).toBe(
            "/oi/5/some.model?view_type=some_viewtype"
        );
        // edge cases
        expect(
            stateToUrl({ active_id: 5, action: "some-path", resId: 2, some_key: "some_value" })
        ).toBe("/oi/5/some-path/2?some_key=some_value", {
        ...
        ).toBe("/oi/5/some-path/2", { message: "action has priority on model" });
        expect(stateToUrl({ active_id: 5, model: "some.model", resId: 2, view_type: "list" })).toBe(
            "/oi/5/some.model/2?view_type=list",
    ...
    test("actionStack: one action", () => {
        expect(stateToUrl({ actionStack: [] })).toBe("/oi");
        // action
        expect(stateToUrl({ actionStack: [{ action: "some-path" }] })).toBe("/oi/some-path");
        ...
        ).toBe("/oi/5/some-path/new");
        expect(stateToUrl({ actionStack: [{ action: 1, resId: 2 }] })).toBe("/oi/action-1/2", {
        ...
        // model
        expect(stateToUrl({ actionStack: [{ model: "some.model" }] })).toBe("/oi/some.model");
    ...
    test("actionStack: multiple actions", () => {
        ...
        ).toBe("/oi/some-path/other-path");
        expect(
            stateToUrl({
                actionStack: [{ active_id: 5, action: "some-path" }, { action: "other-path" }],
            })
        ).toBe("/oi/5/some-path/other-path");
        expect(
            stateToUrl({
        ...
        ).toBe("/oi/5/some-path/2/other-path");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/oi/some-path/5/other-path/2");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/oi/some-path/5/other-path/new");
        expect(
            stateToUrl({
                actionStack: [
        ...
        ).toBe("/oi/some-path/5/other-path", {
            message:
        ...
        ).toBe("/oi/action-1/5/action-6/2", { message: "numerical actions" });
        expect(
            stateToUrl({
                actionStack: [
        ...
        ).toBe("/oi/action-module.xml_id/5/action-module.other_xml_id/2", {
        ...
        ).toBe("/oi/7/some-path/7/some-path", {
            message:
        ...
        ).toBe("/oi/5/some-path/2/some-path");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/oi/some-path/5/some-path/2", {
            message:
        ...
        ).toBe("/oi/some-path/5/some-path/new");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path", resId: 5 },
        ...
        ).toBe("/oi/5/some-path/5/some-path/2", {
        ...
        ).toBe("/oi/action-1/5/action-1/2", { message: "numerical actions" });
        expect(
            stateToUrl({
                actionStack: [
                    { action: "module.xml_id" },
        ...
        ).toBe("/oi/5/some.model/2/other.model");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/oi/some.model/5/other.model/2");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/oi/some.model/5/other.model/new");
        expect(
            stateToUrl({
                actionStack: [
        ...
        ).toBe("/oi/some.model/5/other.model", {
            message:
                "action with resId followed by action with same value as active_id is not duplicated",
        });
        expect(
        ...
        ).toBe("/oi/5/some-path/2/some.model");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/oi/some-path/5/some.model/2");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/oi/some-path/5/some.model/new");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path", resId: 5 },
        ...
        ).toBe("/oi/some-path/5/some.model", {
            message:
                "action with resId followed by action with same value as active_id is not duplicated",
        });
        expect(
        ...
        ).toBe("/oi/action-1/5/m-model_no_dot/2");
        expect(
            stateToUrl({
                actionStack: [
        ...
        ).toBe("/oi/5/some.model/2/other-path");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/oi/some.model/5/other-path/2");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/oi/some.model/5/other-path/new");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "some.model", resId: 5 },
        ...
        ).toBe("/oi/some.model/5/other-path", {
            message:
                "action with resId followed by action with same value as active_id is not duplicated",
        });
        expect(
        ...
        ).toBe("/oi/m-model_no_dot/5/action-1/2");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "model_no_dot", resId: 5 },
        ...
        ).toBe("/oi/m-model_no_dot/5/action-module.xml_id/2");
        ...
        ).toBe("/oi/some-path/5/other-path/2?some_key=some_value", {
            message:
                "pieces of state unrelated to actions are added as query string even with actionStack",
        });
        expect(
        ...
        ).toBe("/oi/some-path/5/other-path/2", { message: "action has priority on model" });
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path", resId: 5 },
        ...
        ).toBe("/oi/some-path/5/some.model/2?view_type=list", {
            message: "view_type and resId aren't incompatible",
        });
        expect(
            stateToUrl({
        ...
        ).toBe("/oi/some-path/2/5/other-path", {
            message:
        ...
        ).toBe("/oi/some.model/2/5/other.model", {
            message:
                "action with resId followed by action with different active_id gets both ids in a row",
        });
        expect(
        ...
        ).toBe("/oi/other-path/5/some-path/2", {
            message:
...
describe("urlToState", () => {
    test("deserialize queryString", () => {
        expect(_urlToState("/oi?a=11&g=summer%20wine")).toEqual({
            a: 11,
            g: "summer wine",
        });
        expect(_urlToState("/oi?g=2&c=&e=kloug%2Cgloubi")).toEqual({
            g: 2,
            c: "",
            e: "kloug,gloubi",
        });
    ...
    test("deserialize single action", () => {
        ...
        expect(_urlToState("/oi")).toEqual({});
        // action
        expect(_urlToState("/oi/some-path")).toEqual({
            action: "some-path",
            actionStack: [{ action: "some-path" }],
        });
        expect(_urlToState("/oi/5/some-path")).toEqual({
            active_id: 5,
            action: "some-path",
            actionStack: [{ active_id: 5, action: "some-path" }],
        });
        expect(_urlToState("/oi/some-path/2")).toEqual(
            {
                action: "some-path",
                resId: 2,
                actionStack: [{ action: "some-path" }, { action: "some-path", resId: 2 }],
        ...
        expect(_urlToState("/oi/some-path/new")).toEqual(
            {
                action: "some-path",
                resId: "new",
        ...
        expect(_urlToState("/oi/5/some-path/2")).toEqual({
            active_id: 5,
            action: "some-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/oi/action-1/2")).toEqual({
            action: 1,
            resId: 2,
            actionStack: [{ action: 1 }, { action: 1, resId: 2 }],
        });
        expect(_urlToState("/oi/action-module.xml_id/2")).toEqual({
            action: "module.xml_id",
            resId: 2,
            actionStack: [{ action: "module.xml_id" }, { action: "module.xml_id", resId: 2 }],
        });
        // model
        expect(_urlToState("/oi/some.model")).toEqual({
            model: "some.model",
            actionStack: [{ model: "some.model" }],
        });
        expect(_urlToState("/oi/some.model/2")).toEqual(
            {
                model: "some.model",
                resId: 2,
        ...
        expect(_urlToState("/oi/some.model/new")).toEqual(
            {
                model: "some.model",
                resId: "new",
        ...
        expect(_urlToState("/oi/5/some.model")).toEqual({
            active_id: 5,
            model: "some.model",
            actionStack: [{ active_id: 5, model: "some.model" }],
        });
        expect(_urlToState("/oi/5/some.model/2")).toEqual({
            active_id: 5,
            model: "some.model",
            resId: 2,
            actionStack: [{ active_id: 5, model: "some.model", resId: 2 }],
        });
        expect(_urlToState("/oi/5/some.model?view_type=some_viewtype")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                view_type: "some_viewtype",
        ...
        expect(_urlToState("/oi/m-model_no_dot/2")).toEqual({
            model: "model_no_dot",
            resId: 2,
            actionStack: [{ model: "model_no_dot", resId: 2 }],
        });
        // edge cases
        expect(_urlToState("/oi/5/some-path/2?some_key=some_value")).toEqual(
            {
                active_id: 5,
                action: "some-path",
                resId: 2,
        ...
        expect(_urlToState("/oi/5/some.model/2?view_type=list")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                resId: 2,
    ...
    test("deserialize multiple actions", () => {
        // action
        expect(_urlToState("/oi/some-path/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ action: "some-path" }, { action: "other-path" }],
        });
        expect(_urlToState("/oi/5/some-path/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ active_id: 5, action: "some-path" }, { action: "other-path" }],
        });
        expect(_urlToState("/oi/some-path/2/other-path")).toEqual({
            action: "other-path",
            active_id: 2,
            actionStack: [
                { action: "some-path" },
        ...
        expect(_urlToState("/oi/some-path/other-path/2")).toEqual({
            action: "other-path",
            resId: 2,
            actionStack: [
                { action: "some-path" },
        ...
        expect(_urlToState("/oi/5/some-path/2/other-path")).toEqual({
            action: "other-path",
            active_id: 2,
            actionStack: [
                { active_id: 5, action: "some-path" },
        ...
        expect(_urlToState("/oi/some-path/5/other-path/2")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/oi/some-path/5/other-path/new")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/oi/action-1/5/action-6/2")).toEqual({
            active_id: 5,
            action: 6,
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/oi/action-module.xml_id/5/action-module.other_xml_id/2")).toEqual({
            active_id: 5,
        ...
        // model
        expect(_urlToState("/oi/some.model/other.model")).toEqual(
            {
                model: "other.model",
        ...
        expect(_urlToState("/oi/5/some.model/other.model")).toEqual(
            {
                model: "other.model",
                actionStack: [{ model: "other.model" }],
            },
        ...
        expect(_urlToState("/oi/some.model/7/other.model")).toEqual({
            active_id: 7,
            model: "other.model",
            actionStack: [
        ...
        expect(_urlToState("/oi/some.model/other.model/2")).toEqual({
            model: "other.model",
            resId: 2,
            actionStack: [{ model: "other.model", resId: 2 }],
        });
        expect(_urlToState("/oi/5/some.model/2/other.model")).toEqual({
            active_id: 2,
            model: "other.model",
            actionStack: [
                { active_id: 5, model: "some.model", resId: 2 },
        ...
        expect(_urlToState("/oi/some.model/5/other.model/2")).toEqual({
            active_id: 5,
            model: "other.model",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/oi/some.model/5/other.model/new")).toEqual({
            active_id: 5,
            model: "other.model",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/oi/m-model_no_dot/5/m-no_dot_model/2")).toEqual({
            active_id: 5,
            model: "no_dot_model",
            resId: 2,
            actionStack: [
        ...
        // action + model
        expect(_urlToState("/oi/some-path/some.model")).toEqual({
            model: "some.model",
            actionStack: [{ action: "some-path" }, { model: "some.model" }],
        });
        expect(_urlToState("/oi/5/some-path/some.model")).toEqual({
            model: "some.model",
            actionStack: [{ active_id: 5, action: "some-path" }, { model: "some.model" }],
        });
        expect(_urlToState("/oi/some-path/7/some.model")).toEqual({
            active_id: 7,
            model: "some.model",
            actionStack: [
                { action: "some-path" },
        ...
        expect(_urlToState("/oi/some-path/some.model/2")).toEqual({
            model: "some.model",
            resId: 2,
            actionStack: [{ action: "some-path" }, { model: "some.model", resId: 2 }],
        });
        expect(_urlToState("/oi/5/some-path/2/some.model")).toEqual({
            active_id: 2,
            model: "some.model",
            actionStack: [
                { active_id: 5, action: "some-path" },
        ...
        expect(_urlToState("/oi/some-path/5/some.model/2")).toEqual({
            active_id: 5,
            model: "some.model",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/oi/some-path/5/some.model/new")).toEqual({
            active_id: 5,
            model: "some.model",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/oi/action-1/5/m-model_no_dot/2")).toEqual({
            active_id: 5,
            model: "model_no_dot",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/oi/action-module.xml_id/5/m-model_no_dot/2")).toEqual({
            active_id: 5,
            model: "model_no_dot",
            resId: 2,
            actionStack: [
        ...
        // model + action
        expect(_urlToState("/oi/some.model/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ action: "other-path" }],
        });
        expect(_urlToState("/oi/5/some.model/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ action: "other-path" }],
        });
        expect(_urlToState("/oi/some.model/2/other-path")).toEqual({
            active_id: 2,
            action: "other-path",
            actionStack: [
                { model: "some.model", resId: 2 },
        ...
        expect(_urlToState("/oi/some.model/other-path/2")).toEqual({
            action: "other-path",
            resId: 2,
            actionStack: [{ action: "other-path" }, { action: "other-path", resId: 2 }],
        });
        expect(_urlToState("/oi/5/some.model/2/other-path")).toEqual({
            active_id: 2,
            action: "other-path",
            actionStack: [
                { active_id: 5, model: "some.model", resId: 2 },
        ...
        expect(_urlToState("/oi/some.model/5/other-path/2")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/oi/some.model/5/other-path/new")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/oi/m-model_no_dot/5/action-1/2")).toEqual({
            active_id: 5,
            action: 1,
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/oi/m-model_no_dot/5/action-module.xml_id/2")).toEqual({
            active_id: 5,
            action: "module.xml_id",
            resId: 2,
            actionStack: [
        ...
        // edge cases
        expect(_urlToState("/oi/some-path/5/other-path/2?some_key=some_value")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/oi/some-path/5/some.model?view_type=list")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                actionStack: [
        ...
        expect(_urlToState("/oi/some-path/5/some.model/2?view_type=list")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                resId: 2,
        ...
        expect(_urlToState("/oi/some-path/2/5/other-path")).toEqual(
            {
                active_id: 5,
                action: "other-path",
                actionStack: [
        ...
        expect(_urlToState("/oi/some.model/2/5/other.model")).toEqual(
            {
                active_id: 5,
                model: "other.model",
                actionStack: [
...
describe("pushState", () => {
    ...
    test("can hide keys", async () => {
        ...
        await tick();
        expect(router.current).toEqual({ k1: 2, k2: 3 });
        expect(browser.location.href).toBe("https://www.hoot.test/oi?k2=3");
    });
    test("different order of keys shouldn't push a new state", async () => {
        ...
        expect(browser.location.href).toBe("https://www.hoot.test/oi?k1=2&z=1&a=2");
...
describe("History", () => {
    test("properly handles history.back and history.forward", async () => {
        ...
        router.pushState({ k1: 1 });
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/oi?k1=1");

        router.pushState({ k2: 2 });
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/oi?k1=1&k2=2");

        router.pushState({ k3: 3 }, { replace: true });
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/oi?k3=3");
        ...
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/oi?k4=3");
    ...
    test("unserialized parts of action stack are preserved when going back/forward", async () => {
        ...
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/oi/some-path");
        expect(router.current).toEqual({
            actionStack: [{ action: "some-path", displayName: "A cool display name" }],
        });
        router.pushState({
        ...
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/oi/other-path");
        expect(router.current).toEqual({
            actionStack: [{ action: "other-path", displayName: "A different display name" }],
        });
    ...
    test("properly handles history.back with hidden keys", async () => {
        ...
        expect(browser.location.href).toBe("https://www.hoot.test/oi?k2=2");
...
describe("Retrocompatibility", () => {
    test("parse an url with hash (key/values)", async () => {
        ...
        expect(browser.location.pathname).toBe("/oi");
    });
    ...
    test("parse an url with hash (anchor link) and query string", async () => {
        redirect("/oi?a=114&g=c.e&f=1#anchor");
        browser.location.hash = "#anchor";
...
describe("internal links", () => {
    ...
    test("click on internal link with different protocol does a loadState", async () => {
        ...
        const fixture = getFixture();
        const link = document.createElement("a");
        link.href = "http://" + browser.location.host + "/oi/some-action/2";
        fixture.appendChild(link);
    ...
    test("click on internal link with hash (anchor)", async () => {
        ...
        const fixture = getFixture();
        const link = document.createElement("a");
        link.href = "/oi/1/action-114/22#anchorId";
        fixture.appendChild(link);
        ...
        await tick();
        expect.verifySteps([
            "pushState: https://www.hoot.test/oi/1/action-114/22#anchorId",
            "click",
        ]);
        expect(router.current).toEqual({
            action: 114,
