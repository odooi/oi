...
from oi.addons.gamification.tests.common import TransactionCaseGamification
...
class TestGamificationCommon(TransactionCaseGamification):
    ...
    def setUp(self):
        ...
        self.robot = self.env['res.users'].with_context(no_reset_password=True).create({
            'name': 'R2D2',
            'login': 'r2d2@odoodooi.com',
            'email': 'r2d2@odoodooi.com',
...
class test_challenge(TestGamificationCommon):
    ...
    @mute_logger('oi.models.unlink', 'oi.addons.mail', 'oi.addons.auth_signup')
