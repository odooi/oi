...
export const presenceService = {
    start(env) {
        ...
        function onFocusChange(isFocused) {
            ...
            isOiFocused = isFocused;
            browser.localStorage.setItem(`${LOCAL_STORAGE_PREFIX}.focus`, isOiFocused);
            if (isOiFocused) {
                lastPresenceTime = luxon.DateTime.now().ts;
                env.bus.trigger("window_focus", isOiFocused);
            }
        }

        function onStorage({ key, newValue }) {
            if (key === `${LOCAL_STORAGE_PREFIX}.focus`) {
                isOiFocused = JSON.parse(newValue);
