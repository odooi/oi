...
from oi import fields, http
from oi.addons.base.tests.common import HttpCaseWithUserDemo, HttpCaseWithUserPortal
from oi.addons.mail.tests.common import mail_new_test_user
from oi.addons.website_event.tests.common import TestEventOnlineCommon, OnlineEventCase
...
class TestEventRegisterUTM(HttpCase, TestEventOnlineCommon):
    def test_event_registration_utm_values(self):
        ...
        self.opener.cookies.update({
            'oi_utm_campaign': event_campaign.name,
            'oi_utm_source': self.env.ref('utm.utm_source_newsletter').name,
            'oi_utm_medium': self.env.ref('utm.utm_medium_email').name
        })
