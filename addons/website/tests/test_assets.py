...
class TestWebsiteAssets(oi.tests.HttpCase):
    ...
    def test_01_multi_domain_assets_generation(self):
        ...
        self.url_open(domain_1 + '/oi')
        check_asset()
        self.url_open(domain_2 + '/oi')
        check_asset()
        self.url_open(domain_1 + '/oi')
        check_asset()
        self.url_open(domain_2 + '/oi')
...
class TestWebAssets(oi.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with oi.tools.mute_logger('oi.addons.web.controllers.binary'):
            self.assertEqual(
