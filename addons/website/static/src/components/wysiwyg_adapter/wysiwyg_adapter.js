...
import { isMediaElement } from '@web_editor/js/editor/oi-editor/src/utils/utils';
...
import { Component, onWillStart, useEffect, onWillUnmount } from "@oi/owl";
...
export class WysiwygAdapterComponent extends Wysiwyg {
    ...
    async startEdition() {
        ...
        const webEditorFilterMutationRecords = this.oiEditor.options.filterMutationRecords;
        Object.assign(this.oiEditor.options, {
            /**
             * @override
             */
        ...
        // Disable OiEditor observer's while setting up classes
        this.oiEditor.observerUnactive();
        ...
        // the dirty observer (not to be confused with oiEditor
        // observer) as the widgets might trigger DOM mutations.
        this._setObserver();
        this.oiEditor.observerActive();
    }
    _getBannerCommands() {
        return [];
    }
    ...
    async destroy() {
        ...
        const mod = await oi.loader.modules.get('@website/snippets/s_social_media/options')[Symbol.for('default')];
        mod.clearDbSocialValuesCache();

        const formOptionsMod = await oi.loader.modules.get('@website/snippets/s_website_form/options')[Symbol.for('default')];
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.oiEditor.filterMutationRecords(records);
        ...
        this.oiEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.oiEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
    ...
    _addEditorMessages() {
        ...
        for (let htmlEl of $wrap.not("[placeholder]").filter('[data-oe-sanitize="no_block"]')) {
            ...
            if (htmlEl.innerText.trim().length === 0) {
                ...
                // It is removed by OiEditor when text is entered.
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await oi.loader.modules.get('@website/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    async _onSaveRequest(event) {
        ...
        if (event.data.reload || event.data.reloadEditor) {
            ...
            callback = () => redirect(`/oi/action-website.website_preview?website_id=${encodeURIComponent(websiteId)}&path=${currentPath}&enable_editor=1`);
