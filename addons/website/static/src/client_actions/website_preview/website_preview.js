...
export class WebsitePreview extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            if (!session.website_bypass_domain_redirect // Used by the Oi support (bugs to be expected)
                    ...
                    // mysupercompany.odoodooi.com explicitly to enter their
                    // backend instead of mysupercompany.be.
                    // Bugs are to be expected while editing/using the website
                    // mysupercompany.be from mysupercompany.odoodooi.com though,
                ...
                this.dialogService.add(WebsiteDialog, {
                    ...
                    onClose: () => {
                        window.location.href = `${encodeURI(this.websiteDomain)}/oi/action-website.website_preview?path=${encodedPath}&website_id=${encodeURIComponent(this.websiteId)}`;
                    }
                });
            } else {
        ...
        onMounted(() => {
            ...
            // OiFrameContentLoaded event to unblock the iframe, as it is
            // triggered faster than the load event.
            this.iframe.el.addEventListener('OiFrameContentLoaded', () => this.websiteService.unblockPreview('load-iframe'), { once: true });
        });
        ...
        /**
         * This removes the 'Oi' prefix of the title service to display
        ...
        useEffect(() => {
            this.iframe.el.addEventListener('OiFrameContentLoaded', toggleIsMobile);
            return () => this.iframe.el.removeEventListener('OiFrameContentLoaded', toggleIsMobile);
        }, () => []);
    }
    ...
    reloadIframe(url) {
        return new Promise((resolve, reject) => {
            this.websiteService.websiteRootInstance = undefined;
            this.iframe.el.addEventListener('OiFrameContentLoaded', resolve, { once: true });
            if (url) {
    ...
    _isTopWindowURL({ host, pathname }) {
        const backendRoutes = ['/web', '/web/session/logout', '/oi'];
    ...
    /**
     * This replaces the browser url (/oi/action-website...) with
    ...
    _replaceBrowserUrl() {
        if (!wUtils.isHTTPSorNakedDomainRedirection(this.iframe.el.contentWindow.location.origin, window.location.origin)) {
            ...
            // trigger a traceback.
            history.replaceState(history.state, document.title, '/oi');
            return;
        }
    ...
    _onKeydownRefresh(ev) {
        ...
        redirect(`/oi/action-website.website_preview?path=${encodeURIComponent(path)}${debugMode}`);
    }
}
