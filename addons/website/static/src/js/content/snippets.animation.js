...
registry.slider = publicWidget.Widget.extend({
    ...
    start: function () {
        ...
        if (this.editableMode && this.el.matches("section > .carousel")
            ...
            this.options.wysiwyg.oiEditor.observerUnactive("disable_controls");
            ...
            this.options.wysiwyg.oiEditor.observerActive("disable_controls");
    ...
    destroy: function () {
        ...
        this.options.wysiwyg && this.options.wysiwyg.oiEditor.observerUnactive("destroy");
        this.$(".carousel-item")
            .toArray()
            .forEach((el) => {
                $(el).css("min-height", "");
            });
        this.options.wysiwyg && this.options.wysiwyg.oiEditor.observerActive("destroy");
        ...
        if (this.editableMode && this.el.matches("section > .carousel")
            ...
            const indicatorEls = this.el.querySelectorAll(".carousel-indicators > *");
            this.options.wysiwyg.oiEditor.observerUnactive("restore_controls");
            ...
            this.options.wysiwyg.oiEditor.observerActive("restore_controls");
            [...this.controlEls, ...indicatorEls].forEach(controlEl => {
    ...
    _computeHeights: function () {
        ...
        this.options.wysiwyg && this.options.wysiwyg.oiEditor.observerUnactive("_computeHeights");
        ...
        this.options.wysiwyg && this.options.wysiwyg.oiEditor.observerActive("_computeHeights");
    },
...
/**
 * This class is used to fix carousel auto-slide behavior in Oi 17.4 and up.
...
registry.Parallax = Animation.extend({
    ...
    _updateBgCss(cssValues) {
        ...
        if (this.options.wysiwyg) {
            this.options.wysiwyg.oiEditor.observerUnactive('_updateBgCss');
        }
        this.$bg.css(cssValues);
        if (this.options.wysiwyg) {
            this.options.wysiwyg.oiEditor.observerActive('_updateBgCss');
        }
    },
...
registry.mediaVideo = publicWidget.Widget.extend(
    ...
    start: function () {
        ...
        // integrated with Oi (this refactoring should be done in master).

        const proms = [this._super.apply(this, arguments)];
        let iframeEl = this.el.querySelector(':scope > iframe');

        // The following code is only there to ensure compatibility with
        // videos added before bug fixes or new Oi versions where the
...
registry.ImagesLazyLoading = publicWidget.Widget.extend({
    ...
    _updateImgMinHeight(imgEl, reset = false) {
        if (this.options.wysiwyg) {
            this.options.wysiwyg.oiEditor.observerUnactive('_updateImgMinHeight');
        }
        if (reset) {
        ...
        if (this.options.wysiwyg) {
            this.options.wysiwyg.oiEditor.observerActive('_updateImgMinHeight');
        }
    },
});
...
￼    */
    _setImgSrc(svg, resolve) {
        ...
        preloadedImg.onload = () => {
            ...
            this.options.wysiwyg && this.options.wysiwyg.oiEditor.observerUnactive("setImgHoverEffectSrc");
            ...
            this.options.wysiwyg && this.options.wysiwyg.oiEditor.observerActive("setImgHoverEffectSrc");
