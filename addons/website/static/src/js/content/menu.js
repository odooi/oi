...
const BaseAnimatedHeader = animations.Animation.extend({
    ...
    start: function () {
        ...
        // transitionstart, so we listen to a custom oi event.
        this._transitionCount = 0;
        this.$el.on('oi-transitionstart.BaseAnimatedHeader', () => {
    ...
    _adaptToHeaderChange: function () {
        this.options.wysiwyg && this.options.wysiwyg.oiEditor.observerUnactive();
        ...
        this.options.wysiwyg && this.options.wysiwyg.oiEditor.observerActive();
    },
    /**
     * @private
    ...
    _updateHeaderOnScroll: function (scroll) {
        ...
        if (this.headerIsScrolled !== headerIsScrolled) {
            ...
            if (!this.scrollHeightTooShort) {
                ...
                this.$el.trigger('oi-transitionstart');
...
publicWidget.registry.StandardAffixedHeader = BaseAnimatedHeader.extend({
     ...
     */
    destroy() {
        this.options.wysiwyg?.oiEditor.observerUnactive("destroyStandardHeader");
        this.$el.css('transform', '');
        this.el.classList.remove("o_transformed_not_affixed");
        this.options.wysiwyg?.oiEditor.observerActive("destroyStandardHeader");
        this._super(...arguments);
    },
    ...
    _updateHeaderOnScroll: function (scroll) {
        ...
        this.options.wysiwyg?.oiEditor.observerUnactive("updateHeaderOnScroll");
        ...
        this.options.wysiwyg?.oiEditor.observerActive("updateHeaderOnScroll");
    },
});
...
publicWidget.registry.MegaMenuDropdown = publicWidget.Widget.extend({
    ...
    _moveMegaMenu(megaMenuToggleEl) {
        ...
        this.options.wysiwyg?.oiEditor.observerUnactive("moveMegaMenu");
        ...
        this.options.wysiwyg?.oiEditor.observerActive("moveMegaMenu");
    },
...
publicWidget.registry.HeaderGeneral = publicWidget.Widget.extend({
     ...
     */
    _onCollapseShow() {
        this.options.wysiwyg?.oiEditor.observerUnactive("addCollapseClass");
        this.el.classList.add('o_top_menu_collapse_shown');
        this.options.wysiwyg?.oiEditor.observerActive("addCollapseClass");
    },
    /**
     * @private
     */
    _onCollapseHidden() {
        this.options.wysiwyg?.oiEditor.observerUnactive("removeCollapseClass");
        ...
        this.options.wysiwyg?.oiEditor.observerActive("removeCollapseClass");
    },
});
