...
import * as OiEditorLib from "@web_editor/js/editor/oi-editor/src/utils/utils";
import { Component, onMounted, onWillStart, useEffect, useRef, useState } from "@oi/owl";
...
const getTraversedNodes = OiEditorLib.getTraversedNodes;
...
export class WebsiteSnippetsMenu extends weSnippetEditor.SnippetsMenu {
    ...
    _getDragAndDropOptions(options = {}) {
        // TODO: This is currently not in use by Oi's D&D
        // There is currently no way in Oi D&D to offset the edge scrolling.
     ...
     */
    _getSelection() {
        return this.options.wysiwyg.oiEditor.document.getSelection();
    }
    /**
     * @override
     */
    _addToolbar() {
        ...
        this.options.wysiwyg.oiEditor.computeFontSizeSelectorValues();
    }
    /**
    * @override
    */
    ...
    _handleTextOptions(classSelector, optionClassList, textOptionsPostActivate = () => {}) {
        ...
        if (selectedTextEl) {
            const restoreCursor = OiEditorLib.preserveCursor(this.$body[0].ownerDocument);
            ...
            this.options.wysiwyg.oiEditor.historyResetLatestComputedSelection();
