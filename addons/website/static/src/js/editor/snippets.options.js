...
import { Component, markup, useEffect, useRef, useState } from "@oi/owl";
...
options.registry.OptionsTab = options.registry.WebsiteLevelColor.extend({
    ...
    async updateUI() {
        ...
        // If the gray palette has been generated by Oi standard option,
        ...
        // same hue by the Oi editor, there might be rounding errors
                ...
                this.trigger_up('menu_dialog', {
                    ...
                    save: (name, url) => {
                        ...
                        return this.orm.call(
                            ...
                            widget.wysiwyg.oiEditor.observerUnactive();
                            widget.$target.attr('href', url);
                            $menu.text(name);
                            widget.wysiwyg.oiEditor.observerActive();
                        });
                    },
                });
                widget.popover.hide();
...
options.registry.DeviceVisibility = options.Class.extend({
     ...
     */
    async onTargetHide() {
        this.options.wysiwyg.oiEditor.observerUnactive("onTargetHide");
        this.$target[0].classList.remove('o_snippet_override_invisible');
        this.options.wysiwyg.oiEditor.observerActive("onTargetHide");
    },
    /**
     * @override
     */
    async onTargetShow() {
        ...
        if ((this.$target[0].classList.contains('o_snippet_mobile_invisible')
            ...
            this.options.wysiwyg.oiEditor.observerUnactive("onTargetShow");
            this.$target[0].classList.add('o_snippet_override_invisible');
            this.options.wysiwyg.oiEditor.observerActive("onTargetShow");
        }
    },
    /**
     * @override
...
options.registry.TextHighlight = options.Class.extend({
    ...
    */
    ...
    _autoAdaptHighlights() {
        this.trigger_up("snippet_edition_request", { exec: async () =>
            await this._refreshPublicWidgets($(this.options.wysiwyg.oiEditor.editable))
        });
    },
