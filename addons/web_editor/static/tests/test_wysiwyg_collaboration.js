...
import {
    ...
    patchEditorIframe,
} from '@web_editor/js/editor/oi-editor/test/utils';
...
class PeerTest {
    ...
    async startEditor() {
        ...
        if (this.initialParsedSelection) {
            ...
            this.wysiwyg.oiEditor._recordHistorySelection();
        } else {
    ...
    async getValue() {
        this.wysiwyg.oiEditor.observerUnactive('PeerTest.getValue');
        renderTextualSelection(this.wysiwyg.oiEditor);
        ...
        this.wysiwyg.oiEditor.observerActive('PeerTest.getValue');
...
export async function createPeers(peers) {
    ...
    for (const peerId of peers) {
        ...
        patch(wysiwyg, {
            ...
            // Hacky hook as we know this method is called after setting the value in the wysiwyg start and before sending the value to oiEditor.
    ...
    QUnit.module('Collaboration', {}, () => {
        ...
        QUnit.module('Focus', {}, () => {
            QUnit.test('Focused client should not receive step if no data channel is open', async (assert) => {
                ...
                await peers.p1.wysiwyg.oiEditor.execCommand('insert', 'b');
        ...
        QUnit.module('Stale detection & recovery', {}, () => {
            QUnit.module('detect stale while unfocused', async () => {
                QUnit.test('should do nothing until focus', async (assert) => {
                    ...
                    await peers.p1.wysiwyg.oiEditor.execCommand('insert', 'c');
        ...
        QUnit.module('Disconnect & reconnect', {}, () => {
            QUnit.test('should sync history when disconnecting and reconnecting to internet', async (assert) => {
                ...
                const setSelection = peer => {
                    ...
                    const pElement = peer.wysiwyg.oiEditor.editable.querySelector('p')
                ...
                const addP = (peer, content) => {
                    ...
                    peer.wysiwyg.oiEditor.editable.append(p);
                    peer.wysiwyg.oiEditor.historyStep();
                }
                ...
                await peers.p2.wysiwyg.oiEditor.execCommand('insert', 'e');
                addP(peers.p2, 'f');
