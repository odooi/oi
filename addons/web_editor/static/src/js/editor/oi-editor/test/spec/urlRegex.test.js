...
import { URL_REGEX } from '../../src/OiEditor.js';
...
describe('urlRegex', () => {
    ...
    it('should match 1234-abc.runbot007.odoodooi.com/web#id=3&menu_id=221', () => {
        const url = '1234-abc.runbot007.odoodooi.com/web#id=3&menu_id=221';
        const text = `abc ${url} abc`;
        const match = text.match(URL_REGEX);
        chai.expect(match[0]).to.be.equal(url);
    });
    it('should match https://1234-abc.runbot007.odoodooi.com/web#id=3&menu_id=221', () => {
        const url = 'https://1234-abc.runbot007.odoodooi.com/web#id=3&menu_id=221';
        const text = `abc ${url} abc`;
        const match = text.match(URL_REGEX);
        chai.expect(match[0]).to.be.equal(url);
    });
