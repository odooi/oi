...
import { closestElement, isUnremovable } from "@web_editor/js/editor/oi-editor/src/utils/utils";
...
import { Tooltip as OiTooltip } from "@web/core/tooltip/tooltip";
...
var SnippetEditor = publicWidget.Widget.extend({
    ...
    clone: async function (recordUndo) {
        ...
        if (recordUndo) {
            this.options.wysiwyg.oiEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.trigger_up('call_for_each_child_snippet', {
                $snippet: $clone,
    ...
    _outPreviousDropzone(self, currentDropzoneEl) {
        ...
        if (rowEl.classList.contains('o_grid_mode')) {
            ...
            self.options.wysiwyg.oiEditor.observerActive('dragAndDropMoveSnippet');
            gridUtils._resizeGrid(rowEl);
            self.options.wysiwyg.oiEditor.observerUnactive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStart({ helper, addStyle }) {
        this.options.wysiwyg.oiEditor.observerUnactive('dragAndDropMoveSnippet');
        this.trigger_up('drag_and_drop_start');
        this.options.wysiwyg.oiEditor.automaticStepUnactive();
        var self = this;
        this.dragState = {};
        ...
        if (rowEl.classList.contains('row') && this.options.isWebsite) {
            if (allowGridMode) {
                ...
                if (!rowEl.classList.contains('o_grid_mode')) {
                    this.options.wysiwyg.oiEditor.observerActive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStop({ x, y }) {
        this.options.wysiwyg.oiEditor.automaticStepActive();
        this.options.wysiwyg.oiEditor.automaticStepSkipStack();
        this.options.wysiwyg.oiEditor.unbreakableStepUnactive();
...
class SnippetsMenu extends Component {
    ...
    setup() {
        ...
        // Oi Editor uses the HTML Element to bind commands.
        ...
        onMounted(async () => {
            ...
            this.options.wysiwyg.oiEditor.bindExecCommand(titleButtons);

            // Get table container and bind commands to Oi Editor.
            const customizeTableBlock = this.customizePanel.querySelector('#o-we-editor-table-container');
            this.options.wysiwyg.oiEditor.bindExecCommand(customizeTableBlock);
    ...
    async start() {
        ...
        $(this.options.wysiwyg.oiEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.oiEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.oiEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            const updateHistoryButtons = () => {
                this.state.canRedo = this.options.wysiwyg.oiEditor.historyCanRedo();
                this.state.canUndo = this.options.wysiwyg.oiEditor.historyCanUndo();
            };
            this.options.wysiwyg.oiEditor.addEventListener('historyStep', updateHistoryButtons);
            this.options.wysiwyg.oiEditor.addEventListener('observerApply', () => {
                $(this.options.wysiwyg.oiEditor.editable).trigger('content_changed');
            });
    ...
    _makeSnippetDraggable() {
        ...
        const dragAndDropOptions = this.options.getDragAndDropOptions({
            ...
            onDragStart: ({ element }) => {
                ...
                const doc = this.options.wysiwyg.oiEditor.document;
                $(doc.body).addClass('oe_dropzone_active');

                this.options.wysiwyg.oiEditor.automaticStepUnactive();

                this.$el.find('.oe_snippet_thumbnail').addClass('o_we_ongoing_insertion');
                this.options.wysiwyg.oiEditor.observerUnactive('dragAndDropCreateSnippet');
            ...
            onDragEnd: async ({ x, y, helper }) => {
                ...
                this.options.wysiwyg.oiEditor.observerActive('dragAndDropCreateSnippet');
    ...
    _onClick(ev) {
        ...
        if ($oeStructure.length && !$oeStructure.children().length && this.snippets.size > 0) {
            ...
            this._activateSnippet(false).then(() => {
                this.$el.find('.oe_snippet').oiBounce();
            });
            return;
        }
        this._activateSnippet($target);
   ...
   _onSnippetDragAndDropStart() {
    ...
    _showSnippetTooltip(snippetEl, delay = 1500) {
        ...
        this.hideShownTooltip = this.popover.add(snippetEl, OiTooltip, {
    ...
    _onSaveRequest(ev) {
        ...
        // If it's an OiEvent sent by sub-widgets, we prevent the event
        ...
        if (!ev.currentTarget.matches(".o_disabled") && ev.currentTarget.dataset.snippetGroup) {
            ...
            for (const el of $els) {
                if (!el.children.length) {
                    $(el).oiBounce('o_we_snippet_area_animation');
                }
            }
        }
    }
    ...
    _checkEditorToolbarVisibility(e) {
        ...
        const selection = this.options.wysiwyg.oiEditor.document.getSelection();
    ...
    _onRequestEditable(ev) {
        ev.data.callback($(this.options.wysiwyg.oiEditor.editable));
    }
    /**
    ...
    _installModule(moduleID, moduleDisplayName) {
        ...
        const linkUrl = '/oi/action-base.open_module_tree/' + encodeURIComponent(moduleID);
