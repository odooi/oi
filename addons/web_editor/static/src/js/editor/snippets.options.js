...
const resetOuids = OiEditorLib.resetOuids;
let _serviceCache = {
    orm: {},
    rpc: {},
};
...
const SnippetOptionWidget = publicWidget.Widget.extend({
    ...
    _select: async function (previewMode, widget) {
        ...
        if (previewMode === true) {
            this.options.wysiwyg.oiEditor.automaticStepUnactive('preview_option');
        }
        ...
        if (previewMode === 'reset' || previewMode === false) {
            this.options.wysiwyg.oiEditor.automaticStepActive('preview_option');
        }
    ...
    _onUserValueUpdate: async function (ev) {
        ...
        if (shouldRecordUndo) {
            this.options.wysiwyg.oiEditor.unbreakableStepUnactive();
        }
        ...
        this.trigger_up('snippet_edition_request', {exec: async () => {
            ...
            if (shouldRecordUndo) {
                this.options.wysiwyg.oiEditor.historyStep();
            }
    ...
    start: function () {
        ...
        this.$handles.on('mousedown', function (ev) {
            ...
            if (rowEl.classList.contains("o_grid_mode") && !isMobile) {
                self.options.wysiwyg.oiEditor.observerUnactive('displayBackgroundGrid');
                backgroundGridEl = gridUtils._addBackgroundGrid(rowEl, 0);
                gridUtils._setElementToMaxZindex(backgroundGridEl, rowEl);
                self.options.wysiwyg.oiEditor.observerActive('displayBackgroundGrid');
            }
            ...
            self.options.wysiwyg.oiEditor.automaticStepUnactive('resizing');
            ...
            const iframeWindowMouseUp = function (ev) {
                ...
                self.options.wysiwyg.oiEditor.automaticStepActive('resizing');
                ...
                setTimeout(function () {
                    self.options.wysiwyg.oiEditor.historyStep();
...
registry.layout_column = SnippetOptionWidget.extend(ColumnLayoutMixin, {
    ...
    async addElement(previewMode, widgetValue, params) {
        ...
        if (elementType === 'image') {
            ...
            if (!isImageSaved) {
                ...
                // media dialog closed.
                this.options.wysiwyg.oiEditor.historyRevertCurrentStep();
                return;
            }
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["row-gap", "column-gap"].includes(params.cssProperty) && !isMobileView) {
            ...
            this.options.wysiwyg.oiEditor.observerUnactive("addGridPreview");
            ...
            this.options.wysiwyg.oiEditor.observerActive("addGridPreview");
     ...
     */
    _removeGridPreview() {
        this.options.wysiwyg.oiEditor.observerUnactive("removeGridPreview");
        ...
        this.options.wysiwyg.oiEditor.observerActive("removeGridPreview");
    },
    /**
     * @returns {boolean}
     */
...
registry.GridColumns = SnippetOptionWidget.extend({
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["--grid-item-padding-y", "--grid-item-padding-x"].includes(params.cssProperty)) {
            ...
            this.options.wysiwyg.oiEditor.observerUnactive("addPaddingPreview");
            this.$target[0].classList.add("o_we_padding_highlight");
            this.options.wysiwyg.oiEditor.observerActive("addPaddingPreview");
    ...
    _removePaddingPreview() {
        this.options.wysiwyg.oiEditor.observerUnactive("removePaddingPreview");
        ...
        this.options.wysiwyg.oiEditor.observerActive("removePaddingPreview");
    },
});
...
registry.ReplaceMedia = SnippetOptionWidget.extend({
     ...
     */
    onFocus() {
        this.options.wysiwyg.oiEditor.addEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.oiEditor.addEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
     ...
     */
    _clearListeners() {
        this.options.wysiwyg.oiEditor.removeEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.oiEditor.removeEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
    },
    /**
     * @private
     */
...
registry.ImageTools = ImageHandlerOption.extend({
    ...
    async removeStretch(previewMode, widgetValue, params) {
        this.options.wysiwyg.oiEditor.historyPauseSteps();
        ...
        this.options.wysiwyg.oiEditor.historyUnpauseSteps();
    },
    /**
...
￼    */
    ...
    async _writeShape(svgText) {
        ...
        if (hasHoverEffect) {
            ...
            clonedImgEl = img.cloneNode(true);
            this.options.wysiwyg.oiEditor.observerUnactive("addClonedImgForHoverEffectPreview");
            img.classList.add("d-none");
            img.insertAdjacentElement("afterend", clonedImgEl);
            this.options.wysiwyg.oiEditor.observerActive("addClonedImgForHoverEffectPreview");
        }
        const loadedImg = await loadImage(dataURL, img);
        if (hasHoverEffect) {
            this.options.wysiwyg.oiEditor.observerUnactive("removeClonedImgForHoverEffectPreview");
            clonedImgEl.remove();
            img.classList.remove("d-none");
            this.options.wysiwyg.oiEditor.observerActive("removeClonedImgForHoverEffectPreview");
        }
    ...
    _computeMaxDisplayWidth() {
        ...
        // width since we only use col-lg-* in Oi).
...
registry.BackgroundImage = SnippetOptionWidget.extend({
    ...
    _setBackground(backgroundURL) {
        ...
        this.options.wysiwyg.oiEditor.editable.focus();
    },
});
...
registry.ColoredLevelBackground = registry.BackgroundToggler.extend({
    ...
    _markColorLevel: function () {
        this.options.wysiwyg.oiEditor.observerUnactive('_markColorLevel');
        this.$target.addClass('o_colored_level');
        this.options.wysiwyg.oiEditor.observerActive('_markColorLevel');
    },
});
