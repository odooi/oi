...
import * as OiEditorLib from "@web_editor/js/editor/oi-editor/src/OiEditor";
...
import { isIconElement, isSelectionInSelectors, peek } from '@web_editor/js/editor/oi-editor/src/utils/utils';
...
const OiEditor = OiEditorLib.OiEditor;
const getDeepRange = OiEditorLib.getDeepRange;
const getInSelection = OiEditorLib.getInSelection;
const isProtected = OiEditorLib.isProtected;
const rgbToHex = OiEditorLib.rgbToHex;
const preserveCursor = OiEditorLib.preserveCursor;
const closestElement = OiEditorLib.closestElement;
const setSelection = OiEditorLib.setSelection;
const endPos = OiEditorLib.endPos;
const hasValidSelection = OiEditorLib.hasValidSelection;
const parseHTML = OiEditorLib.parseHTML;
const closestBlock = OiEditorLib.closestBlock;
const getRangePosition = OiEditorLib.getRangePosition;
const childNodeIndex = OiEditorLib.childNodeIndex;
const fillEmpty = OiEditorLib.fillEmpty;
const isVisible = OiEditorLib.isVisible;
const getSelectedNodes = OiEditorLib.getSelectedNodes;
const getDeepestPosition = OiEditorLib.getDeepestPosition;
const paragraphRelatedElements = OiEditorLib.paragraphRelatedElements;
...
export class Wysiwyg extends Component {
    ...
    setup() {
        ...
        const getColorPickedHandler = (colorType) => {
            return (params) => {
                if (this.hadNonCollapsedSelectionBeforeColorpicker) {
                    this.oiEditor.historyResetLatestComputedSelection(true);
                }
                // Unstash the mutations now that the color is picked.
                this.oiEditor.historyUnstash();
                ...
                // hides it).
                if (hasValidSelection(this.oiEditor.editable)) {
                    this.oiEditor.deselectTable();
                }
        ...
        const getColorHoverHandler = (colorType) => {
            return (props) => {
                ...
                this.oiEditor.historyPauseSteps();
                try {
                    this._processAndApplyColor(colorType, props.color, true);
                    this.oiEditor._computeHistorySelection();
                } finally {
                    this.oiEditor.historyUnpauseSteps();
                }
            }
        };
        ...
        const colorPaletteCommonOptions = {
            ...
            onColorLeave: () => {
                ...
                this.oiEditor.withoutRollback(() => this.oiEditor.historyRevertCurrentStep());
    ...
    async startEdition() {
        ...
        this.oiEditor = new OiEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.oiEditor.addEventListener('contentChanged', function () {
        ...
        if (this.oiEditor.document !== document) {
            this.oiEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.oiEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.oiEditor.document.addEventListener("keyup", this._signalOnline, true);
        }
        ...
        if ($('wrapwrap').length) {
            this.multiSelectionTarget = $().getScrollingTarget(this.oiEditor.document);
            this.multiSelectionTarget.addEventListener('scroll', this.oiEditor.multiselectionRefresh, { passive: true });
        }
        ...
        this._observeOiFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.oiEditor.observerUnactive();
                const $field = $(this);
        ...
        this.$editable.on('dblclick', mediaSelector, ev => {
            ...
            if (isEditable) {
                ...
                if (!isProtected(this.oiEditor.document.getSelection().anchorNode)) {
        ...
        if (options.snippets) {
            ...
            $(this.oiEditor.document.body).addClass('editor_enable');
        ...
        if (this.options.getContentEditableAreas) {
            $(this.options.getContentEditableAreas(this.oiEditor)).find('*').off('mousedown mouseup click');
        }
        ...
        $(this.oiEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.oiEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.oiEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.oiEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        if (!this.state.showSnippetsMenu) {
            this.setCSSVariables(this.toolbarEl);
        }

        this.oiEditor.addEventListener('preObserverActive', () => {
            ...
            this._onPostSanitize(this.oiEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.oiEditor.isMobile) {
                this.oiEditor.editable.before(this.toolbarEl);
            } else {
    ...
    setupToolbar(toolbarEl) {
        ...
        this.oiEditor.setupToolbar(toolbarEl);
    ...
    destroy() {
        ...
        if (this.oiEditor) {
            this.oiEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.oiEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.oiEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.oiEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            this.oiEditor.destroy();
        }
        ...
        this.multiSelectionTarget?.removeEventListener('scroll', this.oiEditor.multiselectionRefresh, { passive: true });
    ...
    _renderElement() {
        ...
        if (this.options.resizable && !isMobileOS()) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper oi-editor">');
    ...
    getValue(options) {
        ...
        if (this.oiEditor) {
            this.oiEditor.cleanForSave($editable[0]);
    ...
    async saveContent(reload = true, editable = false) {
        ...
        if (!editable) {
            ...
            const editables = "getContentEditableAreas" in this.options ? this.options.getContentEditableAreas(this.oiEditor) : [];
     ...
     */
    setValue(value) {
        this.oiEditor.resetContent(value);
    }
    /**
     * Undo one step of change in the editor.
     */
    undo() {
        this.oiEditor.historyUndo();
    }
    /**
     * Redo one step of change in the editor.
     */
    redo() {
        this.oiEditor.historyRedo();
    }
    /**
     ...
     */
    focus() {
        if (this.oiEditor && !this.oiEditor.historyResetLatestComputedSelection(true)) {
            ...
            const selection = this.oiEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    }
    getDeepRange() {
        return getDeepRange(this.oiEditor.editable);
    }
    closestElement(...args) {
        return closestElement(...args);
    }
    async cleanForSave(editable = this.oiEditor.editable) {
        if (this.oiEditor) {
            this.oiEditor.cleanForSave(editable);
    ...
    isSelectionInEditable() {
        return this.oiEditor.isSelectionInEditable();
    }
    /**
     * Start or resume the Oi field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeOiFieldChanges() {
        ...
        if (this.oiFieldObservers) {
            for (let observerData of this.oiFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const oiFieldSelector = '[data-oe-model], [data-oe-translation-source-sha]';
            const $oiFields = this.$editable.find(oiFieldSelector);
            const renderingClassesSelector = this.oiEditor.options.renderingClasses
                .map(className => `.${className}`).join(", ");
            this.oiFieldObservers = [];

            $oiFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.oiEditor.filterMutationRecords(mutations);
                    ...
                    let $nodes = $oiFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    //  `_pauseOiFieldObservers(field)` call.
                    this.__oiFieldObserversToPause = this.oiFieldObservers.filter(
                    ...
                    for (const node of fieldNodeClone.querySelectorAll(renderingClassesSelector)) {
                        node.classList.remove(...this.oiEditor.options.renderingClasses);
                    }
                    const html = $(fieldNodeClone).html();
                    this.oiEditor.withoutRollback(() => {
                ...
                this.oiFieldObservers.push({field: field, observer: observer});
            });
        }
    }
    /**
     ...
     */
    _pauseOiFieldObservers() {
        // TODO adapt in master: remove this and directly exclude observers with
        // targets inside the current field (we use `this.oiFieldObservers`
        // as fallback for compatibility here).
        const fieldObserversData = this.__oiFieldObserversToPause || this.oiFieldObservers;
    ...
    openLinkToolsFromSelection() {
        ...
        for (const el of getSelectedNodes(this.oiEditor.editable)) {
            if (
        ...
        if (targetEl.nodeType === Node.ELEMENT_NODE
            ...
            this.oiEditor.dispatchEvent(new Event('activate_image_link_tool'));
            return;
        }
        this.toggleLinkTools();
    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        const linkEl = getInSelection(this.oiEditor.document, 'a');
        ...
        if (this.state.showSnippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                if (!this.state.linkToolProps || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkToolsInfos.link)) {
                    const { link } = getOrCreateLink({
                        containerNode: this.oiEditor.editable,
                    ...
                    const addHintClasses = () => {
                        this.oiEditor.observerUnactive("hint_classes");
                        link.classList.add('oe_edited_link');
                        $button.addClass('active');
                        this.oiEditor.observerActive("hint_classes");
                    };
                    ...
                    this.state.linkToolProps = {
                        ...
                        wysiwyg: this,
                        editable: this.oiEditor.editable,
                        link,
                ...
                this.oiEditor.document.removeEventListener('click', this._onClick, true);
                ...
                this.oiEditor.document.addEventListener('click', this._onClick, true);
                document.addEventListener('click', this._onClick, true);
            } else {
                this.destroyLinkTools();
            }
        } else {
            const historyStepIndex = this.oiEditor.historySize() - 1;
            this.oiEditor.historyPauseSteps();
            ...
            this.env.services.dialog.add(LinkDialog, {
                ...
                onClose: () => {
                    this.oiEditor.historyUnpauseSteps();
                    this.oiEditor.historyRevertUntil(historyStepIndex)
                }
            });
        }
    }
    ...
    openChatGPTDialog(mode = 'prompt', options={}) {
        const restore = preserveCursor(this.oiEditor.document);
        const params = {
            insert: content => {
                this.oiEditor.historyPauseSteps();
                const insertedNodes = this.oiEditor.execCommand('insert', content);
                this.oiEditor.historyUnpauseSteps();
                ...
                if (start && end) {
                    const divContainer = this.oiEditor.editable.parentElement;
        ...
        if (mode === 'alternatives' || mode === 'translate') {
            params.originalText = this.oiEditor.document.getSelection().toString() || '';
        }
        if (mode === 'translate') {
            params.language = options.language;
        }
        this.oiEditor.document.getSelection().collapseToEnd();
    ...
    removeLink() {
        if (this.state.showSnippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.mutex.exec(() => {
                this.oiEditor.dispatchEvent(new Event('deactivate_image_link_tool'));
            });
        } else {
            this.oiEditor.execCommand('unlink');
        }
    }
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.oiEditor.document.getSelection();
        ...
        const restoreSelection = preserveCursor(this.oiEditor.document);

        const editable = OiEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.oiEditor.editable;
    ...
    // todo: test me
    showEmojiPicker() {
        const targetEl = this.oiEditor.document.getSelection();
        ...
        this.popover.add(closest, EmojiPicker, {
                onSelect: (str) => {
                    restoreSelection();
                    this.oiEditor.execCommand('insert', str);
                },
            }, {
    ...
    _onMediaDialogSave(params, element) {
        ...
        if (params.node) {
            ...
            this.oiEditor.unbreakableStepUnactive();
            ...
            if (params.node.matches(".oe_unremovable")) {
                ...
                this.oiEditor.historyStep();
                this.oiEditor.observerUnactive("unremovable");
                element.classList.add("oe_unremovable");
                this.oiEditor.observerActive("unremovable");
            } else {
                this.oiEditor.historyStep();
            }

            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.oiEditor.editable.focus();
        } else {
            const result = this.oiEditor.execCommand('insert', element);
            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.oiEditor.editable.focus();
            return result;
        }
    ...
    getInSelection(selector) {
        return getInSelection(this.oiEditor.document, selector);
    }
    /**
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await oi.loader.modules.get('@web_editor/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _configureToolbar(options) {
        ...
        const openTools = e => {
            ...
            switch (e.currentTarget.id) {
                ...
                case 'open-chatgpt': {
                    this.openChatGPTDialog(this.oiEditor.document.getSelection()?.isCollapsed ? 'prompt' : 'alternatives');
                    break;
                }
            }
        };
        ...
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.oiEditor.document});
            const destroyTransfo = () => {
                $image.transfo('destroy');
                $(this.oiEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                this.oiEditor.document.removeEventListener('keydown', keydown);
            }
            const mouseup = () => {
                imgTransformBtn.classList.toggle('active', $image[0].matches('[style*="transform"]'));
            };
            $(this.oiEditor.document).on('mouseup', mouseup);
            ...
            $(this.oiEditor.document).on('mousedown', mousedown);
            ...
            this.oiEditor.document.addEventListener('keydown', keydown);
        });
    ...
    _showImageCrop() {
        ...
        this.oiEditor.toolbarHide();
        $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.oiEditor.toolbarShow());
    }
    /**
     * @private
     * @param {jQuery} $
    ...
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        const selectedTds = this.oiEditor.document.querySelectorAll('td.o_selected_td');
        ...
        let coloredElements = this.oiEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        // Some nodes returned by applyColor can be removed of the document by the sanitization in historyStep
        coloredElements = coloredElements.filter(element => this.oiEditor.document.contains(element));
        ...
        if (selectedTds.length === 1 && !previewMode) {
            ...
            range.setEnd(...endPos(last));
                const { startContainer, startOffset, endContainer, endOffset } = getDeepRange(this.oiEditor.editable, { range });
            if (isSelForward) {
        ...
        this.oiEditor.updateColorpickerLabels({
     ...
     */
    _updateEditorUI(e) {
        let selection = this.oiEditor.document.getSelection();
        if (!selection) return;
        const anchorNode = selection.anchorNode;
        if (isProtected(anchorNode)) {
            return;
        }
        if (this.oiEditor.document.querySelector(".transfo-container")) {
            return;
        }

        this.oiEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.oiEditor.document.defaultView;
        ...
        if (isInMedia) {
            ...
            const selection = this.oiEditor.document.getSelection();
            const range = this.oiEditor.document.createRange();
        ...
        this.oiEditor.updateToolbarPosition();
    ...
    _updateMediaJustifyButton(commandState) {
        ...
        // Change the ID to prevent OiEditor from controlling it as this is
    ...
    _getBannerCommand(title, emoji, alertClass, iconClass, description, priority) {
        return {
            ...
            isDisabled: () => isSelectionInSelectors('.o_editor_banner') || !this.oiEditor.isSelectionInBlockRoot(),
            callback: () => {
                const bannerElement = parseHTML(this.oiEditor.document, `
                ...
                `).childNodes[0];
                this.oiEditor.execCommand('insert', bannerElement);
                this.oiEditor.activateContenteditable();
    ...
    _getPowerboxOptions() {
        ...
        const commands = [
            ...this._getBannerCommands(),
            {
                ...
                isDisabled: () => !this.oiEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.oiEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.oiEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.oiEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (currentUser && currentUser.signature) {
                        this.oiEditor.execCommand('insert', parseHTML(this.oiEditor.document, currentUser.signature));
                    }
                },
            },
            {
            ...
            commands.push(
                {
                    ...
                    callback: () => this.oiEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.oiEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                        const anchor = this.oiEditor.document.getSelection().anchorNode;
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.oiEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.oiEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.oiEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.oiEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.oiEditor.execCommand('columnize', 0),
                    isDisabled: () => {
                        if (!this.oiEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
    ...
    async _saveElement($el, context) {
        ...
        // remove ZeroWidthSpace from oi field value
        // ZeroWidthSpace may be present from OiEditor edition process
    ...
    _onSelectionChange() {
        if (this.linkPopover && this.isSelectionInEditable()) {
            const selectionInLink = getInSelection(this.oiEditor.document, 'a') === this.linkPopover.target;
            const isVisible = this.linkPopover.el.offsetParent;
            if (
                isVisible &&
                (
                    (this.options.autohideToolbar && !this.oiEditor.document.getSelection().isCollapsed) ||
                    !selectionInLink
                )
            ) {
                this.linkPopover.hide();
    ...
    async _signalOnline() {
        ...
        this.ptp.notifyAllClients('oe_history_step', peek(this.oiEditor.historyGetSteps()), { transport: 'rtc' });
    }
    /**
    ...
    async _processMissingSteps(missingSteps) {
        ...
        this.ptp && this.oiEditor.onExternalHistorySteps(missingSteps);
        return true;
    }
    _showConflictDialog() {
        if (this._conflictDialogOpened) return;
        const content = markup(this.oiEditor.editable.cloneNode(true).outerHTML);
    ...
    _getNewPtp() {
        ...
        return new PeerToPeer({
            ...
            onRequest: {
                ...
                get_missing_steps: (params) => this.oiEditor.historyGetMissingSteps(params.requestPayload),
                get_history_from_snapshot: () => this._getHistorySnapshot(),
                get_collaborative_selection: () => this.oiEditor.getCurrentCollaborativeSelection(),
                recover_document: (params) => {
                    const { serverDocumentId, fromStepId } = params.requestPayload;
                    if (!this.oiEditor.historyGetBranchIds().includes(serverDocumentId)) {
                        return;
                    }
                    return {
                        missingSteps: this.oiEditor.historyGetMissingSteps({ fromStepId }),
            ...
            onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                switch (notificationName) {
                    case 'ptp_remove':
                        this.oiEditor.multiselectionRemove(notificationPayload);
                        break;
                    case 'ptp_disconnect':
                        this.ptp.removeClient(fromClientId);
                        this.oiEditor.multiselectionRemove(fromClientId);
                        break;
                    case 'rtc_data_channel_open': {
                        ...
                        const getClientNamePromise = this.requestClient(
                            ...
                            this.oiEditor.multiselectionRefresh();
                        });
                    ...
                    case 'oe_history_step':
                        if (this._historySyncFinished) {
                            this.oiEditor.onExternalHistorySteps([notificationPayload]);
                        } else {
                    ...
                    case 'oe_history_set_selection': {
                        ...
                        this.oiEditor.onExternalMultiselectionUpdate(selection);
                        break;
                    }
                }
            }
    ...
    async _setCollaborativeSelection(fromClientId) {
        ...
        if (remoteSelection) {
            this.oiEditor.onExternalMultiselectionUpdate(remoteSelection);
        }
    }
    /**
    ...
    _isLastDocumentStale() {
        ...
        return !this.oiEditor.historyGetBranchIds().includes(this._serverLastStepId);
    }
    /**
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    'recover_document', {
                        ...
                        fromStepId: peek(this.oiEditor.historyGetBranchIds()),
                    },
                    { transport: 'rtc' }
                ).then((response) => {
                    nbPendingResponses--;
    ...
    async _resetFromServerAndResyncWithClients() {
        ...
        // lastHistoryId will be different if the oi bus did not have time to
        // notify the user.
    ...
    async _resetFromClient(fromClientId, resetCollabCount) {
        ...
        if (this._historyStepsBuffer.length) {
            this.oiEditor.onExternalHistorySteps(this._historyStepsBuffer);
            this._historyStepsBuffer = [];
        }
    ...
    async resetValue(value) {
        ...
        if (this._serverLastStepId) {
            this.oiEditor.historySetInitialId(this._serverLastStepId);
        }
    }
    /**
    ...
    async resetEditor(value, options) {
        ...
        this._currentClientId = this._generateClientId();
        this.oiEditor.collaborationSetClientId(this._currentClientId);
        this.resetValue(value);
    ...
    _getHistorySnapshot() {
        return Object.assign(
            {},
            this.oiEditor.historyGetSnapshotSteps(),
    ...
    _applySnapshot(snapshot) {
        ...
        this._historySyncAtLeastOnce = true;
        this.oiEditor.historyResetFromSteps(steps, historyIds);
        this.oiEditor.historyResetLatestComputedSelection();
        return true;
    }
    /**
    ...
    _onPostSanitize(node) {
        ...
        // solution is merged.
        if (node?.querySelectorAll && this.oiEditor && !this.oiEditor._fixLinkMutatedElements) {
    ...
    _attachHistoryIds(editable = this.oiEditor.editable) {
        if (this.options.collaborative) {
            ...
            const historyIds = this.oiEditor.historyGetBranchIds().join(',');
    ...
    _beforeAnyCommand() {
        ...
        for (const node of OiEditorLib.getTraversedNodes(this.$editable[0])) {
