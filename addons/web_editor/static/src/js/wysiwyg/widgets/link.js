...
import { deduceURLfromText } from "@web_editor/js/editor/oi-editor/src/utils/sanitize";

const { getDeepRange, getInSelection, EMAIL_REGEX, PHONE_REGEX } = OiEditorLib;
...
export class Link extends Component {
    ...
    _isAbsoluteURLInCurrentDomain(url) {
        ...
        return urlObj.origin === window.location.origin
            ...
            // its Oi instance domain. We just suppose it should be a relative
            // URL (if unexpected behavior, the user can just not enter its Oi
            // instance domain but its real domain, or opt-out from the domain
            // stripping). Mentioning an .odoodooi.com domain, especially its own
            // one, is always a bad practice anyway.
            || new RegExp(`^https?://${session.db}\\.oi\\.com(/.*)?$`).test(urlObj.origin);
    }
    /**
    ...
    async _updateState(props) {
        ...
        if (this.state.range) {
            this.$link = this.$link || $(OiEditorLib.getInSelection(this.editable.ownerDocument, 'a'));
