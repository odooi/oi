...
import { getAdjacentPreviousSiblings, isBlock, rgbToHex, commonParentGet } from '../editor/oi-editor/src/utils/utils';
...
export async function toInline($editable, options) {
    $editable.removeClass('oi-editor-editable');
    const editable = $editable.get(0);
    ...
    $editable.addClass('oi-editor-editable');
}
/**
...
function fontToImg($editable) {
    const editable = $editable.get(0);
    const { fonts } = oi.loader.modules.get("@web_editor/js/wysiwyg/fonts");
