...
from oi.addons.hw_drivers.main import iot_devices, manager
...
class ConnectionManager(Thread):
    ...
    def run(self):
        if not helpers.get_oi_server_url() and not helpers.access_point():
    ...
    def _connect_box(self):
        ...
        try:
            ...
            req = requests.post('https://iot-proxy.odoodooi.com/oi-plus/iot/connect-box', json=data, verify=False)
            ...
            if all(key in result for key in ['pairing_code', 'pairing_uuid']):
                ...
                self._refresh_displays()
            elif all(key in result for key in ['url', 'token', 'db_uuid', 'plus_code']):
                self._connect_to_server(result['url'], result['token'], result['db_uuid'], result['plus_code'])
        except Exception:
            _logger.exception('Could not reach iot-proxy.odoodooi.com')

    def _connect_to_server(self, url, token, db_uuid, plus_code):
        # Save DB URL and token
        helpers.save_conf_server(url, token, db_uuid, plus_code)
