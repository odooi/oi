...
from oi.netsvc import DBFormatter

_logger = logging.getLogger(__name__)

IOT_LOG_TO_SERVER_CONFIG_NAME = 'iot_log_to_server'  # config name in oi.conf
...
class AsyncHTTPHandler(logging.Handler):
    ...
    def __init__(self, oi_server_url, active):
        """
        :param oi_server_url: Oi Server URL
        """
        super().__init__()
        self._oi_server_url = oi_server_url
    ...
    def toggle_active(self, is_active):
        ...
        if self._active and self._oi_server_url:
    ...
    def _periodic_flush(self):
        ...
        while self._oi_server_url and self._active:  # allow to exit the loop on thread.join
            time.sleep(self._FLUSH_INTERVAL)
            self._flush_logs(oi_session)

    def _flush_logs(self, oi_session):
        ...
        if not self._oi_server_url or queue_size == 0:
            return
        try:
            oi_session.post(
                self._oi_server_url + '/iot/log',
                data=empty_queue(),
...
_server_log_sender_handler = AsyncHTTPHandler(helpers.get_oi_server_url(), get_oi_config_log_to_server_option())
...
# Set it in the 'root' logger, on which every logger (including oi) is a child
